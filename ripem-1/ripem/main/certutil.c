...
FILE *outStream;
{
  ...
  PrintCertNameAndDigest (&certStruct, innerDER, innerDERLen);
...
void PrintCertNameAndDigest (certStruct, innerDER, innerDERLen)
CertificateStruct *certStruct;
...
unsigned int innerDERLen;
{
  ...
  fputs ("User: ", stderr);
  WritePrintableName (stderr, &certStruct->subject);
  fprintf (stderr, "\n");

  fprintf (stderr, "User certificate digest: ");
  for (i = 0; i < digestLen; ++i)
    fprintf (stderr, "%02X ", (int)digest[i]);
  fprintf (stderr, "\n");
}
