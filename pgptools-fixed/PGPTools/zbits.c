 ...
 *  INTERFACE
 *
 *      void bi_init (struct fifo *zipfile)
...
local struct fifo *zfile; /* output zip file */
...
#define PUTSHORT(w) \
{  (void) fifo_put ((char)((w) & 0xff), zfile); \
   (void) fifo_put ((char)((ush)(w) >> 8), zfile); \
}
...
void bi_windup()
{
    ...
    bi_buf = 0;
    bi_valid = 0;
/*    if (ferror (zfile)) zip_error ("write error on zip file");
(not for fifos) */
#ifdef DEBUG
