/*____________________________________________________________________________
	...
	$Id: RDdraw.c,v 1.7 1997/10/20 14:53:53 wjb Exp $
____________________________________________________________________________*/
...
BOOL GetValidityDrawPrefs(PGPBoolean *pbDisplayMarginal,
						  PGPBoolean *pbMarginalInvalid)
{
	PGPPrefRef	PrefRefClient=NULL;
	PGPError	err;

	*pbDisplayMarginal=FALSE;
	*pbMarginalInvalid=FALSE;
	...
	if(IsntPGPError(err))
	{
		PGPGetPrefBoolean (PrefRefClient,
			kPGPPrefDisplayMarginalValidity, pbDisplayMarginal);
		PGPGetPrefBoolean (PrefRefClient,
			kPGPPrefMarginalIsInvalid, pbMarginalInvalid);
...
LISTSTRUCT *GetListStruct(HWND hwndList)
{
	...
	lsRet=&((RecGbl(GetParent(hwndList)))->lsUser);

	if(lsRet->hwndlist==hwndList)
		return lsRet;

	lsRet=&((RecGbl(GetParent(hwndList)))->lsRec);
...
void DrawStuff(LPDRAWITEMSTRUCT lpDrawItem)
{
	...
	ImageList_Draw(ds->hIml,pui->icon,
		...
		ILD_TRANSPARENT);
#else
	hIcon=LoadIcon(g_hinst,MAKEINTRESOURCE(pui->icon));
