 ...
 *
 * $Id: pgpKeyMisc.c,v 1.28 1998/02/24 22:38:23 hal Exp $
 */
	...
	PGPCFBContext *cfb, unsigned *checksump, int old)
{
	...
	if (cfb) {
		if (old) {
			PGPCFBSync(cfb);
			PGPCFBEncrypt(cfb, buf+2, t-2, buf+2);
		} else {
			PGPCFBEncrypt(cfb, buf, t, buf);
		}
	}
	return t;
}
