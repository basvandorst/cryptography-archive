 ...
 *
 * $Id: pgpDSAKey.c,v 1.51.2.1 1998/09/24 18:39:27 hal Exp $
 */
...
dsaPubMaxsig(PGPPubKey const *pubkey, PGPPublicKeyMessageFormat format)
{
	...
	if (format == kPGPPublicKeyMessageFormat_PGP)
		...
		/* BITSTRING, length, 0, SEQUENCE, length, INT, INT */
		PGPUInt32 len, lenlen;
		PGPUInt32 qbytes = bnBytes(&pub->q);
		len = 2*(pgpBnX509LenLen(qbytes+1) + 1 + qbytes+1);
		lenlen = pgpBnX509LenLen(len);
		return 1 + pgpBnX509LenLen(1+lenlen+len) + 1 + 1 + lenlen + len;
	}
	pgpAssert(0);
	return 0;
}
	...
	PGPPublicKeyMessageFormat format)
{
	...
	if (format == kPGPPublicKeyMessageFormat_X509) {
		/* Parse BITSTRING/SEQUENCE header for 509 sig data */
	...
	PGPPublicKeyMessageFormat format)
{
	...
	t = 0;
	if (format == kPGPPublicKeyMessageFormat_X509) {
		/* Put in BITSTRING/SEQUENCE header for 509 sig data */
		PGPUInt32 len_seq, len_bitstr, lenlen_seq, lenlen_bitstr;
		...
		len_bitstr = 1 + 1 + lenlen_seq + len_seq;
