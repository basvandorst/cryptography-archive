/*____________________________________________________________________________
	...
	$Id: pgpRSAKey.c,v 1.55.2.2.2.1 1998/11/12 03:22:12 heller Exp $
____________________________________________________________________________*/
...
rsaPubMaxsig(PGPPubKey const *pubkey, PGPPublicKeyMessageFormat format)
{
	...
	if (format == kPGPPublicKeyMessageFormat_PGP)
		...
		/* BITSTRING, length bytes, 0, data */
		return 1 + bnBytes(&pub->n) + 1 + pgpBnX509LenLen(bnBytes(&pub->n)+1);
	...
	PGPPublicKeyMessageFormat format)
{
	...
	if (format == kPGPPublicKeyMessageFormat_X509) {
		/* Parse BITSTRING header for 509 sig data */
		PGPByte const *sigp = sig + off;
		PGPUInt32 len;
		if (pgpBnX509TagLen(&sigp, &len) != X509_TAG_BITSTRING) {
			i = kPGPError_MalformedKeyComponent;
			goto done;
		}
		/* BITSTRING should have 0 leftover bits */
		if (*sigp++ != 0) {
			i = kPGPError_MalformedKeyComponent;
			goto done;
		}
		...
		/* The integer is directly embedded in the bitstring, not as an INT */
		format = kPGPPublicKeyMessageFormat_PKCS1;
	}
	i = pgpBnGetFormatted(&bn, sig+off, siglen-off, bnBytes(&pub->n), format);
	if (i <= 0)
...
rsaSecMaxsig(PGPSecKey const *seckey, PGPPublicKeyMessageFormat format)
{
	...
	if (format == kPGPPublicKeyMessageFormat_PGP)
		...
		/* BITSTRING, length bytes, 0, data */
		return 1 + bnBytes(&sec->s.n) + 1 +
			 			pgpBnX509LenLen(bnBytes(&sec->s.n)+1);
	...
	PGPPublicKeyMessageFormat format)
{
	...
	if (i >= 0) {
		t = 0;
		if (format == kPGPPublicKeyMessageFormat_X509) {
			/* Put in BITSTRING header for 509 sig data */
			PGPUInt32 len_bitstr, lenlen_bitstr;
			len_bitstr = 1 + bnBytes(&sec->s.n);
			lenlen_bitstr = pgpBnX509LenLen(len_bitstr);
			sig[t++] = X509_TAG_BITSTRING;
			if (--lenlen_bitstr == 0) {
				sig[t++] = len_bitstr;
			} else {
				sig[t++] = 0x80 | lenlen_bitstr;
				len_bitstr <<= 8 * (4-lenlen_bitstr);
				while (lenlen_bitstr--) {
					sig[t++] = (PGPByte)(len_bitstr >> 24);
					len_bitstr <<= 8;
				}
			}
			sig[t++] = 0;	/* no excess bits in bitstring */
			/* Value is put in bitstring directly, not formatted as an INT */
			format = kPGPPublicKeyMessageFormat_PKCS1;
		}
