/*____________________________________________________________________________
	...
	$Id: pgpKeyMan.c,v 1.181.2.4.2.1 1998/11/12 03:22:01 heller Exp $
____________________________________________________________________________*/
#include <string.h>
...
static PGPError
sChangePassphraseInternal (PGPKeyDB *keyDB, RingSet const *ringset, 
	...
	)
{
	...
	err = sChangePassphraseInternal (key->keyDB, ringset, key->key, NULL,
	...
	)
{
	...
	err = sChangePassphraseInternal (subkey->key->keyDB, ringset,
	...
	PGPSize *		outLength )
{
	...
	*outLength	= 0;
	...
	err	= pgpGetSigStringInternal (ringset, cert->cert,
			propname, outData, bufferSize, outLength );
...
sGetPrimaryUserID (
	PGPKey *		 key,
	PGPBoolean		 getAttribute,
	PGPAttributeType attributeType,
	PGPUserIDRef *	 outRef)
{
	const PGPUserID *	userID;
	RingSet	const	*ringset;
	PGPBoolean		 attributeOK = FALSE;
	PGPUInt32		 type;
	PGPError		 err	= kPGPError_NoErr;
	...
	do
	{
		...
		if( getAttribute )
		{
			if( ringNameIsAttribute( ringset, userID->userID ) )
			{
				(void) ringNameAttributeSubpacket( userID->userID, ringset,
												   0, &type, NULL, NULL );
				attributeOK = ((PGPAttributeType)type == attributeType);
			}
			else
			{
				attributeOK = FALSE;
		...
		{
			attributeOK = !ringNameIsAttribute( ringset, userID->userID );
		}
	...
	PGPUserIDRef *	outRef)
{
	...
	*outRef	= NULL;
	...
	return sGetPrimaryUserID (key, FALSE, (PGPAttributeType)0, outRef );
}
	...
	PGPUserIDRef *	outRef)
{
	...
	*outRef	= NULL;
	...
	return sGetPrimaryUserID( key, TRUE, attributeType, outRef );
}
