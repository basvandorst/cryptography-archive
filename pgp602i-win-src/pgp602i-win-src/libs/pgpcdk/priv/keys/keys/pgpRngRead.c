 ...
 *
 * $Id: pgpRngRead.c,v 1.84 1998/08/04 23:49:18 hal Exp $
 */
#include "pgpConfig.h"
...
#define RINGNAME_MAXLEN	128000			/* Name/attribute maximum size */
...
ringFileCloseList(union RingObject **objp, RingFile *file)
{
	...
	while ((obj = *objp) != NULL) {
		...
		if (pgpIsRingSetMember(&file->set, obj) ||
			 ...
			 * Except if they have children, because they may have a
			 * child obj in the MEMRING which didn't get deleted
...
{
	PGPByte *certbuf;			/* X.509 cert we want to find signer of */
	PGPSize certlen;
	PGPByte *certinfo;			/* cert info subpart of cert */
	PGPSize certinfolen;
	PGPByte *issuername;		/* issuer name subpart of cert */
	PGPSize issuerlen;
	PGPByte *subjectname;		/* subject name of other key's cert */
	PGPSize subjectlen;
	PGPByte *pktcopy;			/* copy of signature packet */
	PGPSize pktlen;
	PGPByte *sigbuf;			/* sig of other X.509 key */
	PGPSize siglen;
	...
	certbuf = (PGPByte *)ringSigFindSubpacket ((PGPByte *)pool->pktbuf,
					SIGSUB_X509, 0, &certlen, NULL, NULL, NULL, NULL);
	
	pgpAssert (IsntNull( certbuf ) );
	
	/* Find issuer name in cert */
	pgpX509CertPartition (certbuf, certlen, &certinfo, &certinfolen,
						  NULL, NULL, NULL, NULL);
	pgpX509CertInfoPartition (certinfo, certinfolen, NULL, NULL,
							  NULL, NULL, &issuername, &issuerlen, NULL, NULL,
							  NULL, NULL, NULL, NULL);
	...
	/* Correct issurename pointer to be in copy */
	issuername = pktcopy + (issuername - (PGPByte *)pool->pktbuf);
		...
		for (names=keys->g.down; names; names = names->g.next) {
			...
			for (sigs=names->g.down; sigs; sigs = sigs->g.next) {
				if (OBJISSIG(sigs) && SIGISX509(&sigs->s)) {
					...
					certbuf = (PGPByte *)ringSigFindSubpacket (sigbuf,
						SIGSUB_X509, 0, &certlen, NULL, NULL, NULL, NULL);
					pgpX509CertPartition (certbuf, pool->pktbuflen,
						&certinfo, &certinfolen, NULL, NULL, NULL, NULL);
					pgpX509CertInfoPartition (certinfo, certinfolen,
						NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,
						&subjectname, &subjectlen, NULL, NULL);
					if (issuerlen == subjectlen &&
						0 == memcmp (issuername, subjectname, issuerlen)) {
						/* Have a match! */
						break;
					}
				}
 ...
 */
	...
	for (name=key->g.down; name; name=nextname) {
		...
		if (pos == -1)
			return kPGPError_FileOpFailed;
		attrtype = 0;
		ringNameAttributeSubpacket (name, set, 0, &attrtype, NULL, NULL);
	...
	int trusted, PGPByte pkttype)
{
	...
	err = ringKeyParse(file->set.pool->context,
	                   ...
	                   keyID, &keybits, &tstamp, &validity, &v3, 0);
	...
	PGPByte pkttype)
{
	...
	err = ringKeyParse(file->set.pool->context,
					   (PGPByte const *)file->set.pool->pktbuf,
	                   file->set.pool->pktbuflen, &pkalg,
	                   keyID, &keybits, &tstamp, &validity, &v3, 1);
 ...
 */
static PGPError
ringSigFixupX509Dummy (RingPool *pool, RingObject *topkey)
{
	PGPByte *certbuf;
	PGPSize certlen;
	PGPByte *certinfo;
...
ringAddSig(RingFile *file, RingIterator *iter, PGPUInt32 fpos, int trusted)
{
	 ...
	 */
	if (SIGISX509 (&sig->s)) {
		ringSigFixupX509Dummy (file->set.pool, iter->stack[0]);
	}
