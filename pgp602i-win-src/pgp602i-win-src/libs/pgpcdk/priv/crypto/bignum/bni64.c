	...
	BNWORD64 const *e, unsigned elen, BNWORD64 *mod, unsigned mlen)
{
	...
	BNIALLOC(mgr, secure, a, BNWORD64, 2*mlen);
	if (!a)
		return -1;
	BNIALLOC(mgr, secure, b, BNWORD64, 2*mlen);
	if (!b) {
		BNIFREE(a, 2*mlen);
		return -1;
	}
	 ...
	 */
	for (i = 1; i < tblmask; i++) {
		BNIALLOC(mgr, secure, t, BNWORD64, mlen);
   ...
   BNWORD64 *mod, unsigned mlen)
{
	...
	if (!e2len)
		return bniExpMod_64( mgr, secure, result, n1, n1len, e1, e1len,
							mod, mlen);
	pgpAssert(e2bits);
	...
	if (e1bits == 1) {
		...
		BNIALLOC(mgr, secure, a, BNWORD64, n1len+n2len);
		if (!a)
			return -1;
	...
	BNWORD64 *a, unsigned alen, BNWORD64 const *mod, unsigned mlen)
{
	...
	BNIALLOC(mgr, secure, b, BNWORD64, mlen+1);
	if (b) {
		/*
		 ...
		 */
		BNIALLOC(mgr, secure, p, BNWORD64, mlen+1);
		if (p) {
			BNIALLOC(mgr, secure, t0, BNWORD64, mlen);
			if (t0) {
				BNIALLOC(mgr, secure, t1, BNWORD64, mlen);
				if (t1)
						goto allocated;
				BNIFREE(t0, mlen);
			}
