 ...
 *
 * $Id: pgpPrsBin.c,v 1.56.2.1 1998/10/21 05:59:27 heller Exp $
 */
#include "pgpConfig.h"
...
typedef struct PrsBinContext {
	...
	PGPUICb const *ui;
	void *ui_arg;
	PGPByte beginline[sizeof(pgp_message_begin)-1];	/* for literal recursion */
	PGPSize beginlength;				/* chars in beginline */
	...
	*error = kPGPError_NoErr;
	...
	case 1:
		...
		if (ctx->subtype == PGP_LITERAL_TEXT) {
			...
			ctx->beginlength = inputMerged(&ctx->input) - (p[1]+6);
			pgpAssert (ctx->beginlength <= sizeof(ctx->beginline));
			pgpCopyMemory (p+p[1]+6, ctx->beginline, ctx->beginlength);
		}
		*error = ctx->tail->annotate (ctx->tail, myself,
		                              PGPANN_LITERAL_TYPE, &ctx->subtype, 1);
		if (*error)
	...
	case 5:
		...
		if( myself->write == DoWrite && (size+ctx->beginlength) == 0 ) {
			ctx->tail->write( ctx->tail, buf, 0, error );
		}
		if (*error)
			break;
		/* Write any chars we peeked-ahead above */
		ctx->input.head.pktlen += ctx->beginlength;	/* counted this already */
		len = myself->write(myself, ctx->beginline, ctx->beginlength, error);
		if (*error)
			break;
		/* We assume it will accept them all! */
		pgpAssert(len == ctx->beginlength);
