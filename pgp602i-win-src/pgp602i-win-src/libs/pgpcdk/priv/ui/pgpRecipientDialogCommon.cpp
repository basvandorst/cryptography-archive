/*____________________________________________________________________________
	...
	$Id: pgpRecipientDialogCommon.cpp,v 1.32.10.1 1998/11/12 03:23:02 heller Exp $
____________________________________________________________________________*/
	...
	PGPKeySetRef		keySet)
{
	...
	static PGPError
CountUniqueGroupItems(
	PGPGroupSetRef	groupSet,
	PGPGroupID		groupID,
	PGPUInt32 *		outNumKeys,
	PGPUInt32 *		outTotal)
{
	PGPGroupSetRef	flattenedSet;
	PGPError		err;
	...
	err = PGPNewGroupSet( PGPGetGroupSetContext( groupSet ), &flattenedSet );
	if( IsntPGPError( err ) )
	{
		PGPGroupID	flattenedID;
		
		err = PGPNewFlattenedGroupFromGroup( groupSet, groupID, flattenedSet,
					&flattenedID );
		if( IsntPGPError( err ) )
		{
			err = PGPCountGroupItems( flattenedSet, flattenedID, TRUE,
					outNumKeys, outTotal );
		}
		
		PGPFreeGroupSet( flattenedSet );
	}
...
/* Must use PGPFreeData to dispose of foundGroups */

	static PGPError
FindGroupsFromUserID(
	PGPRecipientsList	*recipients,
	const char			*userID,
	PGPUInt32			*numFoundGroups,
	PGPRecipientUser	***foundGroups)
{
	PGPError			err = kPGPError_NoErr;
	PGPRecipientUser	*curUser;
	PGPUInt32			markValue;
	...
	*foundGroups = (PGPRecipientUser **) PGPNewData( recipients->memoryMgr,
								*numFoundGroups * sizeof( **foundGroups ),
								kPGPMemoryMgrFlags_Clear );
	if( IsntNull( *foundGroups ) )
	{
		PGPUInt32	userIndex = 0;
