/*____________________________________________________________________________
	...
	$Id: RDkeyDB.c,v 1.14.10.1 1998/11/12 03:24:25 heller Exp $
____________________________________________________________________________*/
				...
				PUSERKEYINFO *UserLinkedList)
{
	...
	switch(pru->kind)
	{
		...
		case kPGPRecipientUserKind_Group:
		{
			...
			{
				LoadString(g_hinst, IDS_NUMKEYS, StrRes, sizeof(StrRes));

				sprintf(pui->szSize,"%d keys",pru->groupInfo.numKeys);
			}
			else
			{
				LoadString(g_hinst, IDS_NUMOFNUMKEYS, StrRes, sizeof(StrRes));
