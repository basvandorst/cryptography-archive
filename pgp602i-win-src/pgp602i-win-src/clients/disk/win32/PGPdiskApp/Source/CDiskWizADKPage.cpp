...
// $Id: CDiskWizADKPage.cpp,v 1.1.2.6.2.2 1998/09/01 22:12:10 nryan Exp $
...
CDiskWizADKPage::OnSetActive() 
{
...

	{
		...
		PGPBoolean				gotAllKeys, gotNewContext, isThereADK;
		PGPContextRef			context;
		PGPKeySetRef			allKeys;

		gotAllKeys = gotNewContext = isThereADK = FALSE;

		// Create new PGP context.
		notFatalDerr = PGPNewContext(kPGPsdkAPIVersion, &context);
		gotNewContext = notFatalDerr.IsntError();

		// Is there an ADK?
		if (notFatalDerr.IsntError())
		{
			notFatalDerr = IsTherePGPdiskADK(context, &isThereADK);
		}

		// Load default key ring if ADK.
		if (notFatalDerr.IsntError() && isThereADK)
		{
			fatalDerr = PGPOpenDefaultKeyRings(context, 0, &allKeys);
		...
		{
			fatalDerr = GetADKUserID(context, allKeys,userID, 
				kMaxStringSize);
		}
