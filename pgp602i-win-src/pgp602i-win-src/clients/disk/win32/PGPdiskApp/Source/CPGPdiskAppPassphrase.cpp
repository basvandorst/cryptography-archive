...
// $Id: CPGPdiskAppPassphrase.cpp,v 1.1.2.21.2.3 1998/10/22 22:27:47 nryan Exp $
...
CPGPdiskApp::AddRemovePGPdiskPublicKeys(LPCSTR path)
{
	...
	CSinglePassphraseDialog		masterPassDlg(kSPDT_Master);
	DualErr						derr;
	PGPBoolean					gotContext, gotAllKeys, gotExistingKeys;
	PGPContextRef				context;
	PGPKeySetRef				allKeys;
	PGPRecipientSpec			*existRecipKeys;
	PGPUInt32					numRecipKeys;

	gotContext = gotAllKeys = gotExistingKeys = FALSE;
	...
	// Initialize PGP context.
	if (derr.IsntError())
	{
		derr = PGPNewContext(kPGPsdkAPIVersion, &context);
		gotContext = derr.IsntError();
	}

	// Load SDK prefs.
	if (derr.IsntError())
	{
		derr = PGPsdkLoadDefaultPrefs(context);
	}

	// Open default key rings.
	if (derr.IsntError())
	{
		derr = PGPOpenDefaultKeyRings(context, 0, &allKeys);
	...
	if (derr.IsntError())
	{
		derr = GetPGPdiskRecipKeys(path, context, allKeys, &existRecipKeys, 
			&numRecipKeys);
	...
	if (derr.IsntError())
	{
		derr = publicKeyDialog.AskForKeys(context, allKeys, existRecipKeys, 
			numRecipKeys);
	}
	...
	if (derr.IsntError())
	{
		derr = UpdatePGPdiskPublicKeys(path, 
			masterPassDlg.mPassphraseEdit.mContents, context, allKeys, 
