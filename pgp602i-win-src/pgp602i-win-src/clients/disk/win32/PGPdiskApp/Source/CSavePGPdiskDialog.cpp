...
// $Id: CSavePGPdiskDialog.cpp,v 1.2.2.17.2.1 1998/08/22 01:55:36 nryan Exp $
...
CSavePGPdiskDialog::AskForPath(CString *path, LPCSTR defaultPath)
{
	...
	try
	{
		...
		else
		{
			...
			PGPdiskGetPrefString(kRegistryLastSaveDirKey, kEmptyString, 
				&initialDir);
		}
...
CSavePGPdiskDialog::UpdateSaveDialogPrefs()
{
	...
	try
	{
		...
		PGPdiskSetPrefString(kRegistryLastSaveDirKey, dir);
	}
	catch (CMemoryException *ex)
	{
		ex->Delete();
