...
// $Id: CPGPdiskApp.cpp,v 1.2.2.43.2.1 1998/10/25 23:31:26 nryan Exp $
...
typedef int __stdcall NetReviewFunc	
(HWND hwndParent, 
DWORD dwBindFlags);
...
CPGPdiskApp::InitInstance()
{
	...
	if (derr.IsntError())
	{
		// Show splash screen if not in command-line mode.
		if (!mCommandLineMode)
			ShowSplashScreen();
...
{
	CAboutBoxDialog	aboutBox;
	DualErr			derr;

	derr = aboutBox.DisplayAboutBox();
...
{
	CSplashScreenDialog	splashDialog;
	PGPUInt32			currentDayIndex, lastDayIndex;
	PGPUInt64			currentTime, lastDateSplashed;

	// Get the current time.
	currentTime = GetSecondsSince1970();
	currentDayIndex = (PGPUInt32) (currentTime / kSecondsInADay);

	// Get the last time we splashed the user.
	lastDateSplashed = PGPdiskGetPrefQWord(kRegistryLastDateSplashKey, 
		kInvalidRegQWordValue);

	lastDayIndex = (PGPUInt32) (lastDateSplashed / kSecondsInADay);

	// If the key doesn't exists or if we are on a different day than the
	// last time we splashed, splash now.

	if ((lastDateSplashed == kInvalidRegQWordValue) || 
		(currentDayIndex > lastDayIndex))
	{
		splashDialog.ShowSplashScreen();

		// Remember that we splashed.
		PGPdiskSetPrefQWord(kRegistryLastDateSplashKey, currentTime);
	}
}
