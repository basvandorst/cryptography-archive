/*____________________________________________________________________________
	...
	$Id: PKgenwiz.c,v 1.100.2.2 1998/10/05 21:30:56 pbj Exp $
____________________________________________________________________________*/
...
#define KGWIZ_DONE		15
#define NUM_WIZ_PAGES	16
...
sInstallWindowsHooks (VOID)
{
	...
	hhookJournalRecord = SetWindowsHookEx (WH_JOURNALRECORD, 
		(int (__stdcall *)(void))sWizGenericHookProc, 
		NULL, dwThreadID);
...
sKeyGenerationThread (void *pArgs)
{
	...
	if (PGPRefIsValid (keyDefault)) bNewDefaultKey = FALSE;
		...
		LPARAM	lParam) 
{
	...
	if (sWizCommonNewKeyPhraseMsgProc (hWnd, uMsg, wParam, lParam)) return 0;
	switch (uMsg) {
	case WM_GETTEXT :
		wParam = lPhrase1Len;
...
sCreateKeyGenWizard (VOID *pArg)
{
	...
	if (!KGInfo.bFinishSelected) {
		PGPKeySetRef KeySet;
		if (KGInfo.Key) {
			PGPclErrorBox (NULL, PGPNewSingletonKeySet (KGInfo.Key, 
															&KeySet));
	...
	PGPKeySetRef	keyset) 
{
	...
	ul = _beginthreadex (	NULL, 0, 
							(LPTHREAD_START_ROUTINE)sCreateKeyGenWizard, 
