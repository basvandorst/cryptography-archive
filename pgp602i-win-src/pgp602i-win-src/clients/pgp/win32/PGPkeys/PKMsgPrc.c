/*____________________________________________________________________________
	...
	$Id: PKMsgPrc.c,v 1.165.2.3 1998/10/28 23:11:34 pbj Exp $
____________________________________________________________________________*/
...
sImportPrefsKeys (PGPKeySetRef keysetMain)
{
	...
	if (IsntPGPError (err)) {
		...
		PGPclCloseAdminPrefs (prefref, bSave);
	}
...
static VOID 
sReloadKeyrings (PGPKEYSSTRUCT* ppks,PGPBoolean *UserCancel) 
{
	BOOL				bMutable			= TRUE;
		...
		case kPGPError_FileLocked :
			...
			if (iLockTries < WRITELOCKTRIES) {
				...
				PKMessageBox (g_hWndMain, IDS_CAPTION, IDS_LOCKEDKEYRING,
					MB_OK|MB_ICONSTOP);
				if (!PKPGPPreferences (ppks, g_hWndMain, 1)) {
					*UserCancel=TRUE;
		...
		LPARAM	lParam) 
{
	...
	case WM_CREATE :
		...
		ppks->hKM = PGPkmCreateKeyManager (g_Context, g_TLSContext,
							hWnd, IDC_TREELIST, PKDialogListFunc, 
							0, ppks->iToolHeight, 
							rc.right-rc.left, 
							ppks->iKeysHeight);
...
///		}
		...
		if (ppks->bMainWindow) {
			...
			if (!PGPkmOKToClose (ppks->hKM, &uReason)) {
				if (uReason == KMR_EXISTINGSPLITKEYDLGS) {
					if (!PKSplitKeyWarn (hWnd)) break;
				}
			}

			if (bKeyHasBeenGenerated) {
				if (!PKBackupWarn (hWnd, &bMakeBackups)) break;
			}
	...
	case WM_ENABLE :
		ppks = (PGPKEYSSTRUCT*)GetWindowLong (hWnd, GWL_USERDATA);
		if (ppks->bMainWindow) PKEnableDlgList ((BOOL)wParam);
		if (wParam)
	...
	case WM_COPYDATA :
		ppks = (PGPKEYSSTRUCT*)GetWindowLong (hWnd, GWL_USERDATA);
		return (sImportData (ppks, (PCOPYDATASTRUCT)lParam));
	...
	case WM_TIMER :
		...
		if (wParam == RELOADTIMERID) {
			PGPBoolean UserCancel;

			KillTimer (hWnd, wParam);

			// do the actual loading
			bIgnoreReloads = TRUE;
			sReloadKeyrings (ppks, &UserCancel);
			bIgnoreReloads = FALSE;

			// wjb if we cancel, we need to break out of here since
			// ppks will be freed by PGPkeys WM_DESTROY routine
			if(UserCancel)
				break;
			...
			if (bFirstKeyringLoad) {
				...
				// see if it's time to auto-update any keys
