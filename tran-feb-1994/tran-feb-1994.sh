From cme@world.std.com Wed Feb 23 22:19:37 PST 1994
Article: 23680 of sci.crypt
Newsgroups: sci.crypt
Path: vanbc.wimsey.com!deep.rsoft.bc.ca!agate!howland.reston.ans.net!cs.utexas.edu!uunet!world!cme
From: cme@world.std.com (Carl M Ellison)
Subject: tran.sh
Message-ID: <CLo3BH.81r@world.std.com>
Organization: The World Public Access UNIX, Brookline, MA
Date: Wed, 23 Feb 1994 07:54:52 GMT
Lines: 394

The following source code is offered as an example of how one might write
stream XOR or transposition encryption programs.  Although these programs
might give the impression of encrypting their source streams, they are not
encryption programs.  They take no key.  They are each self-inverses.

Therefore, they achieve no privacy -- any more than gzip does.
...
X	prngxor <tmpo |tran|prngxor > tmp ; diff foo tmp
X
Xtmp: foo
X	cp foo tmp
X
...
if test "$Sum" != "371"
then echo original size 371, current size $Sum;fi
fi
...
if test -f prngxor.c; then echo "File prngxor.c exists"; else
echo "x - extracting prngxor.c (Text)"
sed 's/^X//' << 'SHAR_EOF' > prngxor.c &&
X/* prngxor.c  */
X/* modified from tran.c, by Carl Ellison, 21 Aug 93 */
X
X/* This program uses a fixed key and is therefore useless as an */
X/* encryption program.  It is offered as an example of how such a */
X/* program might be written. */
X
X/* prngxor uses XOR to mask the bytes of the input file. */
X/*  prngxor | prngxor is a no-op  */
X/* The original tran.c was written by setzer@math.ncsu.edu (William Setzer) */
X
X#include <stdio.h>
X
...
X
X  for (i=0;i<256;i++) key[i] = i ; /* init the ranno seed key */
X  set_rnd_seed(key);		/* init the ranno generator with key */
X
X  while (len = fread(buf, 1, BLOCKSIZE, infp)) 
X    {
X      for ( i = 0; i < len; i++) buf[i] ^= (rnd() & 0xff) ;
X      fwrite(buf, 1, len, outfp);
X    }
X}
SHAR_EOF
...
if test "$Sum" != "1252"
then echo original size 1252, current size $Sum;fi
fi
...
X   a preset random vector for each of the 48 seed values.  Rather than
X   keep 48 full random vectors of length 256, this uses vectors offset
X   by 10 values -- using an array of 800 random numbers.
X
...
if test "$Sum" != "9849"
then echo original size 9849, current size $Sum;fi
fi
...
X/* Modified to init the PRNG from a histogram of the input bytes. */
X
X#include <stdio.h>
X
Xextern void set_rnd_seed();
...
if test "$Sum" != "1520"
then echo original size 1520, current size $Sum;fi
fi
exit 0
