...
{"Arrow keys find, space bar activates OR",
 ...
 "Software, 94 Shangan Rd,Dublin,Ireland "};     /******/
...
#ifdef MIRACL_32
    int386(0x10,&regs,&regs);  /* Use Dos Interrupt 10h */
#else    
...
static void apchar(attr,x,y,ch)
int attr,x,y;
char ch;
...
static void aprint(attr,x,y,text)
int attr,x,y;
char* text;
{ /* attribute print */
    while (*text!='\0')
...
{ /* set status line */
    int i;
    if (MR_ERNUM) instat(erptr,"ERROR");
    else       instat(erptr,"     ");
...
static void show(force)
bool force;
{ /* output display */
    if (force || strcmp(oldstat,status)!=0)
    {
        if (MR_ERNUM) aprint(BLINKING,2,2,status);
...
{ /*  MIRACL rational calculator */
    ...
    screen();
#ifdef MIRACL_16
    mirsys(10,0);      /*** 16-bit computer ***/
#else
    mirsys(6,0);       /*** 32-bit computer ***/
#endif
    x=mirvar(0);
    ...
    loge10=mirvar(0);
    eps=mirvar(0);
    cinstr(mr_w15,cpi);            /* read in constants */
    fpmul(mr_w15,1,180,radeg);
    cinstr(loge2,clg2);
    cinstr(loge10,clg10);
    cinstr(eps,ceps);
    help=OFF;
