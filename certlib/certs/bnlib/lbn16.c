...
volatile const char * volatile const lbnCopyright = bnCopyright;
 ...
 * the inversion will be done on an int and the valie won't be quite what
 * is expected.
 */
 ...
 * One of the building blocks, along with addn1, of adding two bignums of
 * differing lengths.
 *
 * Technique: Maintain a word of carry.  If there is no double-width type,
 * use the same technique as in addn1, above, to maintain the carry by
 ...
 * n+1-word product.  This uses either the umul16_ppmm and umul16_ppmma
 ...
 * of *that*.)  This uses either the umul16_ppmma and umul16_ppmmaa macros,
 ...
 * This uses either the umul16_ppmm and umul16_ppmma macros, or
...
#if !defined(lbnMontMul) && defined(BNWORD32) && PRODUCT_SCAN
/*
 ...
 * faster, since it can assume normalization.
 */
#ifndef BN_SLOW_DIVIDE_32
#define BN_SLOW_DIVIDE_32 1
#endif
 ...
 *   First, estimate a q digit so that nh/dh works.  Subtracting q*dh from
 *   the (nh.h nh.l) list leaves a 1/2-word remainder r.  Then compute the
 *   low part of the subtractor, q * dl.   This also needs to be subtracted
 *   from (nh.h nh.l nl.h) to get the final remainder.  So we take the
 *   remainder, which is (nh.h nh.l) - q*dl, shift it and add in nl.h, and
 *   try to subtract q * dl from that.
 *   It is possible that the remainder we're working with, r, is less than
 *   the product q * d1, due to an error in estimating q.  The estimation
 *   technique can produce a q that is too large (never too small), leading
 *   to r which is too small.  In that case, decrement the digit q, add
 ...
 * (shift) normalization of the divisor.  This is hairy!
 *
 ...
 * by 2^(16*mlen).  I'll fill in an explanation of Montgomery representation
 * later.  Returns the result in the *top* n words of the argument n.
 * This is ready for another multiplication using mulnn.
 * 
...
/*
 * Convert a number to Montgomery form - requires mlen + nlen + 1 words
 * of memory in "n".
 */
void
 ...
 * multiples by powers of the two bases are independent, so
 ...
 * drops below w1bits+1).  When the most-significant bit of each windows
 ...
 * result.
 * TODO: Use Lehmer's high-precision GCD algorithm (Knuth 4.5.2, Algorithm L).
 */
int
