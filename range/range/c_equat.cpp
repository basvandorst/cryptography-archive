...
cout <<"                        for complex solutions\n\n";
cout <<"        All equation coefficients and constants may be complex\n";
...
if (svar::input_from_file == FALSE && svar::input_logs_allowed == TRUE) {
     ofstream input_log("c_equat.log");
     input_log << ltos(n) << "\n" << ltos(d1) << "\n" << ltos(d2) << "\n";
...
do {
     C = B / A;
     if (cmatrix::failure) { // C cannot be computed
          temp = abs(det(A));
