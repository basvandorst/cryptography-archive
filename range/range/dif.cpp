...
const int N = 9; // N is max order of dif. equat
...
cout << "\
Program to solve a differential equation of order R in the form\n\
               u'   = f(x, u)          if R = 1,\n\
               u''  = f(x, u, u')      if R = 2,\n\
               u''' = f(x, u, u', u'') if R = 3,\n\
               etc\n\
for the function u and its first R-1 derivatives over an interval [x0, x1].\n\
The initial value of u and its first R-1 derivatives at x0 must be given \n\
and the function f must be analytic in a region of its argument space \n\
...
int_entry("Enter the order R of the equation    ", R, 1, N, "", !var1);
symbol = svector(R+1); value = svector(R+1);
f_entry = svector(R); u0 = vector(R); u0_entry = svector(R); 
symbol(R) = svar("x"); 
for (i=1; i<R; i++) symbol(R) += svar(" ");
for (i=0; i<R; i++) {
     var1 = svar("u"); var2 = svar("");
     for (k=0; k<i; k++) var1 += svar("\'");
     if (i < R-1) f_entry(i) = var1 + svar("\'");
     for (k=i+1; k<R; k++) var2 += svar(" ");
     symbol(i) = var1 + var2;
     if (i == 0) variables = var1; else variables += svar(",") + var1;}
variables += svar(",x");
cout << "\n"; list_tokens(temp);
for (i=0; i<R-1; i++) entry("", zero, f_entry(i), variables);
entry("Enter the function f   ", zero, f_entry(R-1), variables);
entry("Enter x0   ", x0, x0_entry);
for (i=0; i<R; i++) {
     var1 = svar("Enter the value at x0 of ") + symbol(i) + svar("  ");
...
if (svar::input_from_file == FALSE && svar::input_logs_allowed == TRUE) {
     ofstream input_log("dif.log");
     input_log << ltos(R) << "\n";
     input_log << f_entry(R-1) << "\n";
     input_log << x0_entry << "\n";
