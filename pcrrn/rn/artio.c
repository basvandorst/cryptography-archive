...
ART_NUM artnum;
{
...
#ifdef SERVER
	sprintf(artname, "i:/tmp/rrn%ld.art", (long) openart);
	UNLINK(artname);
#endif SERVER
    ...
    sprintf(artname,"i:/tmp/rrn%ld.art", (long) artnum);
    artfp = fopen(artname, "wb+");	/* create the temporary article */
    if (artfp == Nullfp) {
	UNLINK(artname);
	return Nullfp;
    }
    ...
    if (artfp = fopen(artname,"rb"))	/* if we can open it */
