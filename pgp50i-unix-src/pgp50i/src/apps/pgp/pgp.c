 ...
 *
 * $Id: pgp.c,v 1.1.2.4.2.16 1997/10/08 21:59:33 quark Exp $
 */
...
mainAddKeys (void *arg, char const *filename)
{
	...
	buf = pgpMemAlloc(strlen(prefix) + strlen(filename) + 1 + 32);
	if (!buf)
		return PGPERR_NOMEM;
	...
	strcpy(buf, prefix);
	strcat(buf, filename);
	if(pgpenvGetInt(ui_arg->env, PGPENV_BATCHMODE, NULL, NULL))
...
mainGetPass (char *passp, struct UIArg *ui_arg)
{
	...
	do {
		...
		pgpTtyGetPass (showpass, pass, sizeof(pass), ui_arg->arg.env);
		InteractionOutput(TRUE, "NEED_PASSPHRASE_AGAIN");
		pgpTtyGetPass (showpass, pass2, sizeof(pass2), ui_arg->arg.env);
		   ...
		   struct PgpFileRead *readhead, struct UIArg *ui_arg)
{
	...
	Boolean HaveRSARecip = FALSE, HaveNonRSARecip = FALSE;
	byte pkalg;
	...
	if (pgpFileTypeBinary (pgpenvGetString (env, PGPENV_LANGUAGE,
		...
		if (pgpFileTypePGP (ptr, len)) {
			...
			if (!pgpenvGetInt (ui_arg->arg.env, PGPENV_BATCHMODE,
				...
				if (pgpTtyGetBool (0, 1, ui_arg->arg.env))
	...
	if (flags->numrecips) {
		...
		if (! publicKey) {
			...
			/*If Encrypt to self is on, add us to the list of recipients:*/
			if (pgpenvGetInt (env, 
			...
			while ((e = ringIterNextObject (ringiter, 1)) > 0) {
			  ...
			  if (ringObjectType (ringobj) == RINGTYPE_KEY) {
				  ...
				  if(pkalg == PGP_PKALG_RSA ||
					 pkalg == PGP_PKALG_RSA_ENC ||
					 pkalg == PGP_PKALG_RSA_SIG) {
					  HaveRSARecip = TRUE;
				  }
				  else
					  HaveNonRSARecip = TRUE;
			  }
			}
	...
	if(HaveRSARecip && HaveNonRSARecip) {
		WarningOutput(TRUE,
					  LEVEL_SEVERE,
					  "RSA_AND_DH_RECIPS");
	}
...
appMain (int argc, char *argv[])
{
	...
	if((p = getenv("PGPPASSFD")) && strlen(p) > 0)
	{
		int FD = atol(p);
		unsigned x = 0, done = 0;
		char achar;

		while(x < (sizeof(phrase) - 1) && !done)
		{
			if(read(FD, &achar, 1))
			{
				if(achar != '\n')
		...
		pgpPassCacheAdd (passcache, phrase, strlen (phrase));
	}

	InitializeOutputHandlesToConfigValues(env);
