 ...
 *
 * $Id: pgpRingUI.c,v 1.6.2.3.2.3 1997/08/18 22:37:59 quark Exp $
 */
			...
			union RingObject *key)
{
    ...
    for (i = 4; i < 8; i++) {
	*(dest + destcount++) = hexchar[buf[i] >> 4 & 15];
	*(dest + destcount++) = hexchar[buf[i] & 15];
    }
		       ...
		       union RingObject *key)
{
	...
	InteractionOutput(TRUE, "KEY_IS_DISABLED");
