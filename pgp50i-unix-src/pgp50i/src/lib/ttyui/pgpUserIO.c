 ...
 *
 * $Id: pgpUserIO.c,v 1.5.2.10.2.10 1997/10/08 18:44:35 quark Exp $
 */
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
static Boolean
CheckDevRandom(char **RandomDevice, struct PgpEnv const *env);
 ...
 */
int
pgpTtyGetString(char *buf, int len, int echo, const PgpEnv *env)
{
    int c;
    int n = 0;

    kbCbreak(pgpenvGetInt(env, PGPENV_BATCHMODE, NULL, NULL));
...
pgpTtyGetBool(int def, int echo, const PgpEnv *env)
{
	...
	for (;;) {
	    if (pgpTtyGetString(buf, sizeof (buf), echo, env) == 0)
			return def;
		switch (buf[0]) {
			case 'y':
			case 'Y':
...
static int
pgpTtyNeedFile (char *buffer, int buflen, const PgpEnv *env)
{
	char *p;

	*buffer = '\0';
	pgpTtyGetString (buffer, buflen, 1, env);
...
int
pgpTtyGetPass (int showpass, char *buffer, int buflen, const PgpEnv *env)
{
	int i;

	InteractionOutput(TRUE, "ENTER_PASSPHRASE");
	i = pgpTtyGetString (buffer, buflen, showpass, env);
	InteractionOutputString(FALSE, "\n");
	return i;
}
...
/*This is intended to read count bits from a random device, if it's available,
 *while not blocking.
 */
int
pgpTtyGetInitialBitsFromRandDevice(struct PgpEnv const *env,
				   unsigned count)
{
    char *RandomDevice = NULL;
    fd_set RandomDevSet;
    int fdDevRandom = -1, SelectReturn = 0;
    unsigned InitialBits = 0, CurrentBits = 0;
    struct timeval tv;
    Boolean GoodSelect = TRUE;

    InitialBits = pgpRandPoolEntropy();
    if((CheckDevRandom(&RandomDevice, env))) {
	if(VerifyRandomDevOSVersion()) {
		...
		tv.tv_usec = 20;
		do {
		    FD_ZERO(&RandomDevSet);
		    FD_SET(fdDevRandom, &RandomDevSet);	
	
		    SelectReturn = select(fdDevRandom + 1, 
					  &RandomDevSet, 
					  NULL, 
					  NULL, 
					  &tv);
		    ...
		    if(SelectReturn > 0 &&
		       FD_ISSET(fdDevRandom, &RandomDevSet)) {
			pgpDevRandomAccum(fdDevRandom, 1);
			CurrentBits = pgpRandPoolEntropy();
			GoodSelect = TRUE;
		    }
		    else
			GoodSelect = FALSE;
    ...
    return(CurrentBits - InitialBits);
}
 ...
 */
void
pgpTtyRandAccum (struct PgpEnv const *env, unsigned count)
/* Get this many random bits ready */
{
	...
	fdKB = kbOpenKbd(O_RDONLY, FALSE); /*Can't fail*/
	FD_SET(fdKB, &AllDevicesSet);
	LargestFD = fdKB;

	if((HasDevRandom = CheckDevRandom(&RandomDevice, env))) {
	    /*
	...
	do {
	    ...
	    StatusOutputString(FALSE, 
			       "%c", 
			       pgpRandPoolEntropy () == randbits ? '?' : '.');
	    randbits = pgpRandPoolEntropy ();

	} while (randbits < count);

    /* Do final display update */
	StatusOutputString(FALSE, "\r   0 *");
        InteractionOutput(TRUE, "ENOUGH_THANK_YOU");
...
pgpTtyNeedInput (void *arg, struct PgpPipeline *head)
{
	...
	for (;;) {
	    ...
	    i = pgpTtyNeedFile (namebuf, sizeof(namebuf), ui->env);
	    if (!i) {
		if (suggested_name[0])
 ...
 "pgpTtyNewOutput: type is a literal type, but not \'t\' or \'b\': %c (%d)\n",
	...
	while (!name || (!moreflag && !use_stdout && !nullflag &&
		...
		if (pgpenvGetInt (ui->env, PGPENV_BATCHMODE, NULL, NULL)) {
		    ...
		    len = pgpTtyNeedFile (buffer, sizeof (buffer), ui->env);
		}
...
pgpTtyUnlockSeckey (void *arg, union RingObject *key,
	struct PgpSecKey *seckey, char const *prompt, const PgpEnv *env)
{
	struct PgpTtyUI *ui = (struct PgpTtyUI *)arg;
	int i = 3;
	int err;
	...
	if(!pgpenvGetInt(env, PGPENV_BATCHMODE, NULL, NULL))
	{
		...
		len = pgpTtyGetPass (ui->showpass, pass, sizeof (pass), env);
		if (!len)
		    break;
...
int
pgpTtyCheckOverwrite (const struct PgpEnv *env, char const *name)
{
        FILE *fp;
	int status = 0;
 ...
 */
static Boolean CheckDevRandom(char **RandomDevice, 
			      const struct PgpEnv *env)
{
    char *DeviceToUse, *prefDevRandom;
    Boolean HasDevRandom = TRUE;
    static Boolean HaveWarnedIsWritable = FALSE, 
	HaveWarnedNotDefault = FALSE,
	HaveWarnedIsUnreadable = TRUE;
    FILE *fDev = NULL;
    ...
    prefDevRandom = (char *)
	pgpenvGetString(env, PGPENV_RANDOMDEVICE, NULL, NULL);

    if(!prefDevRandom)
	prefDevRandom = (char *) "/dev/random";

    if(strcmp(prefDevRandom, "/dev/random") != 0 &&
	!HaveWarnedNotDefault) {
	WarningOutput(TRUE,
		      LEVEL_INFORMATION,
		      "RANDOM_DEVICE_NOT_DEFAULT",
		      "/dev/random");
	HaveWarnedNotDefault = TRUE;
    }
	...
	*RandomDevice = pgpAlloc(sizeof(char) * (strlen(prefDevRandom) + 1));
	...
	if((((stat_buf.st_mode & S_IRWXO) == S_IRWXO) ||
	   ...
	   !HaveWarnedIsWritable) {
	    WarningOutput(TRUE,
			  LEVEL_SEVERE,
			  "RANDOM_DEVICE_WRITABLE",
			  DeviceToUse);
	    HaveWarnedIsWritable = TRUE;
	}
    }
    else {
	HasDevRandom = FALSE;
