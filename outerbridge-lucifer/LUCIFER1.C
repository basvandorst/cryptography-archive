 ...
 *
 * 
 *   Defines for MSDOS (& probably good for CP/M, etc.) by Scott A. Hardy 
 *      (scotth@ganieda.davis.ca.us) April 1994.
 */
#include <stdio.h>
#ifdef MSDOS
#include <fcntl.h> /* for setmode(), etc. */
#include <io.h>
#endif
...
#ifdef MSDOS        
	flushall();
	setmode(fileno(stdin), O_TEXT);  /* kind of a pity to do this... */
...
void ruderr() {
#ifdef MSDOS  /* or CP/M, etc. -- anything using CAPS on command line */      
