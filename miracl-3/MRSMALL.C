...
{ /*  returns x^n mod m  */
    ...
    if (x<0) x+=m;
    r=0;
    if (x==0) return (utype)r;
    r=1;
    if (n==0) return (utype)r;
    sx=x;
    forever
    {
        if (n%2!=0) muldiv(r,sx,(small)0,m,&r);
        n/=2;
        if (n==0) return (utype)r;
        muldiv(sx,sx,(small)0,m,&sx);
    }
}
...
{ /* square root mod a small prime by Shanks method  *
   * returns 0 if root does not exist or m not prime */
    ...
    { /* also relatively easy */
        ...
        if (t==(small)(m-1))
        {
            muldiv((small)4,x,(small)0,m,&t);
            t=spmd(t,(m+3)/8,m);
            muldiv(t,(m+1)/2,(small)0,m,&t);
            return (utype)t;
        }
        return 0;
    }
    ...
    { /* find suitable z */
        ...
        { /* check for composite m */
            if (t==(small)(m-1)) pp=TRUE;
            muldiv(t,t,(small)0,m,&t);
            if (t==1 && !pp) return 0;
        }
        if (t==(small)(m-1)) break;
