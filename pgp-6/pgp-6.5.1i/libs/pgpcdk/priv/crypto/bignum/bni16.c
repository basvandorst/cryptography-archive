 ...
 *
 * $Id: bni16.c,v 1.6 1998/02/22 06:38:18 heller Exp $
 *
 ...
 *
 * If you want to write asm routines, start with bniMulAdd1_16().
 * This is the workhorse of modular exponentiation.  bniMulN1_16() is
 ...
 * Once you have bniMulAdd1_16 defined, writing the other two should
...
#ifndef bniCopy_16
void
bniCopy_16(BNWORD16 *dest, BNWORD16 const *src, unsigned len)
{
...
#ifndef bniZero_16
void
bniZero_16(BNWORD16 *num, unsigned len)
{
	while (len--)
		BIGLITTLE(*--num,*num++) = 0;
}
...
#ifndef bniNeg_16
void
bniNeg_16(BNWORD16 *num, unsigned len)
{
	pgpAssert(len);
...
#ifdef BNWORD32
BNWORD16
bniAdd1_16(BNWORD16 *num, unsigned len, BNWORD16 carry)
{
	BNWORD32 t;
 ...
 * is expected.
 */
#ifndef bniSub1_16	/* If defined, it's provided as an asm subroutine */
#ifdef BNWORD32
BNWORD16
bniSub1_16(BNWORD16 *num, unsigned len, BNWORD16 borrow)
{
	BNWORD32 t;
...
#ifdef BNWORD32
BNWORD16
bniAddN_16(BNWORD16 *num1, BNWORD16 const *num2, unsigned len)
{
	BNWORD32 t;
...
#ifdef BNWORD32
BNWORD16
bniSubN_16(BNWORD16 *num1, BNWORD16 const *num2, unsigned len)
{
	BNWORD32 t;
 ...
 */
static void
bniMontMul_16(BNWORD16 *prod, BNWORD16 const *num1, BNWORD16 const *num2,
...
bniExpMod_16(
	PGPMemoryMgrRef	mgr,
	PGPBoolean		secure,
	BNWORD16 *result, BNWORD16 const *n, unsigned nlen,
	BNWORD16 const *e, unsigned elen, BNWORD16 *mod, unsigned mlen)
{
	...
	elen = bniNorm_16(e, elen);
	if (!elen) {
		/* x ^ 0 == 1 */
		bniZero_16(result, mlen);
		BIGLITTLE(result[-1],result[0]) = 1;
		return 0;
	}
	ebits = bniBits_16(e, elen);
	if (ebits == 1) {
		/* x ^ 1 == x */
		if (n != result)
			bniCopy_16(result, n, nlen);
		if (mlen > nlen)
	...
	BNIALLOC(mgr, secure, a, BNWORD16, 2*mlen);
	if (!a)
		return -1;
	BNIALLOC(mgr, secure, b, BNWORD16, 2*mlen);
	if (!b) {
		BNIFREE(a, 2*mlen);
		return -1;
	}
	 ...
	 */
	for (i = 1; i < tblmask; i++) {
		BNIALLOC(mgr, secure, t, BNWORD16, mlen);
	...
	inv = bniMontInv1_16(mod[BIGLITTLE(-1,0)]);	/* LSW of modulus */
	...
	(void)bniDiv_16(t, a, mlen+nlen, mod, mlen);
	/* Copy into first table entry */
	bniCopy_16(table[0], a, mlen);

	/* Square a into b */
	bniMontSquare_16(b, a, mod, mlen, inv);

	/* Use high half of b to initialize the table */
	t = BIGLITTLE(b-mlen, b+mlen);
	for (i = 1; i < tblmask; i++) {
		bniMontMul_16(a, t, table[i-1], mod, mlen, inv);
		bniCopy_16(table[i], BIGLITTLE(a-mlen, a+mlen), mlen);
#if BNYIELD
	...
	for (;;) {
		...
		if (ebits == multpos) {
			...
			if (isone) {
				...
				bniCopy_16(t, mult, mlen);
				isone = 0;
			} else {
				bniMontMul_16(a, t, mult, mod, mlen, inv);
				/* Swap a and b */
				t = a; a = b; b = t;
			}
		}
		...
		if (!isone) {
			t = BIGLITTLE(b-mlen, b+mlen);
			bniMontSquare_16(a, t, mod, mlen, inv);
			/* Swap a and b */
			t = a; a = b; b = t;
		}
#if BNYIELD
	...
	bniCopy_16(b, t, mlen);
	bniZero_16(t, mlen);
	bniMontReduce_16(b, mod, mlen, inv);
	bniCopy_16(result, t, mlen);
	/*
   ...
   BNWORD16 *mod, unsigned mlen)
{
	...
	/* First, a couple of trivial cases. */
	e1len = bniNorm_16(e1, e1len);
	e2len = bniNorm_16(e2, e2len);

	/* Ensure that the first exponent is the longer */
	e1bits = bniBits_16(e1, e1len);
	...
	if (!e2len)
		return bniExpMod_16( mgr, secure, result, n1, n1len, e1, e1len,
							mod, mlen);
	pgpAssert(e2bits);
	...
	if (e1bits == 1) {
		...
		BNIALLOC(mgr, secure, a, BNWORD16, n1len+n2len);
		if (!a)
			return -1;

		bniMul_16(a, n1, n1len, n2, n2len);
		/* Do a direct modular reduction */
		if (n1len + n2len >= mlen)
			(void)bniDiv_16(a+mlen, a, n1len+n2len, mod, mlen);
		bniCopy_16(result, a, mlen);
	...
	t = BIGLITTLE(a-mlen, a+mlen);
	bniCopy_16(t, n1, n1len);
	bniZero_16(a, mlen);
	/* Do the division - lose the quotient into the high-order words */
	(void)bniDiv_16(t, a, mlen+n1len, mod, mlen);
	/* Copy into first table entry */
	bniCopy_16(table1[0], a, mlen);
	...
	t = BIGLITTLE(b-mlen, b+mlen);
	for (i = 1; i < tblmask; i++) {
		bniMontMul_16(a, t, table1[i-1], mod, mlen, inv);
		bniCopy_16(table1[i], BIGLITTLE(a-mlen, a+mlen), mlen);
#if BNYIELD
		if (bnYield && (y = bnYield()) < 0)
			goto yield;
#endif
	...
	bniCopy_16(t, n2, n2len);
	bniZero_16(a, mlen);
	/* Do the division - lose the quotient into the high-order words */
	(void)bniDiv_16(t, a, mlen+n2len, mod, mlen);
	/* Copy into first table entry */
	bniCopy_16(table2[0], a, mlen);

	/* Square it into a */
	bniMontSquare_16(a, table2[0], mod, mlen, inv);
	/* Copy to b, low half */
	bniCopy_16(b, t, mlen);

	/* Use b to initialize the second table */
	for (i = 1; i < buf2; i++) {
		bniMontMul_16(a, b, table2[i-1], mod, mlen, inv);
		bniCopy_16(table2[i], t, mlen);
#if BNYIELD
		if (bnYield && (y = bnYield()) < 0)
			goto yield;
#endif
	...
	if (buf2 & tblmask) {
		...
		if (mult2pos == e1bits) {
			t = BIGLITTLE(b-mlen, b+mlen);
			if (isone) {
				bniCopy_16(t, b, mlen);	/* Copy low to high */
				isone = 0;
			} else {
				bniMontMul_16(a, t, b, mod, mlen, inv);
				t = a; a = b; b = t;
			}
		}
	}
	...
	for (;;) {
		...
		if (e1bits == mult1pos) {
			...
			if (isone) {
				/* Multiply by 1 is a trivial case */
				bniCopy_16(t, mult1, mlen);
				isone = 0;
			} else {
				bniMontMul_16(a, t, mult1, mod, mlen, inv);
				/* Swap a and b */
				t = a; a = b; b = t;
			}
		}
		...
		if (e1bits == mult2pos) {
			...
			if (isone) {
				/* Multiply by 1 is a trivial case */
				bniCopy_16(t, mult2, mlen);
				isone = 0;
			} else {
				bniMontMul_16(a, t, mult2, mod, mlen, inv);
				/* Swap a and b */
				t = a; a = b; b = t;
			}
		}
...
bniTwoExpMod_16(
	...
	BNWORD16 *n, BNWORD16 const *exp, unsigned elen,
	BNWORD16 *mod, unsigned mlen)
{
	...
	for (;;) {
		...
		if (bitpos & bitword) {
			 ...
			 * the modulus.
			 */
			if (bniDouble_16(a1, mlen) ||
			    bniCmp_16(a1, mod, mlen) > 0)
				(void)bniSubN_16(a1, mod, mlen);
		}
		 ...
		 */
		bniMontSquare_16(b, a1, mod, mlen, inv);
	 ...
	 */
	bniCopy_16(a, a1, mlen);
	bniZero_16(a1, mlen);
	bniMontReduce_16(a, mod, mlen, inv);
	bniCopy_16(n, a1, mlen);
 ...
 */
void
bniExtractBigBytes_16(BNWORD16 const *n, unsigned char *buf,
 ...
 */
void
bniInsertBigBytes_16(BNWORD16 *n, unsigned char const *buf,
                  unsigned lsbyte,  unsigned buflen)
{
 ...
 */
void
bniExtractLittleBytes_16(BNWORD16 const *n, unsigned char *buf,
	unsigned lsbyte, unsigned buflen)
{
	BNWORD16 t = 0;	/* Needed to shut up uninitialized var warnings */
 ...
 */
void
bniInsertLittleBytes_16(BNWORD16 *n, unsigned char const *buf,
                  unsigned lsbyte,  unsigned buflen)
{
	BNWORD16 t = 0;	/* Shut up uninitialized varibale warnings */
 ...
 */
unsigned
bniFromBytes_16(BNWORD16 *a, unsigned char const *b, unsigned blen)
{
	BNWORD16 t;
 ...
 */
int
bniGcd_16(BNWORD16 *a, unsigned alen, BNWORD16 *b, unsigned blen,
	unsigned *rlen)
{
#if BNYIELD
	int y;
	...
	while (blen != 0) {
		(void)bniDiv_16(BIGLITTLE(a-blen,a+blen), a, alen, b, blen);
		alen = bniNorm_16(a, blen);
		if (alen == 0) {
			*rlen = blen;
			return 1;
		}
		(void)bniDiv_16(BIGLITTLE(b-alen,b+alen), b, blen, a, alen);
		blen = bniNorm_16(b, alen);
#if BNYIELD
		if (bnYield && (y = bnYield()) < 0)
			return y;
#endif
...
bniInv_16(
	PGPMemoryMgrRef	mgr,
	PGPBoolean secure,
	BNWORD16 *a, unsigned alen, BNWORD16 const *mod, unsigned mlen)
{
	...
	alen = bniNorm_16(a, alen);
	if (!alen)
		return 1;	/* No inverse */

	mlen = bniNorm_16(mod, mlen);

	pgpAssert (alen <= mlen);

	/* Inverse of 1 is 1 */
	if (alen == 1 && BIGLITTLE(a[-1],a[0]) == 1) {
		bniZero_16(BIGLITTLE(a-alen,a+alen), mlen-alen);
		return 0;
	}

	/* Allocate a pile of space */
	BNIALLOC(mgr, secure, b, BNWORD16, mlen+1);
	if (b) {
		/*
		 ...
		 */
		BNIALLOC(mgr, secure, p, BNWORD16, mlen+1);
		if (p) {
			BNIALLOC(mgr, secure, t0, BNWORD16, mlen);
			if (t0) {
				BNIALLOC(mgr, secure, t1, BNWORD16, mlen);
				if (t1)
						goto allocated;
				BNIFREE(t0, mlen);
			}
...
allocated:
	...
	/* b = mod */
	bniCopy_16(b, mod, mlen);
	/* blen = mlen (implicitly) */
	
	/* t1 = b / a; b = b % a */
	cy = bniDiv_16(t1, b, mlen, a, alen);
	*(BIGLITTLE(t1-(mlen-alen)-1,t1+(mlen-alen))) = cy;
	t1len = bniNorm_16(t1, mlen-alen+1);
	blen = bniNorm_16(b, alen);
	...
	while (blen > 1 || BIGLITTLE(b[-1],b[0]) != (BNWORD16)1) {
		...
		if (alen < blen || (alen == blen && bniCmp_16(a, a, alen) < 0))
			pgpAssert(0);
		cy = bniDiv_16(BIGLITTLE(a-blen,a+blen), a, alen, b, blen);
		*(BIGLITTLE(a-alen-1,a+alen)) = cy;
		plen = bniNorm_16(BIGLITTLE(a-blen,a+blen), alen-blen+1);
		pgpAssert(plen);
		alen = bniNorm_16(a, blen);
		if (!alen)
		...
		bniMul_16(p, BIGLITTLE(a-blen,a+blen), plen, t1, t1len);
		plen = bniNorm_16(p, plen + t1len);
		pgpAssert(plen <= mlen);
		if (plen > t0len) {
			bniZero_16(BIGLITTLE(t0-t0len,t0+t0len), plen-t0len);
			t0len = plen;
		}
		cy = bniAddN_16(t0, p, plen);
		if (cy) {
			if (t0len > plen) {
				cy = bniAdd1_16(BIGLITTLE(t0-plen,t0+plen),
						t0len-plen, cy);
			}
			if (cy) {
		...
		if (alen <= 1 && BIGLITTLE(a[-1],a[0]) == (BNWORD16)1) {
			...
			bniCopy_16(a, t0, t0len);
			bniZero_16(BIGLITTLE(a-t0len, a+t0len), mlen-t0len);
			goto success;
		}
		...
		*(BIGLITTLE(b-blen-1,b+blen)) = cy;
		...
		bniMul_16(p, BIGLITTLE(b-alen,b+alen), plen, t0, t0len);
		plen = bniNorm_16(p, plen + t0len);
		pgpAssert(plen <= mlen);
		if (plen > t1len) {
			bniZero_16(BIGLITTLE(t1-t1len,t1+t1len), plen-t1len);
			t1len = plen;
		}
		cy = bniAddN_16(t1, p, plen);
		if (cy) {
			if (t1len > plen) {
				cy = bniAdd1_16(BIGLITTLE(t1-plen,t0+plen),
						t1len-plen, cy);
			}
			if (cy) {
	...
	/* return mod-t1 */
	bniCopy_16(a, mod, mlen);
	pgpAssert(t1len <= mlen);
	cy = bniSubN_16(a, t1, t1len);
	if (cy) {
		pgpAssert(mlen > t1len);
		cy = bniSub1_16(BIGLITTLE(a-t1len, a+t1len), mlen-t1len, cy);
		pgpAssert(!cy);
	}
