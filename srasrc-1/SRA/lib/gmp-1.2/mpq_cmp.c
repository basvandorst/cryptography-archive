...
#endif
{
  ...
  mp_size tmp1_size, tmp2_size, tmp_size;
  ...
  tmp1_size = (num1_size >= den2_size)
    ? _mpn_mul (tmp1_ptr, op1->num.d, num1_size, op2->den.d, den2_size)
    : _mpn_mul (tmp1_ptr, op2->den.d, den2_size, op1->num.d, num1_size);

  tmp2_size = (num2_size >= den1_size)
    ? _mpn_mul (tmp2_ptr, op2->num.d, num2_size, op1->den.d, den1_size)
    : _mpn_mul (tmp2_ptr, op1->den.d, den1_size, op2->num.d, num2_size);

  cc = _mpn_cmp (tmp1_ptr, tmp1_size, tmp2_ptr, tmp2_size);
