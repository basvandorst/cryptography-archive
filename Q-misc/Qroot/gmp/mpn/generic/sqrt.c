...
mp_size_t
#if __STDC__
mpn_sqrt (mp_ptr root_ptr, mp_ptr rem_ptr, mp_srcptr op_ptr, mp_size_t op_size)
#else
mpn_sqrt (root_ptr, rem_ptr, op_ptr, op_size)
     mp_ptr root_ptr;
     mp_ptr rem_ptr;
...
/* Is there a fast sqrt instruction defined for this machine?  */
  ...
  /* ??? MOVE SOME IF THIS STUFF INSIDE THE IF STMT!  */
  ...
  if (tsize > 2)
      ...
      while (--i >= 0)
	{
	  ...
	  cy_limb = rp[xsize - 1] | (1 << (BITS_PER_MP_LIMB - 1));
	  rp[xsize - 1] = cy_limb;
	  rsize = xsize - (cy_limb == 0);
#ifdef DEBUG
  ...
  if (cnt / 2 != 0)
    {
      cy_limb = mpn_rshift (rp, rp, rsize, cnt/2);
      /* ???      rsize -= cy_limb == 0; */
      rsize -= rp[rsize - 1] == 0;
    }

  /* Calculate the remainder.  */
  mpn_mul (tp, rp, rsize, rp, rsize); /* ??? Use __mpn_mul_n, but allocate tspace!!! */
  tsize = rsize + rsize;
  tsize -= tp[tsize - 1] == 0;
  if (op_size < tsize
      || (op_size == tsize && mpn_cmp (op_ptr, tp, op_size) < 0))
    {
      ...
      mpn_add_1 (tp, tp, tsize, (mp_limb) 1);

      mpn_sub_1 (rp, rp, rsize, (mp_limb) 1);
  ...
  if (rem_ptr != NULL)
    {
      ...
      op_size = mpn_normal_size (rem_ptr, op_size);
      return op_size;
    }
  else
    return op_size != tsize || mpn_cmp (op_ptr, tp, op_size);
}
