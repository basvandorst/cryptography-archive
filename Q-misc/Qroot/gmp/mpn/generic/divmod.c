/* mpn_divmod -- Divide natural numbers, producing both remainder and
   quotient.
...
/* Divide num (NUM_PTR/NUM_SIZE) by den (DEN_PTR/DEN_SIZE) and write
   ...
   2. If QUOT_PTR must either not overlap with the input operands at all,
      or QUOT_PTR + DEN_SIZE >= NUM_PTR must hold true.  (This means that
      it's possible to put the quotient in the high part of NUM, right
      after the remainder in NUM.  */

mp_limb
#if __STDC__
mpn_divmod (mp_ptr quot_ptr,
	    mp_ptr num_ptr, mp_size_t num_size,
	    mp_srcptr den_ptr, mp_size_t den_size)
#else
mpn_divmod (quot_ptr, num_ptr, num_size, den_ptr, den_size)
     mp_ptr quot_ptr;
     mp_ptr num_ptr;
     mp_size_t num_size;
     mp_srcptr den_ptr;
