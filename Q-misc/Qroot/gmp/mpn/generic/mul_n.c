...
void
#if __STDC__
__mpn_mul_n (mp_ptr, mp_srcptr, mp_srcptr, mp_size_t, mp_ptr);
#else
__mpn_mul_n ();
#endif
...
void
#if __STDC__
__mpn_mul_n_basecase (mp_ptr prodp, mp_srcptr up, mp_srcptr vp, mp_size_t size)
#else
__mpn_mul_n_basecase (prodp, up, vp, size)
     mp_ptr prodp;
     mp_srcptr up;
     mp_srcptr vp;
     mp_size_t size;
...
void
#if __STDC__
__mpn_mul_n (mp_ptr prodp,
	     mp_srcptr up, mp_srcptr vp, mp_size_t size, mp_ptr tspace)
#else
__mpn_mul_n (prodp, up, vp, size, tspace)
     mp_ptr prodp;
     mp_srcptr up;
     mp_srcptr vp;
     mp_size_t size;
  ...
  if ((size & 1) != 0)
      ...
      if (cy)
	{
	  if (cy > 0)
	    ...
	    {
	      mpn_sub_1 (prodp + hsize + size, prodp + hsize + size, hsize, cy);
	      abort ();
	    }
	}
...
void
#if __STDC__
__mpn_sqr_n_basecase (mp_ptr prodp, mp_srcptr up, mp_size_t size)
#else
__mpn_sqr_n_basecase (prodp, up, size)
     mp_ptr prodp;
     mp_srcptr up;
     mp_size_t size;
#endif
...
void
#if __STDC__
__mpn_sqr_n (mp_ptr prodp,
	     mp_srcptr up, mp_size_t size, mp_ptr tspace)
#else
__mpn_sqr_n (prodp, up, size, tspace)
     mp_ptr prodp;
     mp_srcptr up;
     mp_size_t size;
     mp_ptr tspace;
