...
/* initkey_idea initializes IDEA for ECB mode operations */
void init_this_key_idea(word16 *ZZ, byte key[16], boolean decryp)
{
	word16 userkey[8];	/* IDEA key is 16 bytes long */
	int i;
	/* Assume each pair of bytes comprising a word is ordered MSB-first. */
	...
	en_key_idea(userkey,ZZ);
	if (decryp)
	{
		de_key_idea(ZZ,ZZ);	/* compute inverse key schedule DK */
	}
	for (i=0; i<8; i++)	/* Erase dangerous traces */
		userkey[i] = 0;
} /* initkey_idea */
...
*/
/* vince */
/* I want to use idea_ecb from outside modules */
