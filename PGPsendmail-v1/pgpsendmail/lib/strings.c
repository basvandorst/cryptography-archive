...
/*  This programme intercepts messages sent by user mail agents to the
  ...
  if  string[str_len]  was an invalid address.
    ...
    Updated by      Richard Gooch   19-JUL-1994: Added  #include <ctype.h>
  so that  toupper(3)  does the right thing under BSDI 1.1
  A "known problem" that is not documented.

    Last updated by Richard Gooch   29-SEP-1997: Created <add_string> function.
...
flag add_string (stringlist_type *list, CONST char *string, flag new_alloc)
/*  [SUMMARY] Add a string to a NULL-terminated list of strings.
    <list> The list of NULL-terminated strings.
    <string> The string to add.
    <new_alloc> If TRUE, a copy of the string is allocated, else only a pointer
    is copied, hence the string must not be externally deallocated.
    [RETURNS] TRUE on succcess, else FALSE.
*/
{
    int new_size;
    char *new;
	...
	if ( ( new = m_alloc (new_size) ) == NULL )
	{
	    fprintf (stderr, "Error allocating string\n");
	    return (FALSE);
	}
	m_copy (new, string, new_size);
	string = new;
    }
    if (list->num_set + 1 >= list->num_allocated)
    {
	/*  Reallocate array of string pointers  */
	new_size = (list->num_allocated * 2);
	if (new_size < 1) new_size = 256;
	if ( ( rec = (char **) m_alloc (sizeof *rec * new_size) ) == NULL )
	{
	    fprintf (stderr,"Error allocating array of string pointers\n");
	    return (FALSE);
	}
	m_copy ( (char *) rec, (char *) list->strings,
		 sizeof *rec * list->num_allocated );
	if (list->strings != NULL) m_free ( (char *) list->strings );
	list->strings = rec;
	list->num_allocated = new_size;
    }
    list->strings[list->num_set++] = (char *) string;
    list->strings[list->num_set] = NULL;
