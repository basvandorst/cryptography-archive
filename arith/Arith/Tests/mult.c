...
char **argv;
{
  static unsigned short aa[MAXL], bb[MAXL];
  unsigned short *a, *b, *q;
  ...
  while (more("another trial") == YES) {
    if (Collect("First Factor  ", a) == YES)
      if (Collect("Second Factor ", b) == YES) {
	q = Zmult(a, b);
	Showme("first factor ", a);
	Showme("second factor", b);
	Showme("their product", q);
      }
  }
