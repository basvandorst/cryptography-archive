...
void initkey_idea(byte key[16], IDEAkey Z, boolean decryp)
{
...
*/
void idea_ecb(word16 *inbuf, word16 *outbuf, IDEAkey Z)
{
	/* Assume each pair of bytes comprising a word is ordered MSB-first. */
 ...
 */
void ideacfb(byteptr buf, int count, IDEAkey Z, word16 *iv, boolean decrypt)
...
/*
	...
	around in memory.
*/
void close_idea(IDEAkey Z)	/* erase current key schedule tables */
{
	short i;
	for (i = 0; i < KEYLEN; i++)
		Z[i] = 0;
...
{
	int i;
	initkey_idea(key,randZ, FALSE);	/* initialize IDEA */
	...
	idea_ecb(dtbuf_idea,dtbuf_idea, randZ);
...
byte idearand(void)
{
	...
	{	/* Combine enciphered timestamp with seed material: */
		...
		idea_ecb(randseed_idea,randbuf_idea, randZ);/* fill new block */

		/* Compute new seed vector: */
		for (i=0; i<4; i++)
			randseed_idea[i] = randbuf_idea[i] ^ dtbuf_idea[i];
		idea_ecb(randseed_idea,randseed_idea, randZ);/* fill new seed */
...
{	/* Erase random IDEA buffers and wipe out IDEA key info */
	...
	close_idea(randZ);	/* erase current key schedule tables */
}	/* close_idearand */
#endif
