/*
** 12/1995 Written by gino (Gino.Corradin@dei.unipd.it)
** See license terms in the root directory
*/
#include <stdio.h>
#include <fcntl.h>
#include <sys/types.h>
...
#define BUF_CRC32_SIZE 8192

static unsigned long int crctab[256] = {
    0x0, 
    0x04c11db7, 0x09823b6e, 0x0d4326d9, 0x130476dc, 0x17c56b6b, 0x1a864db2, 
    ...
    0x164f8078, 0x1b0ca6a1, 0x1fcdbb16, 0x018aeb13, 0x054bf6a4, 0x0808d07d, 
    ...
    0x3b5a6b9b, 0x0315d626, 0x07d4cb91, 0x0a97ed48, 0x0e56f0ff, 0x1011a0fa, 
    ...
    0x1cd86d30, 0x029f3d35, 0x065e2082, 0x0b1d065b, 0x0fdc1bec, 0x3793a651, 
...
/*
** CRC32_Sig : computes a crc checksum
**
** return values :
**      . -1 : error, file not exist
**      .  0 : all right, new and old checksum are identical
**      .  1 : danger, old and new checksum differ
** note :
**      this is a posix 1003.2 checksum.
*/
int crc32_sig(filename, signature)
    char *filename;
    char signature[10];
{
int fin;
register int nr;
register unsigned long int crc32, len;
register unsigned char *p;
unsigned char buffer[BUF_CRC32_SIZE];
char newsum[10];

crc32=len=0;
fin=open(filename, O_RDONLY);
while ((nr=read(fin, buffer, BUF_CRC32_SIZE)) > 0)
    for (len+=nr, p=buffer; nr--; ++p) 
        crc32=crc32 << 8 ^ crctab[crc32 >> 24 ^ *p];
close(fin);
if (nr < 0) return (-1);

/* Include the length of the file. */
for (; len != 0; len >>= 8) 
    crc32=crc32 << 8 ^ crctab[crc32 >> 24 ^ (len & 0xff)];

sprintf(newsum, "%08lx", crc32);
