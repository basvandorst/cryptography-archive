...
int main(long argc, char **argv)
{
  ...
  for(;;)
      ...
      else
	{
        ...
        int   _len;
        int   hist=1;
        if(buffer) {free(buffer);buffer = (char *) NULL;}

        do
	  {
            buffer = readline(prompt);
            if(!buffer) {buffer = (char *) malloc(5);strcpy(buffer,"\\q");}
            tch = buffer;
	  ...
	  for(;;)
        ...
        if(hist && *buffer) add_history(buffer);
	
        /*
            PATCH:
            Ajout d'un return a la fin de la chaine pour etre conforme
            a ce que fait fgets.
        */

        _len = strlen(buffer);
        buffer = (char *)realloc(buffer,_len+2);
        strcpy(buffer+_len,"\n");
...
void escape(char *tch, long parisize, long primelimit)
{
  ...
  switch (isupper(c) ? tolower(c) : c)
    ...
    case 'p': checkok(tch); prettyp = (prettyp==2)?0:prettyp+1;
      if(prettyp==2) 
	{
	  sprintf(thestring, "   default format: prettyprint\n");
	  pariputs(thestring);
	}
      else if(prettyp)
	{
	  sprintf(thestring, "   default format: prettymatrix\n");
	  pariputs(thestring);
	}
      else
	{
	  sprintf(thestring, "   default format: raw\n");
	  pariputs(thestring);
	}
      break;
    case 'q': exit(0);
