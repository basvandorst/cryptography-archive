...
extern DWORD FAR PASCAL GlobalDOSAlloc(DWORD dwBytes);
...
SetupSock()
{
   ...
   HANDLE hAllocaddr;
	...
	DebugSock = GetProfileInt(szAppName,"DebugSock",0);

#ifdef UseDOSAlloc
   allocaddr = GlobalDOSAlloc((DWORD)sizeof(struct addr));
   selector_addr = LOWORD(allocaddr);
   para_addr = HIWORD(allocaddr);
   offset_addr = 0;
   a = (struct addr far *) (((DWORD)selector_addr)<<16);
#else
   hAllocaddr = GlobalAlloc(GMEM_MOVEABLE,(DWORD)sizeof(struct addr));
   a = (struct addr far *) GlobalLock(hAllocaddr);
   allocaddr = (DWORD) a;
   selector_addr = para_addr = HIWORD(allocaddr);
   offset_addr = LOWORD(allocaddr);
#endif
   ...
   /* Set up name structure */
	if(DebugSock) {
		MessageBox(hWndConf,"Ready to _fmemcpy","in SetupSock",MB_OK);
	}
   _fmemcpy(&(a->fhost),&destaddr,sizeof(a->fhost));
   a->lsocket = 0;
   a->fsocket = NNTPPort;
   a->protocol = 0;
   ...
   segregs.ds =  para_addr;          /* HIWORD((char far *)(a)); */
   inregs.x.si = offset_addr;        /* LOWORD((char far *)(a)); */
   inregs.x.dx = STREAM;
...
MRRReadComm()
{
   ...
   /* ifdef USE_NET */
	 ...
	 if(DebugSock) {
	    sprintf(mesbuf,"bytesread=%d '%.30s'",bytesread,CommBuff);
	    MessageBox(hWndConf,mesbuf,"Read bytes in MRRReadComm",MB_OK);
	 }
         CharsInCommBuff = bytesread;
         CommBuffIdx = 0;
      } else {
         sprintf(mesbuf,"NNTPSock=%d  bytesread=%d  neterrno=%d",NNTPSock,bytesread,neterrno);
         MessageBox(hWndConf,mesbuf,"in MRRReadComm, probs",MB_OK|MB_ICONHAND);
         return(-1);
      }
#else
  /* ifndef MAC */
...
#define MAXSENDLEN 256
   ...
   if(UsingSocket) {
      ...
      retcode = int86x(0x61,&inregs,&outregs,&segregs);
		...
		mesbuf[0] = '\n';
		segregs.ds  = HIWORD((char far *)mesbuf);
		inregs.x.si = LOWORD((char far *)mesbuf);
      inregs.x.cx = 1;
