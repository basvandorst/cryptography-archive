...
// This code is intended to let INIT writers easily display an icon at
// startup time.
// View in Geneva 9pt, 4-space tabs

// Written by: Peter N Lewis <peter@mail.peter.com.au>,
// Jim Walker <JWWalker@aol.com> and Francois Pottier <pottier@dmi.ens.fr>,
// with thanks to previous ShowINIT authors.
// Send comments and bug reports to Francois Pottier.

// This version features:
// - Short and readable code.
// - Correctly wraps around when more than one row of icons has been
//   displayed.
// - works with System 6
// - Built with Universal Headers & CodeWarrior. Should work with other
//   headers/compilers.
...
// You should set SystemSixOrLater in your headers to avoid including glue
// for SysEnvirons.

// --------------------------------------------------------------------------
// Set this flag to 1 if you want to compile this file into a stand-alone
...
// --------------------------------------------------------------------------
// The ShowINIT mechanism works by having each INIT read/write data from
// these globals.
// The MPW C compiler doesn't accept variables declared at an absolute
// address, so I use these macros instead.
// Only one macro is defined per variable; there is no need to define a Set
// and a Get accessor like in <LowMem.h>.
...
// --------------------------------------------------------------------------
// Prototypes for the subroutines. The main routine comes first; this is
// necessary to make THINK C's "Custom Header" option work.
...
pascal void ShowInitIcon (short iconFamilyID, Boolean advance)
{
	...
	oldA5 = SetA5((long) &qds.qdGlobalsPtr);	// Tell A5 to point to the
												// end of the fake QD Globals
	InitGraf(&qds.qd.thePort);					// Initialize the fake QD
												// Globals
	
	SysEnvirons(curSysEnvVers, &environment);	// Find out what kind of
												// machine this is

	ComputeIconRect(&destRect, &qds.qd.screenBits.bounds);	// Compute where
															// the icon
															// should be drawn
...
static void ComputeIconRect (Rect* iconRect, Rect* screenBounds)
{
	if (CheckSum(LMHCoord) != LMHCheckSum)	// If we are first, we need to
											// initialize the shared data.
		LMHCoord = 8;
	if (CheckSum(LMVCoord) != LMVCheckSum)
		LMVCoord = screenBounds->bottom - 40;
	
	if (LMHCoord + 34 > screenBounds->right) {	// Check whether we must
												// wrap
		iconRect->left = 8;
...
// DrawBWIcon draws the 'ICN#' member of the icon family. It works under
// System 6.
...
// We (PNL) now check for system 7 and colour QD, and use colour graf ports
// and PlotIconID only if both are true
