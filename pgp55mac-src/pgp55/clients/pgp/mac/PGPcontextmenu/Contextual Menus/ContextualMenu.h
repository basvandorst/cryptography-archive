/*
	...
	Change History (most recent first):
         ...
         <5>     3/11/97    GBF     Add the Contextual Menu gestalt selector
         							codes.
         <4>     2/27/97    GBF     Adding trapwords to the declarations.
         <3>     2/13/97    GBF     Remove DisplayContextualMenu.
         <2>     1/14/97    GBF     Fixing the API for
         							Backwards-compatibility INIT as well as
         							the
...
/*
	...
	Change History (most recent first):
        ...
        <14>     7/28/96    GBF     Trying to make this thing Universal
        							Headers compatible for
                                    Harmony. Moving the plugin specific
                                    stuff to another file.
                                    Removed a bunch of unnecessary #includes.
        <13>     7/12/96    GBF     The last change comment (12) got screwed
        							up; I'm fixing it.
        <12>     7/12/96    GBF     Making the DisplayContextualMenu
        							interface more modern.
                                    replacing kShowBalloon and kShowHelp with
                                    some better enums.
                                    Added some more enums.
        <11>     4/11/96    GBF     Putting kExamineContextNoTimeout back,
        							cuz I'm an idiot.
        <10>     4/11/96    GBF     Moved kExamineContextNoTimeout out of
        							here.
         <9>      4/9/96    GBF     Add IsShowContextualMenuClick function.
         							Cleaned up the API.
         <8>     3/22/96    GBF     Added Timeout constant
         <7>     3/14/96    GBF     Removing the AddContextualMenuItem
         							routine. Adding typedef for
                                    ContextualCommandPair. Adding some enums.
         <6>      3/6/96    GBF     Adding the selection code parameter to
         							AddContextualMenuItem
         <5>      3/6/96    GBF     Added the AddContextualMenuItem routine.
         <4>     2/28/96    GBF     DisplayContextualMenu now returns a
         							SInt32
         <3>     2/27/96    GBF     allow hierarchical menus within a
         							contextual menu
         <2>      2/7/96    arno    Changed Point to Point* to follow
         							standard Maxwell conventions
...
extern	pascal Boolean	IsShowContextualMenuClick(const EventRecord* inEvent)
 TWOWORDINLINE(0x7002, 0xAA72);
	...
	// returned.
extern	pascal OSStatus	ContextualMenuSelect(MenuRef inMenuRef,
