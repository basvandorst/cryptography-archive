...
{
	MD5Init( (MD5_CTX *)unhand( unhand( this )->contextBuf )->body );
}

void java_crypt_MD5_update( struct Hjava_crypt_MD5 * this,
						HArrayOfByte * buffer, long offset, long length )
{
	MD5Update( (MD5_CTX *)unhand( unhand( this )->contextBuf )->body, ( unhand( buffer )->body ) + offset, length );
}

void java_crypt_MD5_finish( struct Hjava_crypt_MD5 * this, HArrayOfByte * outputBuffer )
{
	MD5Final( unhand( outputBuffer )->body, (MD5_CTX *)unhand( unhand( this )->contextBuf )->body );
}
