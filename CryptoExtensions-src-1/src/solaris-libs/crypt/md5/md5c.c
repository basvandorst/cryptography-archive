...
static void native_MD5Transform(unsigned long [4], unsigned char [64]);
 ...
 */
void native_MD5Init(MD5_CTX * context)
{
...
/* MD5 block update operation. Continues an MD5 message-digest
  ...
  context.
 */
void native_MD5Update (MD5_CTX *context, unsigned char * input, unsigned int inputLen)
{
	...
	if (inputLen >= partLen)
	{
		...
		native_MD5Transform (context->state, context->buffer);

		for (i = partLen; i + 63 < inputLen; i += 64)
			native_MD5Transform (context->state, &input[i]);
 ...
 */
void native_MD5Final (unsigned char digest[16], MD5_CTX *context)
{
	...
	native_MD5Update (context, PADDING, padLen);

	/* Append length (before padding) */
	native_MD5Update (context, bits, 8);
 ...
 */
static void native_MD5Transform (unsigned long state[4], unsigned char block[64])
{
