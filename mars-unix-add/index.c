/* index.c
 *
 * this program looks for a set of 4 words in the
 * mars sbox that meet the criteria for the multiplication
 * subkey mask templates.
 */
...
{
    int i, j, k, count;
    /* counts for each 8 bit pattern found */
    int A[256];

    /* look for an index i such that S[i]..S[i+3] are "good" */
    for (i = 0; i < 509; i++) {
        /* initialize A to zeros */
        for (j = 0; j < 256; j++)
            A[j] = 0;
        /* count how many times each 8-bit pattern appears in S[i]..S[i+3] */
        for (k = i; k < i+4; k++) {
            WORD w = S[k]; 
            /* go over all the rotation amounts */
            for (j = 0; j < 32; j++) {   
	        A[ w&0xff ]++;
	        w = (w >> 1) | (w << 31);
            }
        }
        /* make sure that there are no eight 1's or seven 0's */
        /* (this is easier than to check for ten 1's)         */
        if (A[0] != 0 || A[255] != 0 || A[1] != 0 || A[128] != 0)
            continue;
        /* see if any pattern appears more than twice  *
         * (and count how many appear exactly twice)   */
        for (count=0, j = 0; j < 256; j++) {
            if (A[j] > 2) {
	        count = -1; 
	        break;
            }
        ...
        /* if count is -1, reject, else it has number of doubled patterns */
        if (count >= 0)
            printf("positions %d through %d, %d patterns appear twice\n",
	           i, i+3, count);
    }
}
