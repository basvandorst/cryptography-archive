...
static char *DoChangePWDriver
  P ((RIPEMInfo *, BOOL, unsigned int, FILE *, RIPEMDatabase *));
static char *DoGenerateKeysDriver
  P ((RIPEMInfo *, unsigned int, unsigned int, unsigned char *,
  ...
  do {
    ...
    if (Action != ACT_GEN_KEY) {
      ...
      if ((errorMessage = LoginUser
           (&ripemInfo, &RipemDatabase, (unsigned char *)password,
            passwordLen)) != (char *)NULL) {
    ...
    if(Action == ACT_ENCRYPT) {
      ...
      errorMessage = DoChangePWDriver
        (&ripemInfo, needSelfSignedCert, ValidityMonths, CertInfoStream,
         &RipemDatabase);
    } else {
      errorMessage = DoGenerateKeysDriver
        (&ripemInfo, Bits, ValidityMonths, (unsigned char *)password,
...
/* If needSelfSignedCert is TRUE, call WriteSelfSignedCert with
     validityMonths for upgrading from RIPEM 1.1.
     Otherwise ignore validityMonths.
 */
static char *DoChangePWDriver
  (ripemInfo, needSelfSignedCert, validityMonths, certInfoStream,
   ripemDatabase)
...
static char *DoGenerateKeysDriver
  (ripemInfo, bits, validityMonths, password, passwordLen, certInfoStream,
   ripemDatabase)
RIPEMInfo *ripemInfo;
unsigned int bits;
unsigned int validityMonths;
