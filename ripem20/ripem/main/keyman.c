...
/* homeDir is the name of the RIPEM home directory.  It must end in the
   ...
   This adds pubkeys, privkey, and crls in the homeDir to the respective
     keySources in ripemDatabase if not already there. Then it makes sure
     these filenames can be opened for output and are in the front of the
     keySource list as required for database output files.  Then it opens
     all the fileList->stream entries in the keySources for read.
...
/* Look in the public key source files for all records with the smartName and
   ...
   RIPEMInfo is only used for debug and debugStream.  If ripemInfo is
     NULL, no debug information is written.
 */
...
UINT4 time;
{
  ...
  *crlDER = (unsigned char *)NULL;
  ...
  while (GetFileLine
    ...
    if (!match (value, publicKeyHexDigest))
      /* Try next one. */
      continue;
