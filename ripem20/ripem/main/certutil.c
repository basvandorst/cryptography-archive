...
static char *CheckAlreadyValidated
  P((RIPEMInfo *, BOOL *, DistinguishedNameStruct *, R_RSA_PUBLIC_KEY *,
     RIPEMDatabase *));
...
/* Create a self-signed cert with the userDN and publicKey which are
     in ripemInfo and sign it with ripemInfo's private key.  Write it
     to the first filename in ripemDatabase->pubKeySource.
   Set the userCertDER in ripemInfo to the new self-signed cert.
 */
char *WriteSelfSignedCert (ripemInfo, validityMonths, ripemDatabase)
RIPEMInfo *ripemInfo;
...
/* On input, the certStruct has the name and public key to validate,
   ...
   This sets the issuerName to the user in ripemInfo and sets other fields
     to default values.  This then signs with ripemInfo's private key and
     inserts the certificate into the public key database.
   ...
   This returns the error ERR_CERT_ALREADY_VALIDATED if the user in ripemInfo
     has already issued a certificate which has a current validity period
     (even if it was revoked.)  Typically, this won't happen because an
     application only tries to make a new certificate after SelectCertChain
     fails to find one.
   Returns NULL for success, otherwise an error message.
 */
char *ValidateAndWriteCert (ripemInfo, certStruct, ripemDatabase)
RIPEMInfo *ripemInfo;
CertificateStruct *certStruct;
RIPEMDatabase *ripemDatabase;
{
  ...
  do {
    if ((errorMessage = CheckAlreadyValidated
         (ripemInfo, &alreadyValidated, &certStruct->subject,
          &certStruct->publicKey, ripemDatabase)) != 0)
      break;
    if (alreadyValidated) {
...
/* For each cert in certList, first check if it is already in the
     database, and insert it if not.   Skip self-signed certs by skipping
     certs with equal issuer and subject names.
 */
...
{
  CertificateStruct *certStruct = (CertificateStruct *)NULL;
  TypListEntry *certListEntry, *selectedCertsEntry;
  ...
  do {
    ...
    for (certListEntry = certList->firstptr;
      ...
      if ((certLen = DERToCertificate
           ((unsigned char *)certListEntry->dataptr, certStruct,
      ...
      if (R_memcmp
          ((POINTER)&certStruct->issuer, (POINTER)&certStruct->subject,
           sizeof (certStruct->issuer)) == 0)
        /* Self-signed cert, so skip and try the next one. */
        continue;

      /* Make sure selectedCerts list is clear, and select certs from the
           database with a matching smart name.
       */
      ...
      if ((errorMessage = GetCertsBySmartname
           (ripemDatabase, &selectedCerts,
            certStruct->subject.AVAValues[smartNameIndex], ripemInfo))
          != (char *)NULL)
        break;
      ...
      for (selectedCertsEntry = selectedCerts.firstptr;
        ...
        if (R_memcmp
            ((POINTER)certListEntry->dataptr,
             (POINTER)selectedCertsEntry->dataptr, certLen) == 0)
          /* Found a match */
          break;
      }
      ...
      if ((errorMessage = WriteCert
           ((unsigned char *)certListEntry->dataptr, ripemDatabase))
          != (char *)NULL)
        break;
    }
  } while (0);
...
/* For each CRL in crlList, first select a chain for the CRL's issuer
     and verify its signature.  Then check if the CRL is already in the
     database, and insert it if not.
   This assumes the first entry in ripemDatabase->crlSource for files
     is opened in "a" mode for inserting the CRL.
 */
char *InsertCRLs (crlList, ripemInfo, ripemDatabase)
TypList *crlList;
RIPEMInfo *ripemInfo;
RIPEMDatabase *ripemDatabase;
{
  ...
  do {
    ...
    /* Allocate the on the heap since it is so big. */
    ...
    for (crlListEntry = crlList->firstptr;
         crlListEntry != (TypListEntry *)NULL;
         crlListEntry = crlListEntry->nextptr) {
      if ((crlLen = DERToCRL
           ((unsigned char *)crlListEntry->dataptr, crlStruct, &fieldPointers))
            < 0) {
        /* Can't decode, so just skip and try the next one. */
        if (ripemInfo->debug > 1)
          fprintf (ripemInfo->debugStream,
      ...
      if ((errorMessage = MaybeWriteCRL
           ((unsigned char *)crlListEntry->dataptr, (unsigned int)crlLen,
            crlStruct->lastUpdate, publicKeyDigest, ripemDatabase))
          != (char *)NULL)
        break;
    }
  } while (0);
...
void RIPEM_CRLsFrameConstructor (frame)
RIPEM_CRLsFrame *frame;
{
  BufferStreamConstructor (&frame->outStream);
}

void RIPEM_CRLsFrameDestructor (frame)
RIPEM_CRLsFrame *frame;
{
...
unsigned int *partOutLen;
{
  ...
  if ((ripemInfo->z.crlsFrame = (RIPEM_CRLsFrame *)malloc
       (sizeof (*ripemInfo->z.crlsFrame))) == (RIPEM_CRLsFrame *)NULL)
    return (ERR_MALLOC);
  RIPEM_CRLsFrameConstructor (ripemInfo->z.crlsFrame);

  /* Get stream for quick access. */
  stream = &ripemInfo->z.crlsFrame->outStream;
...
/* Produce a CRL message containing the CRL for the user in ripemInfo.
...
RIPEMDatabase *ripemDatabase;
{
  ...
  BufferStream *stream;
  char *errorMessage;
  unsigned char *crlDER = (unsigned char *)NULL, issuerKeyDigest[16];
  int crlLen, status;
  UINT4 now;
  ...
  do {
    ...
    if (crlDER == (unsigned char *)NULL) {
      errorMessage = "CRL for the cannot be found.";
      break;
    }

    if ((crlLen = DERToCRL (crlDER, crlStruct, &fieldPointers)) < 0) {
      errorMessage = "CRL cannot be decoded.";
      break;
    }
    if ((status = R_VerifyBlockSignature
      ...
      sprintf (ripemInfo->errMsgTxt, "%s while checking CRL signature.",
...
unsigned char *alias;
{
  ...
  for (i = 0; i < ripemInfo->z.issuerSerialAliasCount; ++i) {
    if (R_memcmp
        ((POINTER)&ripemInfo->z.issuerSerialAliases[i * 16], (POINTER)alias,
         16) == 0)
      return (TRUE);
  }
...
RIPEMDatabase *ripemDatabase;
{
  ...
  unsigned char *preferencesDER = (unsigned char *)NULL,
    publicKeyDigest[16], signature[16], computedSignature[16], *innerDER;
  ...
  do {
    ...
    R_DigestUpdate (&digestContext, ripemInfo->passwordDigest, 16);
    R_DigestFinal (&digestContext, computedSignature, &digestLen);
    if (R_memcmp ((POINTER)computedSignature, (POINTER)signature, 16) != 0) {
...
RIPEMDatabase *ripemDatabase;
{
  char *errorMessage;
  R_DIGEST_CTX digestContext;
  unsigned char *der, signature[16], publicKeyDigest[16];
  ...
  do {
     ...
     */
    DerUnsignedToDerSigned (der, &derLen, signature, 16, DA_MD5, FALSE);
    if ((errorMessage = GetPublicKeyDigest
         (publicKeyDigest, &ripemInfo->publicKey)) != (char *)NULL)
      break;
    if ((errorMessage = WriteRIPEMPreferences
         (der, derLen, publicKeyDigest, ripemDatabase)) != (char *)NULL)
      break;
  } while (0);
...
/* Select the current CRL for the logged-in RIPEM user, set
     ...
     supplied value, and insert the updated CRL into the database.
 ...
 */
char *RIPEMUpdateCRL
  (ripemInfo, nextUpdate, serialNumber, serialNumberLen, ripemDatabase)
RIPEMInfo *ripemInfo;
UINT4 nextUpdate;
unsigned char *serialNumber;
unsigned int serialNumberLen;
RIPEMDatabase *ripemDatabase;
{
  ...
  CRLFieldPointers fieldPointers;
  char *errorMessage;
  unsigned char *selectedCRL = (unsigned char *)NULL, digest[16],
  ...
  do {
    ...
    if (selectedCRL == (unsigned char *)NULL) {
      /* We need to create a new CRL.  Set issuer name to the logged-
           ...
           work correctly.
       */
      crlStruct->digestAlgorithm = DA_MD2;
...
/* For each name in issuerNames, select the best chain starting from
   ...
   This does not add the "topmost" cert in the chain which is
     the issued the logged-in ripem user, since if someone already
 ...
 */
char *WriteOriginatorAndIssuerCerts (ripemInfo, stream, messageFormat)
RIPEMInfo *ripemInfo;
BufferStream *stream;
int messageFormat;
{
...
RIPEMDatabase *ripemDatabase;
{
  ...
  unsigned int smartNameIndex, currentChainLen, i;
  CertificateStruct *issuerCertStruct = (CertificateStruct *)NULL;
  ...
  do {
    ...
    /* Get the smart name of the subject we're supposed to use in the search */
    GetDNSmartNameIndex (&smartNameIndex, selectedCertsSubject);

    /* Select all certs with the issuer as the subject smartname. */
    if ((errorMessage = GetCertsBySmartname
         (ripemDatabase, &selectedCerts,
          selectedCertsSubject->AVAValues[smartNameIndex], ripemInfo))
        != (char *)NULL)
      break;
    ...
    for (entry = selectedCerts.firstptr; entry; entry = entry->nextptr) {
      ...
      if (R_memcmp
          ((POINTER)&issuerInfo->certStruct->subject,
           (POINTER)selectedCertsSubject, sizeof (*selectedCertsSubject)) != 0)
...
/* Set alreadyValidated to whether there is already a direct certificate
     for the user with name and public key which has a validity period
     which is currently active (even if it is revoked).
 */
static char *CheckAlreadyValidated
  (ripemInfo, alreadyValidated, name, publicKey, ripemDatabase)
RIPEMInfo *ripemInfo;
BOOL *alreadyValidated;
DistinguishedNameStruct *name;
R_RSA_PUBLIC_KEY *publicKey;
  ...
  do {
    ...
    *alreadyValidated = FALSE;
    ...
    if ((errorMessage = SelectCertChain
         (ripemInfo, &certChain, &chainStatus, name, publicKey, TRUE,
          ripemDatabase)) != (char *)NULL)
      break;
    if (chainStatus.overall == 0)
      break;
    ...
    DERToCertificate
      ((unsigned char *)certChain.firstptr->dataptr, certStruct,
       (CertFieldPointers *)NULL);
    R_time (&now);
    if (now >= certStruct->notBefore && now <= certStruct->notAfter)
