 ...
 *
 *  Exit: Returns TRUE if the desired user is found.
 */
BOOL
FindUserInRecord(user,userRec)
char *user;
char *userRec;
{
#define LINELEN 240
  BOOL looking_user=TRUE, right_user=FALSE;
  char *cptr=userRec;
...
char *field;
{
  ...
  while(reading && fgets(line,LINEBUFSIZE,stream)) {
     if(strncmp(line,field,fieldlen)==0) {
       return(TRUE);
     }
     if(LineIsWhiteSpace(line)) reading=FALSE;
  }
  return(FALSE);
}
 ...
 */
BOOL
ExtractPublicKeyLines(inBuf,outBuf,outSize)
char *inBuf;
char *outBuf;
int outSize;
{
  int beglen=strlen(PUB_KEY_STRING_BEGIN), endlen=strlen(PUB_KEY_STRING_END);
  char *begptr;
  int nbytes;
  
  while(strncmp(inBuf,PUB_KEY_STRING_BEGIN,beglen) != 0) {
    if(!NextLineInBuf(&inBuf)) return FALSE;
  }
  NextLineInBuf(&inBuf);
  begptr = inBuf;
  
  while(strncmp(inBuf,PUB_KEY_STRING_END,endlen) != 0) {
    if(!NextLineInBuf(&inBuf)) return FALSE;
  }
