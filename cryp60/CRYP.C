...
#include <fcntl.h>  /* defines "open flags" O_BINARY, ...                */
#include <io.h>     /* defines setmode()                                 */
...
char  name[]  = "CRYP - A data encryption system written in C, with pipes";
...
char  usage1[]  =  "usage: CRYP key [D/I] [<infile] [>outfile]";
char  usage2[]  =  "  D = Decipher";
char  usage3[]  =  "  I = Init CRY.CON file using key and exit";
char  usage4[]  =  "  default = Encipher and update CRY.CON file";
...
int  rs[9];         /* Initial random seeds for the 9 generators         */
int  rc[9];         /* Current value of clear continuation seeds         */
int  rr;            /* Random running combination of ri[0] thru ri[8]    */
int  *pool;         /* Pool of POOLSIZE random integers                  */
char *buf;          /* Buffer for file i/o                               */
char key[MAXKEY+1]; /* The short key string in standard form             */
int  encipher;      /* Boolean, Enciphering a file, FALSE => Deciphering */
int  next = 0;      /* Next random no. routine to use, cycles 0 thru 8   */
int  poolmask =     /* Mask for index to random pool = 2**N - 1          */
       POOLSIZE-1;
int  in;            /* handle of input stream */
int  out;           /* handle of output stream */
FILE *confile;      /* File pointer of continuation file "CRY.CON"       */
FILE *infile;       /* File pointer of input of key file                 */
...
void main( int argc, char *argv[])
{
   ...
   int  *jp;                /* Pointer after last integer in buf         */
   int  ind;                /* Random index into pool of random integers */
   int  nr;                 /* Number of bytes read into buf             */
   int  rc;                 /* Return code from init() and term()        */
   ...
   while ( (nr = fread( buf, 1, BUFSIZE, stdin)) != 0) /* read file      */
   {
	 ...
	 *ip ^= pool[ ind];               /* XOR data with random int    */
	 ...
	 if( ++next == 9)                 /* Cycle to next generator     */
      ...
      if( fwrite( buf, 1, nr, stdout) != nr) /* Write file and test      */
      {                                      /* If write error, exit     */
	 erase();
	 fprintf( stderr, "CRYP: write error no. %d\n", errno);
	 exit( 2);
      }
   }
...
int init( int argc, char *argv[])         /* Initialize the process      */
{
   ...
   {
      fprintf( stderr, "CRYP: not enough memory\n");
      return( 1);
   }
   ...
   encipher = TRUE;                       /* Set default to encipher     */
   ...
   if( argc > 2)
   {
	 ...
	 {
	    fprintf( stderr,
	       "CRYP: cannot open CRY.CON, error no. %d\n", errno);
	    return( 2);
	 }
	 ...
	 {
	    fprintf( stderr,
	       "CRYP: cannot write CRY.CON, error no. %d\n", errno);
	    return( 3);
	 }
	 fclose( confile);
	 ...
	 return( 4);
      ...
      else                                /* Set encipher/decipher flag  */
	encipher =  *argv[2] != 'D' && *argv[2] != 'd';
   }

   in = fileno( stdin);  out = fileno( stdout);
   if ( (setmode( in, O_BINARY)) == -1)   /* switch stdin to binary mode */
   {  perror( "Binary mode not available for stdin\n");
      free( buf);  return( 5);
   }
   if ( (setmode( out, O_BINARY)) == -1)  /* switch stdout to binary mode*/
   {  perror( "Binary mode not available for stdout\n");
      free( buf);  return( 6);
   }

   if( encipher)                          /* Use CRY.CON and encipher    */
   {
	 ...
	 return( 7);
      ...
      {
	 fprintf( stderr,
	    "CRYP: cannot read CRY.CON, error no. %d\n", errno);
	 return( 8);
      }
      fclose( confile);
					  /* Write 1st record of output  */

      if( fwrite( pool, 1, CONSIZE, stdout) != CONSIZE)
      {
	 fprintf( stderr, "CRYP: write error no. %d\n", errno);
	 return( 9);
      }
   }
   else					  /* Read first record of file   */
   {
      if( fread( pool, 1, CONSIZE, stdin) != CONSIZE)
      {
	 fprintf( stderr, "CRYP: read error no. %d\n", errno);
	 return( 10);
      }
   }
...
int term( void)
{
   ...
   if( ferror( stdin))                    /* Test for infile read error  */
   {
      fprintf( stderr, "CRYP: read error no. %d\n", errno);
      return( 11);
   }

   fclose( stdin);                        /* Close files                 */
   fclose( stdout);
...
void convkey( void)
{
   ...
   int  i, j;
   int  keyi[MAXKEY];		 /* Key in integer form, 6 bits each int */
