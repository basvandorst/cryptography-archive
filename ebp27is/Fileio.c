/*      fileio.c  - I/O routines for PGP/EBP.
 ...
 * EBPPATH and the file name if EBPPATH is not terminated with a
 * directory separator.
 */
...
/* The EBPPATH environment variable */

static char EBPPATH[] = "EBPPATH";
 ...
 * not_ebp_extension or something would be better.
 */
boolean no_extension(char *filename)
{
	...
	has_extension(filename, EBP_EXTENSION) || has_extension(filename,
	PGP_EXTENSION) || has_extension(filename, SIG_EXTENSION) ||
#ifdef MACTC5
...
static void truncate_name(char *path, int ext_len)
{
	...
	*p = '\0';
	...
	if (verbose)
	    fprintf(ebpout, "Truncating filename '%s' ", path);
	p[namemax - ext_len] = '\0';
	if (verbose)
	    fprintf(ebpout, "to '%s'\n", path);
    }
	...
	p = file_tail(path);
	if (verbose)
		fprintf(ebpout, LANG("Truncating filename '%s' "), path);
...
#ifdef UNIX
    /* On Unix, default to $HOME/.ebp, otherwise, current directory. */
    else {
	s = getenv("HOME");
	if (s && strlen(s) <= 50) {
	    strcpy(result, s);
...
int readPhantomInput(char *filename)
{
    ...
    if (verbose)
	fprintf(ebpout, "writing stdin to file %s\n", filename);
...
int writePhantomOutput(char *filename)
{
    ...
    if (verbose)
	fprintf(ebpout, "writing file %s to stdout\n", filename);
    ...
    if (ferror(stdout)) {
	status = -1;
	fprintf(ebpout, LANG("\007Write error on stdout.\n"));
    }
#if defined(MSDOS) || defined(OS2) || defined (WIN32)
...
static char tmpbasename[64] = "ebptemp";	/* basename for
						   temporary files */
...
char *tempfile(int flags)
{
  ...
  again:
    ...
    if (verbose)
	fprintf(ebpout, "tempfile: created '%s'\n", tmpf[i].path);
    return tmpf[i].path;
}				/* tempfile */
...
void rmtemp(char *name)
{
	...
	if (strlen(name) > 3 && name[strlen(name) - 3] == TMP_EXT) {
	    ...
	    if (verbose)
		fprintf(ebpout, "rmtemp: removing '%s'\n", name);
	    ...
	    fprintf(ebpout, "rmtemp: not removing '%s'\n", name);
    }
}				/* rmtemp */
...
char *savetemp(char *name, char *newname)
{
	...
	if (strlen(name) < 4 || name[strlen(name) - 3] != TMP_EXT) {
	    if (verbose)
		fprintf(ebpout, "savetemp: not renaming '%s' to '%s'\n",
			name, newname);
    ...
    if (verbose)
	fprintf(ebpout, "savetemp: renaming '%s' to '%s'\n", name, newname);
    if (rename2(name, newname) < 0) {
	/* errorLvl = UNKNOWN_FILE_ERROR; */
	fprintf(ebpout, LANG("Can't create output file '%s'\n"), newname);
	return NULL;
    }
    if (i < MAXTMPF)
	tmpf[i].flags = 0;
...
char *ck_dup_output(char *newname, boolean notest, boolean delete_dup)
{
	...
	if (!overwrite) {
	    fprintf(ebpout, "\n");
	    fprintf(ebpout, LANG("Enter new file name:"));
	    fprintf(ebpout, " ");
#ifdef MACTC5
...
int savetempbak(char *tmpname, char *destname)
{
	...
	if (file_exists(destname)) {
		...
		if (header[0] == CTB_CERT_SECKEY)
			EBPSetFinfo(bakpath,'SKey','MEBP');
		if (header[0] == CTB_CERT_PUBKEY)
			EBPSetFinfo(bakpath,'PKey','MEBP');
#endif
	}
    }
	...
	if (header[0] == CTB_CERT_SECKEY)
		EBPSetFinfo(destname,'SKey','MEBP');
	if (header[0] == CTB_CERT_PUBKEY)
		EBPSetFinfo(destname,'PKey','MEBP');
#endif
    return 0;
}
...
static unsigned doc_missing(char *prefix)
{
	...
	/* on the macintosh we must look for the documents in 
	   Documentation:EBP Documentation: folder  */
	...
	if (len && prefix[len - 1] != DIRSEPS[0])
		prefix[len++] = DIRSEPS[0];
	strcpy(prefix + len, "EBP Documentation");
	mask &= files_missing(prefix);
	if (!mask)
		return 0;
#else
...
{
#if defined(VMS)
    "$EBPPATH", "", "[ebp]", "[pgp]", "[pgp26]", "[pgp263]",
		EBP_SYSTEM_DIR, "SYS$LOGIN:", "SYS$LOGIN:[ebp]", "SYS$LOGIN:[pgp]",
    "SYS$LOGIN:[pgp26]", "SYS$LOGIN:[pgp263]", "[-]",
#elif defined(UNIX)
    "$EBPPATH", "", "ebp", "pgp", "pgp26", "pgp263", EBP_SYSTEM_DIR,
    "$HOME/.pgp", "$HOME/.ebp", "$HOME", "$HOME/ebp", "$HOME/pgp", 
	"$HOME/pgp26", "..",
#elif defined(AMIGA)
    "$EBPPATH", "", "ebp", "pgp", "pgp26", ":pgp", ":pgp26", ":pgp263", 
    ":", "/",
#else				/* MSDOS or ATARI */
    "$EBPPATH", "", "ebp", "ebp27", "\\ebp", "\\ebp27", "pgp", "pgp26", "\\pgp",
	"\\pgp26", "\\pgp263", "\\", "..", "c:\\ebp", "c:\\ebp27", "c:\\pgp",
	"c:\\pgp26",
#endif
    0};
