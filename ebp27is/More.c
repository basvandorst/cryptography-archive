/*      more.c  - Unix-style "more" paging output for PGP/EBP.
...
int more_file(char *fileName, boolean eyes_only)
{
    ...
    /* PAGER set in config.txt overrides environment variable, 
       set PAGER in config.txt to 'ebp' to use builtin pager */
    if (pager[0] == '\0') {
    ...
    if (!eyes_only && (strlen(pager) != 0) && strcmp("ebp", pager)) {
	fclose(inFile);
#ifdef UNIX
...
/*
 * open_more() and close_more() redirect ebpout to the pager.
 *
 */

static char *mfile = NULL;
static boolean piping = FALSE;
static FILE *saveebpout;
...
int open_more(void)
{
    ...
    /* Use built-in pager if PAGER is not set or set to "ebp" */
    if ((strlen(pager) != 0) && strcmp("ebp", pager)) {
	if ((ebpout = popen(pager, "w")) != NULL) {
	    piping = TRUE;
	    return 0;
	}
	perror("popen");
	...
	return -1;
    if ((ebpout = fopen(mfile, FOPWTXT)) == NULL) {
	ebpout = saveebpout;
	rmtemp(mfile);
	return -1;
    }
    /* user will not see anything until close_more() is called */
    fprintf(saveebpout, LANG("Just a moment..."));
    fflush(saveebpout);
    return 0;
}
