/*      keyadd.c  - Keyring merging routines for PGP/EBP.
		     ...
		     char *ringfile, long *pringpos, FILE * out)
{
	...
	status = readkeypacket(fkey, FALSE, &ctb, (byte *) & kstamp,
			       NULL, NULL, NULL,
			       NULL, NULL, NULL, NULL, NULL, NULL, keyID, NULL, FALSE);
#ifdef MACTC5
	mac_poll_for_break();
#endif
	...
	if (is_ctb_type(ctb, CTB_SKE_TYPE)) {
		...
		if (kstamp > rstamp) {	/* Update, Maybe */
	       ...
	       LANG("Replacing signature from keyID %s on userid \"%s\"\n"),
			...
			fprintf(ebpout, LANG("on userid \"%s\"\n"),
			...
			if (!status) {
			    ...
			    fprintf(ebpout, LANG("Verification Failed\n"));
		     ...
		     char *ringfile, long *pringpos, FILE * out)
{
	...
	if (check_key_sig(fkey, keykeypos, keypktlen,
	    ...
	    fprintf(ebpout, LANG("Key revocation certificate from \"%s\".\n"),
	...
	if (ctb == CTB_USERID) {
	    ...
	    if (copying) {
		putc('\n', ebpout);
		fprintf(ebpout, LANG("New userid: \"%s\".\n"),
			LOCAL_CHARSET((char *) userid));
		fprintf(ebpout,
...
Acknowledge by pressing return: "), keyIDstring(keyID), ringfile);
	...
	if (copying && (is_key_ctb(ctb) || ctb == CTB_USERID ||
	    ...
	    /* signature certificate before userid must be compromise cert. */
			fprintf(ebpout, LANG("Key has been revoked.\n"));
		    }
...
/* unknown version or bad data: copy (don't remove packets from ringfile) */
	...
	fprintf(ebpout, LANG("No new keys or signatures in keyfile.\n"));
	rmtemp(scratchf);
	endkrent();
	return 0;
    }
	...
	if (verbose)
	    fprintf(ebpout, "addto_keyring: dokeycheck returned %d\n", status);
	goto err;
    }
    endkrent();
    ...
    fprintf(ebpout, LANG("\nKeyfile contains:\n"));
    if (newkeys)
	fprintf(ebpout, LANG("%4d new key(s)\n"), newkeys);
    if (newsigs)
	fprintf(ebpout, LANG("%4d new signatures(s)\n"), newsigs);
    if (newids)
	fprintf(ebpout, LANG("%4d new user ID(s)\n"), newids);
    if (newrvks)
	fprintf(ebpout, LANG("%4d new revocation(s)\n"), newrvks);
	...
	for (nkey = nkeys; nkey; nkey = nkey->next)
		...
		break;
    if (status && verbose)
	fprintf(ebpout, "addto_keyring: maint_update returned %d\n", status);
...
int gpk_open(char *keyfile)
{
    ...
    if (gpkf) {
	fprintf(ebpout, "gpk_open: already open\n");
	return -1;
    }
