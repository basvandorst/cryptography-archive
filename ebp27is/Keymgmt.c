/*      keymgmt.c  - Key management routines for PGP/EBP.
...
static void showkeyID(byteptr keyID, FILE *ebpout)
/*      Print key fragment, which is an abbreviation of the public key. */
{
    fprintf(ebpout, "%s", keyIDstring(keyID));
}				/* showkeyID */
...
static
short writekeyfile(char *fname, struct SaferCfbContext *cfb, word32 timestamp,
    byte * userid, unitptr n, unitptr e, unitptr d, unitptr p, unitptr q,
		   unitptr u, unitptr a, unitptr b)
/*      Write key components p, q, n, e, d, u, a and b to specified file,
	where e, d, and uis used by RSA and a and b is used by Rabin.
...
/*** Begin key certificate header fields ***/
	...
	cert_length = 1 + SIZEOF_TIMESTAMP + 2 + 1
	    + (countbytes(n) + 2)
	    + (countbytes(e) + 2)
	    + 1 + (cfb ? 8 : 0)	/* SAFER algorithm byte and IV */
	    +(countbytes(d) + 2)
	    + (countbytes(p) + 2) + (countbytes(q) + 2)
	    + (countbytes(u) + 2) + (countbytes(a) + 2)
		+ (countbytes(b) + 2) + 2;
	...
	(algostruct.Blockcipher == OLDALG) && (algostruct.Hashing == OLDALG)) 
		...
		alg = cfb ? algostruct.Blockcipher : 0;
    ...
    fclose(f);
    if (verbose)
	fprintf(ebpout, "%d-bit %s key written to file '%s'.\n",
		countbits(n),
 ...
 */
short readkeypacket(FILE * f, struct SaferCfbContext *cfb, byte * pctb,
		    byte * timestamp, char *userid,
	unitptr n, unitptr e, unitptr d, unitptr p, unitptr q, unitptr u,
		    unitptr a, unitptr b, byte * sigkeyID, byte * keyctrl, boolean oldPGP)
{
    byte ctb;
    word16 cert_length;
    int count;
...
/*** End certificate header fields ***/
	...
	if (read_mpi(u, f, FALSE, cfb) < 0)
	    return -4;		/* data corrupted, error return */
	if (!oldPGP){
		if (read_mpi(a, f, FALSE, cfb) < 0)
			return -4;		/* data corrupted, error return */
		if (read_mpi(b, f, FALSE, cfb) < 0)
			return -4;		/* data corrupted, error return */
	}
	...
	if (!oldPGP)
		cert_length -= (countbytes(a) + 2) + (countbytes(b) + 2);
		 ...
		 unitptr n, unitptr e)
{
  ...
  top:
    ...
    if (verbose) {
	fprintf(ebpout, "searching key ring file '%s' ", keyfile);
	if (keyID)
	    fprintf(ebpout, "for keyID %s\n", keyIDstring(keyID));
	else
	    fprintf(ebpout, "for userid \"%s\"\n", LOCAL_CHARSET(userid));
    }
	...
	status = readkeypacket(f, FALSE, &ctb, timestamp, (char *) userid,
			       n, e, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, FALSE);
...
has been revoked.  You cannot use this key.\n"),
  ...
  nogood:
	...
	if (flags & GPK_SECRET)
		fprintf(ebpout,LANG("Enter secret key filename: "));
	else
		fprintf(ebpout,LANG("Enter public key filename: "));
	fprintf(ebpout, "%s\n",keyfile);
	}
#else  
    if (flags & GPK_SECRET)
		  ...
		  byte * timestamp, long *sig_position, int *sig_len)
{
	...
	status = readkeypacket(f, FALSE, &ctb, NULL, NULL, NULL, NULL,
			       NULL, NULL, NULL, NULL, NULL, NULL, keyID0, NULL, FALSE);
...
int getsecretkey(int flags, char *keyfile, byte * keyID,
	   byte * timestamp, byte * hpass, boolean * hkey, byte * userid,
		 ...
		 unitptr u, unitptr a, unitptr b, boolean oldPGP)
{
    byte ctb;			/* returned by readkeypacket */
    FILE *f;
    char keyfilename[MAX_PATH];	/* for getpublickey */
    ...
    byte saferkey[HASHBYTES];
    int guesses;
	int nickelin;
	struct algorithms oldstruct;
    struct hashedpw *hpw, **hpwp;
    struct SaferCfbContext cfb;
	...
	hpw = keypasswds;
	while (hpw) {
	    saferCfbInit(&cfb, hpw->hash);
	    fseek(f, file_position, SEEK_SET);
	    status = readkeypacket(f, &cfb, &ctb, timestamp,
			  (char *) userid, n, e, d, p, q, u, a, b, NULL, NULL, oldPGP);
	    saferCfbDestroy(&cfb);
	    if (status != -5) {
		memcpy(saferkey, hpw->hash, sizeof(saferkey));
		goto done;
	    }
	    hpw = hpw->next;
	}
	...
	fprintf(ebpout, LANG("\nKey for user ID: %s\n"),
		LOCAL_CHARSET((char *) userid));
	fprintf(ebpout, LANG("%d-bit key, key ID %s, created %s\n"),
		countbits(n), key2IDstring(n), cdate((word32 *) timestamp));
	CToPascal((char *) userid);
    }
	nickelin = 0;
	status = 1;
	copystruct(&oldstruct, &algostruct);
    guesses = 0;
    for (;;) {
	if (++guesses > 3)
	    hidekey = 0;
	else
	while ((nickelin <= 32) && (status != 0))
	{
		hidekey = (GetHashedPassPhrase(saferkey, 1, nickelin, key1) > 0);
	...
	saferCfbInit(&cfb, saferkey);
	fseek(f, file_position, SEEK_SET);
	status = readkeypacket(f, &cfb, &ctb, timestamp,
			  (char *) userid, n, e, d, p, q, u, a, b, NULL, NULL, oldPGP);
	saferCfbDestroy(&cfb);
	if ((nickelin <= 32) && (status != 0))
	{
		nickelin += 1;
		TryAlg(nickelin, &algostruct);
	}
	}

	memset(key1, 0, sizeof(key1));
	copystruct(&algostruct, &oldstruct);
	nickelin = 0;
	...
	if (status >= 0) {
		...
		memcpy(hpw->hash, saferkey, sizeof(hpw->hash));
		hpw->next = keypasswds;
		keypasswds = hpw;
	    }
	}
	...
	*hkey = hidekey;
    ...
    if (hpass)
	memcpy(hpass, saferkey, sizeof(saferkey));
    burn(saferkey);
	...
	if (!quietmode) {
	    if (!hidekey)
		fprintf(ebpout,
LANG("\nAdvisory warning: This secret key is not protected by a \
passphrase.\n"));
	    else
		{
#ifndef Spore
		fprintf(ebpout, LANG("Pass phrase is OK.  "));
#else
		fprintf(ebpout, LANG("Pass phrase is OK l'ah.  "));
#endif
		}
	}
...
{
	haval_start(&state);
	haval_hash(&state, mdBuffer, mdIndex);
	haval_end(&state, hash);
}
else
{
	MD5Init(&mdContext);
 ...
 */
    fprintf(ebpout, "%*s", indent ? 29 : 1, LANG("Key fingerprint ="));
    for (i = 0; i < 8; i++)
	fprintf(ebpout, " %02X", hash[i]);
    putc(' ', ebpout);
    for (i = 8; i < 16; i++)
	fprintf(ebpout, " %02X", hash[i]);
    putc('\n', ebpout);
 ...
 */
     ...
     * Why couldn't we have a ebpout and PGPERR, and then we wouldn't
    ...
    fprintf(ebpout, "\n");
    kv_title(ebpout);
    status = kvformat_keypacket(f, ebpout, FALSE, mcguffin, ringfile,
	...
	endkrent();
    if (keycounter == 1)
	fprintf(ebpout, LANG("1 matching key found.\n"));
    else
	fprintf(ebpout, LANG("%d matching keys found.\n"), keycounter);
    close_more();
    ebpout = saveebpout;
...
/*
 * Change the secret key produced by PGP so that it can be used in EBP
 */
int dokeyupdate(char *mcguffin, char *newsecfile, char *oldsecfile)
{
    struct algorithms oldstruct;
	unit n[MAX_UNIT_PRECISION], e[MAX_UNIT_PRECISION], d[MAX_UNIT_PRECISION],
		 ...
		 a[MAX_UNIT_PRECISION], b[MAX_UNIT_PRECISION], t[MAX_UNIT_PRECISION];
    char *fname;
    char nonsens[256];
    byte keyID[KEYFRAGSIZE];
    byte userid[256], userid1[256];
    ...
    if (!newsecfile || strlen(newsecfile) == 0 || !mcguffin
	|| strlen(mcguffin) == 0)
	return -1;		/* Need newsecfile name, user name */

    if (!file_exists(newsecfile))
        force_extension(newsecfile, EBP_EXTENSION);

    /*
     * Although the name of a secret keyring may change in the future, it
     * is a safe bet that anything named "secring.ebp" will be a secret
     * key ring for the indefinite future.  
     */

    strcpy((char *) userid, mcguffin);
    fprintf(ebpout, LANG("\nUpgrading userid \"%s\" in PGP key ring: '%s'.\n"),
	    LOCAL_CHARSET((char *) userid), oldsecfile);

    if (!file_exists(newsecfile)) {
		fprintf(ebpout, LANG("\nCan't open secret key ring file '%s'\n"),
		newsecfile);
		fprintf(ebpout, LANG("\nEnter full filename for the secret key ring generated by EBP: \n"));
		getstring(newsecfile, 59, TRUE);	/* echo keyboard input */
	}
    if (!file_exists(newsecfile))
		force_extension(newsecfile, EBP_EXTENSION);
	if (!file_exists(newsecfile))
	...
	copystruct(&oldstruct, &algostruct);
	SetPGPalg(&algostruct, TRUE);
    status = getsecretkey(GPK_GIVEUP | GPK_SECRET, oldsecfile, NULL, (byte *) & timestamp,
			  saferkey, &hidekey, userid, n, e, d, p, q, u, a, b, TRUE);
	copystruct(&algostruct, &oldstruct);
 if (status < 0) {
	...
	/* Here comes the actual upgrading by making a and b */
	mp_inv(u, p, q);	  /* (p*u) mod q = 1, assuming p<q */
	mp_inv(t, q, p);  /* t = 1/q mod p */
	mp_mult(a, q, t);   /* a = q*(1/q mod p) */
	mp_mult(b, p, u);   /* b = p*(1/p mod q) */

    /* Show user her ID again to be clear */
    PascalToC((char *) userid);
    fprintf(ebpout, LANG("\n\nThe key for userid \"%s\" is upgraded to an EBP key \nand is to be put in the secret key ring: '%s'.\n"),LOCAL_CHARSET((char *) userid), newsecfile);
    CToPascal((char *) userid);


    fprintf(ebpout, LANG("\nSelect pass phrase for your EBP key. "));
	hidekey = (GetHashedPassPhrase((byte *) saferkey, 2, -1, nonsens) > 0);
    /* init CFB SAFER key */
    if (hidekey) {
	saferCfbInit(&cfb, saferkey);
	burn(saferkey);
    ...
    writekeyfile(fname, hidekey ? &cfb : 0, timestamp,
		 userid, n, e, d, p, q, u, a, b);

    if (hidekey)		/* done with SAFER to protect RSA secret key */
	saferCfbDestroy(&cfb);

    if (merge_key_to_ringfile(fname, newsecfile, 0L, 0, -1L) < 0) {
	fprintf(ebpout, LANG("\n\007Unable to update secret key ring.\n"));
	goto err;
    }
    fprintf(ebpout, LANG("\nSecret key ring updated...\n"));
#ifdef MACTC5
	EBPSetFinfo(secring,'SKey','MEBP');
#endif
	...
	strcpy(newsecfile, globalPubringName);
	if (!file_exists(newsecfile)) {
		fprintf(ebpout, LANG("\nCan't open public key ring file '%s'\n"),
		newsecfile);
		fprintf(ebpout, LANG("\nEnter full filename for the EBP public key ring:\n "));
		getstring(newsecfile, 59, TRUE);	/* echo keyboard input */
	}
	if (!file_exists(newsecfile))
		return -1;

    fname = tempfile(TMP_TMPDIR);

    writekeyfile(fname, NULL, timestamp,
		 userid, n, e, NULL, NULL, NULL, NULL, NULL, NULL);
  ...
  /* Automatically sign new userid? */
    ...
    (void) getpublickey(GPK_GIVEUP, newsecfile, &fpp, &pplength,
			keyID, (byte *) & timestamp, userid1, n, e);
      	PascalToC((char *) userid1);
      if (do_sign(newsecfile, fpp, pplength, userid, keyID, (char *)userid1, TRUE) < 0)
            return -1;
	}

    fprintf(ebpout, LANG("Public key ring updated.\n"));
#ifdef MACTC5
 ...
 */
    ...
    /* XXX Send this to stdout.  Do we always want to do this?
     * Why couldn't we have a pgpout [ebpout] and PGPERR [EBPERR], and then
	 *we wouldn't have this problem?  -warlord
     */
    saveebpout = ebpout;
    ebpout = stdout;
	...
	if (!quietmode) {
	    fprintf(ebpout, LANG("\nKey ring: '%s'"), ringfile);
	    if (mcguffin && strlen(mcguffin) > 0)
		fprintf(ebpout, LANG(", looking for user ID \"%s\"."),
			LOCAL_CHARSET(mcguffin));
	}
        fprintf(ebpout, "\n");
        kv_title(ebpout);
	...
	status = readkeypacket(f, FALSE, &ctb, timestamp, (char *) keyuserid,
			       n, e,
			       NULL, NULL, NULL, NULL, NULL, NULL, sigkeyID, NULL, FALSE);
	/* Note that readkeypacket has called set_precision */
	if (status == -1)
	    break;		/* eof reached */
	...
	if (ctb == CTB_USERID || print_userid) {
		...
		fprintf(ebpout, LANG("*** KEY REVOKED ***\n"));
		fprintf(ebpout, "          %s              ",
			blankkeyID);
	    }
	    firstuser = 0;
	...
	if (sigclass == KC_SIGNATURE_BYTE)
	    fprintf(ebpout, LANG("com"));
	else
	    fprintf(ebpout, LANG("sig"));
	if (sigstatus >= 0)
	    fputs("!      ", ebpout);	/* Good */
	else if (status < 0 || sigstatus == -2 || sigstatus == -3)
	    fputs("?      ", ebpout);	/* Uncheckable */
	else if (sigstatus != -20)
	    fputs("%      ", ebpout);	/* Malformed */
	else
	    fputs("*      ", ebpout);	/* BAD! */

	showkeyID(sigkeyID, ebpout);
	...
	if (sigstatus >= 0 || sigstatus == -3 ||
	    ...
	    fprintf(ebpout, " %s ", cdate(&sigtstamp));
	    if (sigclass != KC_SIGNATURE_BYTE)
		putc(' ', ebpout);
	    fputs(LOCAL_CHARSET((char *) siguserid), ebpout);
	    putc('\n', ebpout);
	...
	if (sigstatus == -2)
	    fprintf(ebpout, LANG("(Unknown signator, can't be checked)"));
	else if (sigstatus == -3)
	    fprintf(ebpout, LANG("(Key too long, can't be checked)"));
	else if (sigstatus == -5)
	    fprintf(ebpout, LANG("(Malformed or obsolete signature format)"));
	else if (sigstatus == -6)
	    fprintf(ebpout, LANG("(Unknown public-key algorithm)"));
	else if (sigstatus == -7)
	    fprintf(ebpout, LANG("(Unknown hash algorithm)"));
	else if (sigstatus == -8)
	    fprintf(ebpout, LANG("(Unknown signature packet version)"));
	else if (sigstatus == -9)
	    fprintf(ebpout, LANG("(Malformed signature)"));
	else if (sigstatus == -10)
	    fprintf(ebpout, LANG("(Corrupted signature packet)"));
	else if (sigstatus == -20)
	    fprintf(ebpout, LANG("\007**** BAD SIGNATURE! ****"));
	else
	    fprintf(ebpout, "(Unexpected signature error %d)", sigstatus);
	putc('\n', ebpout);
	...
	fclose(fixedf);
	if (!batchmode)
	    fprintf(ebpout, LANG("Remove bad signatures (Y/n)? "));
...
int remove_sigs(char *mcguffin, char *ringfile)
{
	...
	if (is_ctb_type(ctb, CTB_SKE_TYPE)) {
	    fprintf(ebpout, LANG("sig"));
	    fprintf(ebpout, "%c     ", status < 0 ? '?' : ' ');
	    if (status < 0)
		memset(sigkeyID, 0, KEYFRAGSIZE);
	    showkeyID(sigkeyID, ebpout);
	    fprintf(ebpout, "               ");	/* Indent signator userid */
	    ...
	    fprintf(ebpout, LANG("Remove this signature (y/N)? "));
...
Do you want to remove the whole key (y/N)? "));
		...
		for (;;) {
		    ...
		    if (ctb == CTB_USERID) {
			fprintf(ebpout, LANG("Remove \"%s\" (y/N)? "), userid);
			if (getyesno('n'))
			    break;
		    }
		}
...
int extract_from_keyring(char *mcguffin, char *keyfile, char *ringfile,
			 boolean transflag, boolean oldPGP)
{
    FILE *f;
    FILE *g;
    long fp;
    ...
    int status;
    unit n[MAX_UNIT_PRECISION], e[MAX_UNIT_PRECISION];
	unit n1[MAX_UNIT_PRECISION], e1[MAX_UNIT_PRECISION];
    byte keyID[KEYFRAGSIZE];
    byte userid[256], userid1[256];		/* key certificate userid */
	...
	fprintf(ebpout, "\n");
	fprintf(ebpout, LANG("Extract the above key into which file?"));
	fprintf(ebpout, " ");
	if (batchmode)
	    return -1;
#ifdef MACTC5
	...
	if (oldPGP)
		...
		default_extension(fname, EBP_EXTENSION);
	...
	if (whole_ring || transflag || status < -1) {
	    if (!is_tempfile(fname) && !force_flag && !oldPGP)
...
if (append)
	...
	}else{	/* Now write public key data to file */
	set_precision(MAX_UNIT_PRECISION);	/* safest opening assumption */
    tempf2 = tempfile(TMP_TMPDIR | TMP_WIPE);
 	SetPGPalg(&algostruct, TRUE);
	writekeyfile(tempf2, NULL, timestamp,
		 userid, n, e, NULL, NULL, NULL, NULL, NULL, NULL);

    if (file_exists(fname) && append){ 
	if (merge_key_to_ringfile(tempf2, fname, 0L, 0, -1L) < 0) {
		fprintf(ebpout, LANG("\n\007Unable to update public key ring.\n"));
		rmtemp(tempf2);
		return -1;
    }
		...
		rmtemp(fname);
	    savetemp(tempf2, fname);
	}
  /* Sign the userid? */
	fprintf(ebpout,
	     LANG("\nIs the extracted key your own public key (y/N)? "));
	if (getyesno('n')) {
		extract_keyID(keyID, n);
     	PascalToC((char *) userid);
        strcpy((char *)userid1, (char *)userid);
     /* Get position of key in key file */
    (void) getpublickey(GPK_GIVEUP, fname, &fp, &packetlength,
 		keyID, (byte *) & timestamp, userid, n, e);
        strcpy((char *)userid, (char *)userid1);
		if (do_sign(fname, fp, packetlength, userid, keyID, (char *)userid1, TRUE) < 0)
            return -1;
	}
 ...
 */
{
    unit n[MAX_UNIT_PRECISION], e[MAX_UNIT_PRECISION], d[MAX_UNIT_PRECISION],
         p[MAX_UNIT_PRECISION], q[MAX_UNIT_PRECISION], u[MAX_UNIT_PRECISION], 
		 a[MAX_UNIT_PRECISION], b[MAX_UNIT_PRECISION];
    ...
    if (!file_exists(ringfile))
        force_extension(ringfile, EBP_EXTENSION);
    ...
    if (!strcmp(file_tail(ringfile), "secring.ebp") ||
...
Defaulting to public keyring."));
    ...
    if (!file_exists(secring) && strcmp(file_tail(secring), "secring.ebp")) {
		strcpy(file_tail(secring), "secring.ebp");
	}
#else
    /*
    ...
    status = getsecretkey(GPK_GIVEUP, secring, keyID, (byte *) & timestamp,
			  saferkey, &hidekey, userid1, n, e, d, p, q, u, a, b, FALSE);
...
LANG("\nNo secret key available.  Editing public key trust parameter.\n"));
	...
	init_trust_lst();
	fprintf(ebpout, LANG("Current trust for this key's owner is: %s\n"),
...
LANG("\nMake this user ID the primary user ID for this key (y/N)? "));
    ...
    if (!changed) {
	fprintf(ebpout, LANG("(No changes will be made.)\n"));
	if (hidekey)
	    burn(saferkey);
	goto done;
    }
	...
	fprintf(ebpout, LANG("(No need to update public key ring)\n"));
    }
...
int disable_key(char *keyguffin, char *keyfile)
{
    ...
    if (getsecretkey(GPK_GIVEUP, NULL, keyID, NULL, NULL, NULL,
		     userid, n, e, NULL, NULL, NULL, NULL, NULL, NULL, FALSE) >= 0) {
...
#endif
        fputs("\n", ebpout);
	fputs(LANG("Pick your key size:\n\
    1)   512 bits - Low commercial grade, fast but less secure\n\
    2)   768 bits - High commercial grade, medium speed, good security\n\
    3)  1024 bits - \"Military\" grade, slow, high security\n\
    4)  2048 bits - Even Better grade, very slow, highest security\n\
Choose 1, 2, 3 or 4, or enter desired number of bits: "), ebpout);
#ifdef MACTC5
		fprintf(ebpout, "%s\n",numstr);
#else
	...
	if (keybits == 4)
		keybits = 2048;     /* Even Better grade */

	/* The 2048 bit "Even Better" keylength has been added due to      */
	/* the public demand, but for the time being it seems to be a      */
	/* waste of time, unless you have a really fast computer or a      */
	/* lot of time on your hand. However, it's only the keygeneration  */
	/* that takes an enormous amount of time, the encryption of the    */
	/* session key isn't a very lengthy process.					   */
    ...
    fprintf(ebpout, "\n");
    fprintf(ebpout,
	    LANG("Generating a key with a %d-bit modulus.\n"), keybits);
...
For example: Peyton J. Westlake <peyton.westlake@takashimaya.sg>\n\
Enter a user ID for your public key: \n"), ebpout);
	...
	fprintf(ebpout,
	    LANG("Generating key-pair with UserID \"%s\".\n"), userid);
    }
    CONVERT_TO_CANONICAL_CHARSET((char *) userid);
    CToPascal((char *) userid);	/* convert to length-prefixed string */
...
words, spaces, punctuation, or any other printable characters.\n"), ebpout);
    hidekey = (GetHashedPassPhrase(saferkey, 2, -1, nonsens) > 0);
    /* init CFB SAFER key */
#ifdef MACTC5
    if (Abort)
    	exitEBP(-2);
 ...
 */
    cryptrandflag = (cryptRandOpen((struct SaferCfbContext *)0) < 0);
    if (cryptrandflag)
    ...
    if (keygen(n, e, d, p, q, u, a, b, keybits, ebits) < 0) 
		if (keygen(n, e, d, p, q, u, a, b, keybits, ebits) < 0) /* Try again */
			if (keygen(n, e, d, p, q, u, a, b, keybits, ebits) < 0) /* and again */
	{ /* Give up, it's a lousy computer or, perhaps more probable,
	     lousy programmers that design the SPRP-code in slowtest2 */ 
#ifdef MACTC5
		if (Abort)
			fprintf(ebpout, LANG("Key generation stopped at user request.\n"));
		else
			fprintf(ebpout, LANG("\n\007Keygen failed!\n"));
    ...
    if (verbose) {
	fprintf(ebpout, LANG("Key ID %s\n"), key2IDstring(n));

	mp_display(" modulus n = ", n);
	mp_display("exponent e = ", e);

	fputs(LANG("Display secret components (y/N)?"), ebpout);
	if (getyesno('n')) {
    ...
    fputc('\007', ebpout); /* sound the bell when done with lengthy process */
    fflush(ebpout);

    /* First, write out the secret key... */
    fname = tempfile(TMP_TMPDIR | TMP_WIPE);
    writekeyfile(fname, hidekey ? &cfb : 0, tstamp, userid, n, e, d, p, q, u, a, b);
    ...
    if (keygen_OK) {
        fname = tempfile(TMP_TMPDIR | TMP_WIPE);
        writekeyfile(fname, NULL, tstamp, userid, n, e, NULL, NULL, NULL, NULL, NULL, NULL);
    ...
    if (cryptrandflag) {
	trueRandConsume(192);
	cryptRandInit((struct SaferCfbContext *)0);
	/* It will get saved by exitEBP */
    }
    return 0;			/* normal return */
}				/* dokeygen */
...
int kvformat_keypacket(FILE *f, FILE *ebpout, boolean one_key,
