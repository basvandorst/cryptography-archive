/*	crypto.c  - Cryptographic routines for PGP/EBP.
  ...
  Now over to the old PGP information:
	...
	Modified in January 1997 by Niklas Jarl and Jonas Loefgren
	to create Even Better Privacy. 
...
#define ENCRYPT_IT FALSE	/* to pass to safer_file */
#define DECRYPT_IT TRUE		/* to pass to safer_file */

#define	USE_LITERAL2


/* This variable stores the md5 hash of the current file, if it is
   available.  It is used in make_random_saferkey. */
static unsigned char digest[HASHBYTES];

/* This flag is set if the buffer above has been filled. */
static char already_have_md = 0;
...
/* 
	...
	config.ebp to optionally correct this problem further.  For example,
	set TZFIX=-1 in config.ebp if you live in Colorado and the TZ
...
#define CALENDAR_KLUDGE ((unsigned long)86400L * (((unsigned long)365L \
	...
	t += timeshift; /* timeshift derived from TZFIX in config.ebp */
...
version_byte_error(int val)
{
	if (val != VERSION_BYTE_OLD && val != VERSION_BYTE_NEW) {
		fprintf (ebpout,
LANG("\n\007Unsupported packet format - you need a newer version of EBP/PGP \
for this file.\n"));
		return 1;
	}
	return 0;
...
#define RAND_PREFIX_LENGTH 8	/* Length of IV for SAFER encryption */

/*
 * Make a random SAFER key.  Returns its length (the constant 16).
 * It also generates a random IV, which is placed in the key array
 * after the key proper, but is not counted in the length.
 * Reads SAFER random key and random number seed from file, cranks the
 * seed through the SAFER strong pseudorandom number generator,
 ...
 */
static int make_random_saferkey(byte key[SAFERKEYSIZE+RAND_PREFIX_LENGTH],
			       int skip)
{
	int count;
	...
	saferCfbInit(&cfb, digest);
	burn(digest);

	if (cryptRandOpen(&cfb) < 0) {
		fprintf(ebpout,LANG("Preparing random session key..."));

		 /* get some random key bits */
		trueRandAccum((SAFERKEYSIZE+RAND_PREFIX_LENGTH)*8);

		cryptRandInit(&cfb);
	}

	/*
	 * Generate a good random SAFER key and initial vector.  If we have
	 * no random bytes, the trueRandByte() part will be useless
	 */
	count = SAFERKEYSIZE+RAND_PREFIX_LENGTH;
	for (count = skip; count < SAFERKEYSIZE+RAND_PREFIX_LENGTH; count++)
	 ...
	 */
	saferCfbInit(&cfb, key);
	memcpy(buf, key, 8);
	buf[8] = buf[6];
	buf[9] = buf[7];
	saferCfbEncrypt(&cfb, buf, buf, 10);
	saferCfbSync(&cfb);
...
/*
 * Use SAFER in cipher feedback (CFB) mode to encrypt or decrypt a file. 
 ...
 * check bytes detect if correct SAFER key was used to decrypt ciphertext.
 */
static
int safer_file(byte *saferkey, boolean decryp, FILE *f, FILE *g, word32 lenfile)
{
	int count, status = 0;
...
#define RAND_PREFIX_LENGTH 8
	...
	if (!decryp) {	/* encrypt-- insert key check bytes */
		...
		memcpy(textbuf, saferkey+SAFERKEYSIZE, RAND_PREFIX_LENGTH);
    /* key check bytes are simply duplicates of final 2 random bytes */
		...
		saferCfbEncrypt(&cfb, textbuf, textbuf, RAND_PREFIX_LENGTH+2);
		...
		if (count==(RAND_PREFIX_LENGTH+2)) {
			saferCfbDecrypt(&cfb, textbuf, textbuf,
	...
	while (count && status == 0) {
		...
		lenfile -= count;
		if (decryp)
			saferCfbDecrypt(&cfb, textbuf, textbuf, count);
		else
			saferCfbEncrypt(&cfb, textbuf, textbuf, count);
	...
	saferCfbDestroy(&cfb);	/* Clean up data structures */
             ...
             struct SaferCfbContext *cfb)
{
	...
	if (cfb) {	/* decrypt the field */
		saferCfbSync(cfb);
		saferCfbDecrypt(cfb, buf+2, buf+2, bytecount);
	}
 ...
 */
void write_mpi(unitptr n, FILE *f, struct SaferCfbContext *cfb)
{
	byte buf[MAX_BYTE_PRECISION+2];
	short bytecount;
	...
	if (cfb) { /* encrypt the field, skipping over the bitcount */
		saferCfbSync(cfb);
		saferCfbEncrypt(cfb, buf+2, buf+2, bytecount);
	}
...
*/
static
int make_md5_signature_certificate(byte *certificate, struct MD5Context *MD,
   byte klass, unitptr e, unitptr d, unitptr p, unitptr q, unitptr u,
			       unitptr a, unitptr b, unitptr n)
{
	...
	/*	Note that RSA key must be at least big enough to encipher a
		...
		if (blocksize < 31) {
			fprintf(ebpout,
   "\n\007Error: Key length must be at least 256 bits.\n");
			return -1;
		}
   ...
   LANG("\n\007Error: System clock/calendar is set wrong.\n"));
	...
	MD_addbuffer (MD, &klass, 1, 0);
	MD_addbuffer (MD, timestamp, 4, digest);
   ...
   for random number generation later.   Also make a note of that fact. */
	...
	if (!quietmode) {
		fprintf(ebpout,LANG("Please wait...")); /* Signing will take
							     a while. */
		fflush(ebpout);
	}
...
if (algostruct.PKsign == NEWALG) 
	i = rabin_private_encrypt((unitptr)outbuf, digest, (algostruct.MDlength/8),
	                        p, q, n, a, b);
else
	i = rsa_private_encrypt((unitptr)outbuf, digest, (algostruct.MDlength/8),
	                        e, d, p, q, u, n);
   ...
   "\n\007Error: Key length must be at least 256 bits.\n");
			...
			fputs(
"\a\nError: Key is too large. Keys may be no longer than 2048 bits\
,\ndue to limitations imposed by the software.\n", ebpout);
		} else {
	...
	/*	outbuf now contains a message digest in external byteorder 
		...
		(Note that the first two bytes of digest are used below as
   ...
   /* SKE packet length does not include itself or CTB prefix: */
	...
	certificate[certificate_length++] =  klass & 0xff;
...
{
	certificate[certificate_length++] = algostruct.PKsign;
	i = GetHLandPnbr(&algostruct);
	certificate[certificate_length++] = i;
}

	/* Now append first two bytes of message digest */
	certificate[certificate_length++] = digest[0];
	certificate[certificate_length++] = digest[1];;
	...
	if (!quietmode)
		fputc('.',ebpout);	/* Signal RSA signature completion. */
...
}	/* make_md5_signature_certificate */

static
int make_haval_signature_certificate(byte *certificate, byte digest[HASHBYTES],
   byte klass, unitptr e, unitptr d, unitptr p, unitptr q, unitptr u,
			       unitptr a, unitptr b, unitptr n)
{
	byte inbuf[MAX_BYTE_PRECISION], outbuf[MAX_BYTE_PRECISION+2];
...
}	/* make_haval_signature_certificate */
		...
		char lit_mode, char *literalfile)
{
	...
	{	/* temporary scope for some buffers */
		...
		unit u[MAX_UNIT_PRECISION], a[MAX_UNIT_PRECISION], b[MAX_UNIT_PRECISION];
...
else
{
		...
		if (getsecretkey(0, NULL, NULL, timestamp, NULL, NULL,
						 userid, n, e, d, p, q, u, a, b, FALSE) < 0)
...
if (algostruct.Hashing == NEWALG)
        certificate_length = make_haval_signature_certificate(certificate,
								digest,
			signature_class, e, d, p, q, u, a, b, n);
else
		certificate_length = make_md5_signature_certificate(certificate,
								&MD,
			signature_class, e, d, p, q, u, a, b, n);
	...
	if (!separate_signature) {
		if (!nested) {
			...
			if (lit_mode == MODE_LOCAL)
			    ...
			    fprintf(ebpout, "signfile: invalid mode\n");
#endif
			else {
#ifdef USE_LITERAL2
			    write_ctb_len (g, CTB_LITERAL2_TYPE,
...
int compromise(byte *keyID, char *keyfile)
{
   ...
   LANG("\n\007Can't open key ring file '%s'\n"),keyfile);
	...
	if (is_ctb_type(ctb, CTB_SKE_TYPE)) {
		fprintf(ebpout, LANG("This key has already been revoked.\n"));
		fclose(f);
		return -1;
	}
...
if (algostruct.Hashing == NEWALG)
{
	if (havcompfil(f, digest, pktlen) < 0) {
		fclose(f);
		return -1;
	}
}
...
if (algostruct.Hashing == NEWALG)
    certificate_length = make_haval_signature_certificate(certificate, digest,
		KC_SIGNATURE_BYTE, e, d, p, q, u, a, b, n);
else
	certificate_length = make_md5_signature_certificate(certificate, &MD,
		KC_SIGNATURE_BYTE, e, d, p, q, u, a, b, n);
   ...
   LANG("\n\007Can't open key ring file '%s'\n"),keyfile);
	...
	for (;;) {
		status = readkeypacket(f,FALSE,&ctb,NULL,NULL,NULL,NULL,
					NULL,NULL,NULL,NULL,NULL,NULL,keyID2,NULL,FALSE);
...
{
		haval_start(&state);
        if (havsignfil(g, &state, pktlen) < 0) {
			fclose(f);
			fclose(g);
...
if (algostruct.Hashing == NEWALG)
{
		haval_hash(&state, userid+1, (int) (unsigned char)userid[0]);
...
if (algostruct.Hashing == NEWALG)
        certificate_length =
		  make_haval_signature_certificate(certificate, digest,
					     K0_SIGNATURE_BYTE, e, d, p, q,
					     u, a, b, n);
else
		certificate_length =
		  make_md5_signature_certificate(certificate, &MD,
					     K0_SIGNATURE_BYTE, e, d, p, q,
					     u, a, b, n);
...
int signkey(char *keyguffin, char *sigguffin, char *keyfile)
{
	...
	status = getsecretkey(0, NULL, NULL, timestamp, NULL, NULL,
	                      userid, n, e, NULL, NULL, NULL, NULL, NULL, NULL, FALSE);
	if (status < 0)
			...
			char *preserved_name)
{
	...
	*certificate++ = ctb;	/* copy ctb into certificate */
	...
	klass = *certificate++;
	if (klass != SM_SIGNATURE_BYTE  &&  klass != SB_SIGNATURE_BYTE) {
		(void) version_error(klass, SM_SIGNATURE_BYTE);
		goto err1;
	}
	mdlen--;
	...
	if ((!version_error(algorithm, RSA_ALGORITHM_BYTE))||
		(version_error(algorithm, NEWALG)))
	...
	else
	{
		...
		SetLengthandPasses(&algostruct, algorithm);
	}
	...
	if (((certificate-certbuf) != cert_length+3) && 
		((certificate-certbuf) != cert_length+5))
...
if (algostruct.PKsign == NEWALG)  
	count = rabin_public_decrypt(outbuf, (unitptr)inbuf, e, n);
else
	count = rsa_public_decrypt(outbuf, (unitptr)inbuf, e, n);

	if (!quietmode)
		fputc('.',ebpout);	/* Signal RSA completion. */

	/* outbuf should contain message digest packet */
	/*==================================================================*/
	/* Look at nested stuff within RSA block... */

	if (count == -7 || (count > 0 && count != (algostruct.MDlength/8)))
	{
		fputs(LANG("\007\nUnrecognized message digest algorithm.\n\
This may require a newer version of PGP or EBP.\n\
...
LANG("\a\nSigning key is too large.  Can't check signature integrity.\n"), ebpout);
	...
	if (count < 0) {	/* Catch-all */
		fprintf(ebpout,
LANG("\n\007Error: Decrypted block is corrupted.\n\
This may be caused either by corrupted data or by using the wrong key.\n\
"));
		goto outsig;	/* Output data anyway */
	}

	/* Distinguish PKCS-compatible from pre-3.3 which has an extra byte */
	outbufoffset = (count==(algostruct.MDlength/8)) ? 0 : 1;
...
/* Assume MSB external byte ordering */
	if (!equal_buffers(digest, outbuf+outbufoffset, (algostruct.MDlength/8))) {
		...
		if (klass == SM_SIGNATURE_BYTE && separate_signature
		  ...
		  byte *xtimestamp, byte *sigclass)
{
	...
	*sigclass = klass = *certificate++;
	if (klass != K0_SIGNATURE_BYTE  &&  klass != K1_SIGNATURE_BYTE &&
		klass != K2_SIGNATURE_BYTE  &&  klass != K3_SIGNATURE_BYTE &&
		klass != KC_SIGNATURE_BYTE)
	{
		(void)version_error(klass, K0_SIGNATURE_BYTE);
		return -8;
	}
...
if (algostruct.PKsign == NEWALG) 
	...
	if (count != (algostruct.MDlength/8))
...
{
	haval_start(&state);
	havsignfil(fkey, &state, keypktlen);
}
else
	MDfile0_len(&MD,fkey,keypktlen);

	/* Add data from user id */
	if (klass != KC_SIGNATURE_BYTE)
if (algostruct.Hashing == NEWALG)
		haval_hash(&state, (unsigned char *) keyuserid+1,
...
if (algostruct.Hashing == NEWALG)
	...
	/* now compare computed MD with claimed MD */
/* Assume MSB external byte ordering */
	if (!equal_buffers(digest, outbuf, (algostruct.MDlength/8)))
...
/*======================================================================*/
static int squish_and_safer_file(byte *saferkey, FILE *f, FILE *g, 
...
"\nCompressing [%s] ", plainfile);
			...
			if (verbose) fprintf(ebpout, LANG("compressed.  ") );
			else if (!quietmode)
				fputc('.',ebpout);	/* show progress */
			rewind( t );
	  	}

	/*	Now write out file thru SAFER cipher... */
	...
	safer_file( saferkey, ENCRYPT_IT, t, g, fsize(t) );
...
int squish_file(char *infile, char *outfile)
{
	...
	if (verbose)
		fprintf(ebpout,"squish_file: infile = '%s', outfile = '%s'\n",
		infile,outfile);
...
LANG("\n\007Can't create compressed file '%s'\n"), outfile );
	...
	if (verbose) fprintf(ebpout, LANG("Compressing file..."));
...
int safer_encryptfile(char *infile, char *outfile,
	boolean attempt_compression)
{
	char nonsens[256];
	FILE *f;	/* input file */
	...
	if (verbose)
		fprintf(ebpout,
"safer_encryptfile: infile = '%s', outfile = '%s'\n",
		infile,outfile);
...
LANG("\n\007Can't create ciphertext file '%s'\n"), outfile );
	...
	/* Get SAFER password, hashed to a key */
	if (passwds) {
		memcpy(saferkey, passwds->hash, sizeof(saferkey));
...
LANG("\nYou need a pass phrase to encrypt the file. "));
		if (batchmode
		    || GetHashedPassPhrase((char *)saferkey, NOECHO2,
			-1, nonsens) <= 0)
		{
			fclose(f);
			fclose(g);
	 ...
	 */
	make_random_saferkey(saferkey, (algostruct.MDlength/8));
...
LANG("Just a moment..."));  /* this may take a while */
	...
	/* Now compress the plaintext and encrypt it with SAFER... */
	squish_and_safer_file( saferkey, f, g, attempt_compression );

	burn(saferkey);	/* burn sensitive data on stack */
	...
	boolean attempt_compression)
{
	...
	FILE *f;
	FILE *g;
	byte keybuf[MAX_BYTE_PRECISION]; /* This keeps our SAFER to encrypt */
	byte saferkey[24]; /* must be big enough for make_random_saferkey */
	word32 chksum;
	char keyfile[MAX_PATH];
	int keys_used = 0;
	...
	if (verbose)
		fprintf(ebpout,"encryptfile: infile = %s, outfile = %s\n",
		infile,outfile);
...
else
{
	...
	ckp_length = make_random_saferkey(saferkey, 0);
	/* Returns a 24 byte random SAFER key */
...
(algostruct.Blockcipher == OLDALG) && (algostruct.Hashing == OLDALG)) 
		...
		keybuf[0] = algostruct.Blockcipher;

	for (i=0; i<ckp_length; ++i)
		keybuf[i+1] = saferkey[i];
	...
	/* Ok, we now have our SAFER key which we are going to use
	...
	/* Finished with RSA block containing SAFER key. */

	/* Now compress the plaintext and encrypt it with SAFER... */
	squish_and_safer_file( saferkey, f, g, attempt_compression );

	burn(keybuf);	/* burn the Safer Key Packet */
	burn(saferkey);	/* burn sensitive data on stack */
...
This user will not be able to decrypt this message.\n"), 
		...
		if (warn_signatures(keyfile, fp, (char *)userid, 
				    FALSE) < 0) {
			fprintf(ebpout, LANG("Skipping userid %s\n"), mcguffin);
			continue;
		}
		...
		*/
		...
		if (blocksize < 31) {
			fprintf(ebpout,
"\n\007Error: Key length must be at least 256 bits.\n");
			fprintf(ebpout, "Skipping userid %s\n", mcguffin);
			continue;
		}
		
#ifdef MR_DEBUG
		/* XXX This is dangerous... This will print out the
		 * SAFER Key, which is a breach of security!
		 */
		fprintf(ebpout, "Safer Key: ");
		for (i = 0; i < ckp_length; i++)
			fprintf(ebpout, "%02X ", keybuf[i]);
		fprintf(ebpout, "\n");
#endif
if (algostruct.PKencrypt == NEWALG)
		i = rabin_public_encrypt((unitptr)outbuf, keybuf,
				       ckp_length, e, n);
else
		...
		char *lit_mode)
{
	...
	*lit_mode = MODE_BINARY;
	if (verbose)
		fprintf(ebpout,"strip_literal: infile = %s, outfile = %s\n",
		infile,outfile);
...
"\n\007'%s' is not a literal plaintext file.\n",infile);
	...
	*lit_mode = '\0';
	...
	if (verbose)
		fprintf(ebpout, LANG("File type: '%c'\n"), EXT_C(*lit_mode));
	/* Read literal file name, use it if possible */
	litfile[0] = 0;
	fread (litfile,1,1,f); 
...
int decryptfile(char *infile, char *outfile)
{
	...
	if (verbose)
		fprintf(ebpout,"decryptfile: infile = %s, outfile = %s\n",
		infile,outfile);
...
LANG("\n\007'%s' is not a cipher file.\n"),infile);
		...
		if (thiskey) {
			if (!quietmode) {
				fprintf(ebpout,LANG("Just a moment...")); 
				/* Decryption will take a while. */
				fflush(ebpout);
			}
if (algostruct.PKencrypt == NEWALG)
			count = rabin_private_decrypt(outbuf, (unitptr)inbuf,
			                            p, q, n, a, b);
else
...
"));
				        ...
				        fprintf(ebpout,
LANG("\n\007Error: Block is possibly malformed.  Old format, maybe?\n"));
				} else {
					fprintf(ebpout,
...
"));
	...
	/* outbuf should contain random SAFER key packet */
	/*==================================================================*/
...
/* Assume MSB external byte ordering */
	status = safer_file( outbuf+1, DECRYPT_IT, f, g, flen );
	if (status < 0) {
		fprintf(ebpout,
LANG("\n\007Error: Decrypted plaintext is corrupted.\n"));
	...
	if (status < 0)	/* if safer_file failed, then error return */
		return status;
...
int safer_decryptfile(char *infile, char *outfile)
{
	byte ctb;	/* Cipher Type Byte */
	char nonsens[256];
	FILE *f;
	...
	if (verbose)
		fprintf(ebpout,"safer_decryptfile: infile = %s, outfile = %s\n",
		infile,outfile);
...
LANG("\nYou need a pass phrase to decrypt this file. "));
			if (batchmode
			    || GetHashedPassPhrase((char *)saferkey, NOECHO1, 
				-1, nonsens) <= 0)
			{
				fclose(f);
				fclose(g);
		...
		status = safer_file( saferkey, DECRYPT_IT, f, g, flen );
		if (status == 0) {
			if (hpw) {
				/* "Use up" password. */
				*hpwp = hpw->next;
...
LANG("\n\007Error:  Bad pass phrase.\n"));
	...
	if (status < 0) {	/* if safer_file failed, then complain */
		remove(outfile);	/* throw away our mistake */
		return status;		/* error return */
	}
	if (!quietmode)
		fprintf(ebpout,LANG("Pass phrase appears good. "));
	return 1;		/* always indicate output file has
				   nested stuff in it. */
...
int decompress_file(char *infile, char *outfile)
{
	...
	if (verbose) fprintf(ebpout, LANG("Decompressing plaintext...") );
