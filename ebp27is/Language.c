 ...
 * created if the language set in config.ebp doesn't match the one
...
static void init_lang()
{
    ...
    if (langf == NULL) {
#ifdef EBP_SYSTEM_DIR
	strcpy(subtitles_file, EBP_SYSTEM_DIR);
	...
	strcpy(indexfile, EBP_SYSTEM_DIR);
...
static int make_indexfile(char *indexfile)
{
    ...
    if (verbose)		/* must be set in config.ebp */
	fprintf(stderr,
