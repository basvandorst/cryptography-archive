...
#include "ebp.h"	/* For globalRandseedName */
#include "randpool.h"
#ifdef MACTC5
#include "Macutil2.h"
#include "Macutil3.h"
 ...
 * of how the whole thing works is in order, as people are always suspiscious
 * of the random number generator.  (After the xorbytes bug in 2.6, perhaps
 * you should be.)  There are two random number generators in PGP/EBP.  One
 * is the "cryptRand" family, which is based on X9.17, but uses SAFER instead
 * of IDEA or 2-key EDE triple-DES.  This is the generator with a lot of peer
 * review.
 * The implementation is in safer.c.
 ...
 * and the cryptRand for generating SAFER session keys.  But things have
 ...
 * to bother), and use that as an SAFER key (with a fixed IV of zero)
 * to encrypt the randseed.bin data.
 * After generating an SAFER key and IV, some more random bytes are generated
 ...
 * - Every time you run PGP/EBP, especially when responding to one of PGP/EBP's
 *   prompts, PGP/EBP samples the keystrokes for use as random numbers.
 *   It is a shame to throw this entropy (randomness) away just because
 *   there is no need for it in the current invocation of PGP/EBP.
 ...
 * generating a session key (see make_random_saferkey in crypto.c),
 * but it is also opened (harmless if alreasy open) and saved
 * (harmless if already saved) without a washing key in the exitEBP routine,
...
static struct SaferRandContext randContext;
 ...
 */
int
cryptRandOpen(struct SaferCfbContext *cfb)
{
	byte buf[256];
	int len;
	FILE *f;
	...
	/* First get the bare minimum 24 bytes we need for the SAFER RNG */
	len = fread((char *)buf, 1, 24, f);
	if (cfb)
		saferCfbEncrypt(cfb, buf, buf, 24);
	saferRandInit(&randContext, buf, buf+16);
	randSeedOpen = TRUE;
	if (len != 24) { /* Error */
		fclose(f);
		return -1;
	...
	for (;;) {
		...
		if (len <= 0)
			break;
		if (cfb)
			saferCfbEncrypt(cfb, buf, buf, len);
...
void
cryptRandInit(struct SaferCfbContext *cfb)
{
	byte buf[24];
	int i;
	...
	for (i = 0; i < sizeof(buf); i++)
		buf[i] = trueRandByte();
	if (cfb)
		saferCfbEncrypt(cfb, buf, buf, sizeof(buf));
...
{
	if (!randSeedOpen)
		cryptRandOpen((struct SaferCfbContext *)0);
	return saferRandByte(&randContext);
}
 ...
 */
int
cryptRandWriteFile(char const *name, struct SaferCfbContext *cfb, unsigned bytes)
{
	byte buf[256];
	FILE *f;
	int i, len;
	...
	while (bytes) {
		...
		for (i = 0; i < len; i++)
			buf[i] = saferRandByte(&randContext);
		if (cfb)
			saferCfbEncrypt(cfb, buf, buf, len);
...
#ifdef MACTC5
	EBPSetFinfo((char *)name,'RSed','MEBP');
#endif
 ...
 *
 * This function is always called by exitEBP, without a washing encryption,
...
void
cryptRandSave(struct SaferCfbContext *cfb)
{
#ifndef MACTC5
