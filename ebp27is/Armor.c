 ...
 * delimiter string used by EBP
 * (e.g. "-----BEGIN EBP MESSAGE, PART %02d/%02d-----\n")
 */
...
extern boolean verbose;		/* Undocumented command mode in EBP.C */
...
"armor_file: infile = %s, outfile = %s, filename = %s, clearname = %s\n",
	...
	if ((algostruct.PKencrypt == OLDALG) && (algostruct.PKsign == OLDALG) &&
		...
		fprintf(outFile, "-----BEGIN EBP SIGNED MESSAGE-----\n\n");
	...
	if ((algostruct.PKencrypt == OLDALG) && (algostruct.PKsign == OLDALG) &&
		...
		fprintf(outFile,
		"-----BEGIN EBP MESSAGE, PART %02d/%02d-----\n",
		1, noSections);
    }
	...
	if (++lines == pem_lines && currentSection < noSections) {
	    ...
	    fclose(outFile);
#ifdef MACTC5
		EBPSetFinfo(curOutFile,'TEXT','MEBP');
#endif
    ...
    if (noSections == 1) {
		...
		if ((algostruct.PKencrypt == OLDALG) && (algostruct.PKsign == OLDALG) &&
			...
			fprintf(outFile, "-----END EBP MESSAGE, PART %02d/%02d-----\n",
			noSections, noSections);
	}
    /* Done */
    fclose(inFile);
...
armordecode(FILE *original_in, FILE *out, int *warned)
{
	...
	if (get_armor_line(inbuf, in) == NULL) {
	    ...
	    end_of_message =
		((strncmp(inbuf, "-----END EBP MESSAGE,", 21) == 0) ||
...
/* continue with decoding to see if there are other bad parts */
	    ...
	    while ((strncmp(inbuf, "-----BEGIN EBP MESSAGE", 22)) ||
				(strncmp(inbuf, "-----BEGIN PGP MESSAGE", 22)));
		
		/* Make sure this section is the correct one */
		
	    if ((2 != sscanf(inbuf,
			    "-----BEGIN EBP MESSAGE, PART %d/%d",
			    &section, &noSections)) && (2 != sscanf(inbuf,
...
/* Quit when hit the -----END EBP MESSAGE----- line or a blank,
 * or handle checksum
 */
...
LANG("ERROR: Badly formed ASCII armor checksum, line %d.\n"), line);
	...
	if ((strncmp(inbuf, "-----END EBP ", 13) == 0) ||
		(strncmp(inbuf, "-----END PGP ", 13) == 0))
	    break;
...
is_armorfile(char *infile)
{
	...
	if ((strncmp(inbuf, "-----BEGIN EBP ", 15) == 0) || 
		(strncmp(inbuf, "-----BEGIN PGP ", 15) == 0)) {
	    if ((strncmp(inbuf,
		    "-----BEGIN EBP SIGNED MESSAGE-----", 34) == 0) ||
			(strncmp(inbuf,
...
darmor_file(char *infile, char *outfile)
{
	...
	fprintf(ebpout, LANG("ERROR: Can't find file %s\n"), infile);
	return 10;
    }
	...
	if ((strncmp(buf, "-----BEGIN PGP ", 15) == 0) ||
		(strncmp(buf, "-----BEGIN EBP ", 15) == 0))
	    break;
	}
    if ((strncmp(buf, "-----BEGIN EBP SIGNED MESSAGE-----", 34) == 0) ||
		(strncmp(buf, "-----BEGIN PGP SIGNED MESSAGE-----", 34) == 0)){
	FILE *litout;
	int nline;

	/*
	 * It would be nice to allow headers here, as we could add
	 * additional information to EBP/PGP messages, but it appears to
	 ...
	 * EBP/PGP's jurisdiction, but that has a few conflicts with reality.
	 */
	charset_header[0] = '\0';
	if (get_armor_line(buf, in) == NULL) {
		fprintf(ebpout,
...
LANG("ERROR: ASCII armor decode input ended unexpectedly!\n"));
	    ...
	    if ((strncmp(buf, "-----BEGIN EBP ", 15) == 0) ||
			(strncmp(buf, "-----BEGIN PGP ", 15) == 0))
		break;
	    if (nline)
		putc('\n', litout);
    ...
    if (feof(in)) {
	fprintf(ebpout, LANG("ERROR: Hit EOF in header.\n"));
	fclose(in);
	return 13;
    }
