...
static unsigned int jonas_modmults = 0;
static unsigned int jonas_modsquares = 0;
#endif				/* COUNTMULTS */
...
/*
 * Uses string replacement to reduce number of modmults, e.g., 
 * 10110111 has the 5-SR representation 10030007.
	      ...
	      register unitptr exponent, register unitptr modulus)
{
    ...
    short oldprecision;
    register unit bitmask;
    unit product[MAX_UNIT_PRECISION], m[6][MAX_UNIT_PRECISION];
    unit temp[MAX_UNIT_PRECISION];
	short eprec;

#ifdef COUNTMULTS
    jonas_modmults = 0;		/* clear "number of modmults" counter */
    jonas_modsquares = 0;	/* clear "number of modsquares" counter */
#endif				/* COUNTMULTS */
    mp_init(expout, 1);
    ...
    init_bitsniffer(exponent, bitmask, eprec, bits);
   	...
   	mp_init(m[0],1);		/* Initialize m[0] to m[5] */
	mp_move(m[1],expin);
	for (ones=2;ones<6;ones++)
		mp_init(m[ones],0);
...
/* We can "optimize out" the first modsquare and modmult: */
	...
	poll_for_break();	/* polls keyboard, allows ctrl-C to
				   abort program */
	ones=0;
	mp_move(product,expout);
	while (sniff_bit(exponent, bitmask) && bits && ones<5){ 
		ones++, bits--;
		mp_move(expout,product);
		mp_modsquare(product, expout);
#ifdef COUNTMULTS
	jonas_modsquares++;	/* bump "number of modsquares" counter */
#endif				/* COUNTMULTS */
		if (testeq(m[ones],0)){
			mp_modsquare(temp,m[ones-1]);
#ifdef COUNTMULTS
	jonas_modsquares++;	/* bump "number of modsquares" counter */
#endif				/* COUNTMULTS */
			mp_modmult(m[ones], temp, expin);
#ifdef COUNTMULTS
	jonas_modmults++;	/* bump "number of modmults" counter */
#endif				/* COUNTMULTS */
	}
		bump_bitsniffer(exponent, bitmask);
	}
	...
	jonas_modsquares++;	/* bump "number of modsquares" counter */
		...
		mp_modmult(expout, product, m[ones]);
#ifdef COUNTMULTS
	jonas_modmults++;	/* bump "number of modmults" counter */
#endif	/* COUNTMULTS */
	}
	...
	atomic_mults = (long) jonas_modmults *(unitcount * unitcount);
	atomic_mults += (long) jonas_modsquares *(unitcount * unitcount);
	printf("\n%ld atomic mults for ", atomic_mults);
	printf("\n%d+%d = %d modsqr+modmlt, at %d bits, %d words.\n",
	       jonas_modsquares, jonas_modmults,
	       jonas_modsquares + jonas_modmults,
