 ...
 * system.c
 *
 * Routines specific for non-MSDOS implementations of ebp/pgp.
 * 
 ...
 * variable requesterdesc, which holds the last string printed to ebpout
...
int AmigaRequestString(char *buffer, int maxlen, int echo)
{
        ...
        ti[1].ti_Data = (ULONG) (requesterdesc) ? ((*requesterdesc == '\n') ? requesterdesc+1 : requesterdesc) : "Please enter required string";
                                 ...
                                 */
        sprintf(name, "EBPAmiga %s", rel_version);
        ...
        {  fprintf(stderr,"Could not open ReqTools.library!  Try using EBP "
                   "without -f.\n");
           exitEBP(7);   /* Error exit */
        }
