/**********************************************************************
	...
	NO_ITIMER for the isc port and randaccum_later.
        ...
        7/93 Fix from Edgar Swank for -f passphrase lockup (MSDOS).  [2.62]
        5/93 Added changes from Howard Rensin & Damien Doligez
             (HRDD) to increase maximum keysize to ~4096
        5/95 Use pseudorand for overwrite of temp files to avoid
...
long pseudorand(void);  /* 31-bit LCG pseudorandom generator */
...
#if !defined(__TURBOC__) && !defined(_MSC_VER)     /*[2.62] fix for -f passphrase lockup */
#include <conio.h>
#else
int getch();
int kbhit();
