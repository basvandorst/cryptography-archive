 ...
 *
 *      7/93  Add fix for -f passphrase lockup (MSDOS) [2.62]
 *      1/95  Ctrl-C trap in keyboard reading routines. [2.62].
 *      4/95  Add setcbrk/getcbrk for MSC
 */
#include <stdio.h>
#include "exitpgp.h"
#include "system.h"
...
/*[2.62] Fix -f lockup on passphrase prompts for TURBO C++ */
#if defined(MSDOS) && !defined(__GO32__) && defined(__TURBOC__)
#include <bios.h>
#include <signal.h>
#if !defined(_KEYBRD_READY)
#define _KEYBRD_READY 1    /* To support old versions of Turbo C [2.62]*/
#endif
#if !defined(_KEYBRD_READ)
#define _KEYBRD_READ 0     /* To support old versions of Turbo C [2.62]*/
#endif
...
int getch(void)
 {
   ...
   if (c==11779) raise(SIGINT);   /* Ctrl-C.  [2.62] */
   return c & 0xff;  /*[2.62a]*/
 } /*getch*/
#endif

/*[2.62] Fix -f lockup on passphrase prompts for MSC */
