/*	armor.c  - ASCII/binary encoding/decoding based partly on PEM RFC1113.
	...
	and export controls.  
        ...
        7/93 Added code to support labeling of -kxa keyrings  [2.62]
        9/94 Added code to support "comment" config.txt parameter. [2.62]
             Changed -kvv kxa-labeling to -kv to avoid out of mem on long
             keyrings. [2.62]
        10/94 Added code to check for 1 & only 1 blank line
              after BEGIN PGP SIGNED ...
        12/94 Check for duplicate output for multi-seg armored section.
        07/95 Check for Write errors.  2.63
        07/95 Adjust armor_lines for & insert text generated by
              LabelEncrypt=yes 2.63
*/
 ...
 * special header line.
 */
extern char glb_ringfile[MAX_PATH] ;  /*[2.62] Parameters defined in KEYMGMT.C*/
extern boolean glb_whole_ring;
static
int pem_file(char *infilename, char *outfilename, char *clearfilename)
{
	...
	if (!outfilename || pem_lines == 0)
		...
		rewind(inFile);
         enc_lab_len=enc_label_lines*LINE_LEN;
         noSections = (fileLen + enc_lab_len + BYTES_PER_SECTION - 1) / BYTES_PER_SECTION;
		if (noSections > 99)
		{
          pem_lines = ((fileLen+enc_lab_len+LINE_LEN-1)/LINE_LEN + 98) / 99;
          noSections = (fileLen + enc_lab_len + BYTES_PER_SECTION - 1) / BYTES_PER_SECTION;
	...
	if (outfilename == NULL)
        ...
        {
         if (noSections > 1)   /*[2.62] ck dup output of 1st armored sect.*/
          {
           do
            {
             char *t;
             ...
             t=ck_dup_output(outfilename,TRUE);
	...
	{	fclose(inFile);
        ...
        if (enc_label_a)  /*Insert Encrypted File Label 2.63*/
         {
          long fl;
          int status;
          fclose(enc_lab_f);
          enc_lab_f = fopen (enc_lab_tmp,FOPRTXT);
          fseek(enc_lab_f,0L,SEEK_END);
          fl = ftell(enc_lab_f);
          rewind(enc_lab_f);
          status = copyfile(enc_lab_f,outFile,fl);
          fclose(enc_lab_f);
          rmtemp(enc_lab_tmp);
          enc_label_a = FALSE;
         }
	...
	{
		byte ctb = 0;
                int  keycounter = 0;  /*[2.62]*/
                int  status;          /*[2.62]*/
		ctb = getc(inFile);
		if (is_ctb_type(ctb, CTB_CERT_PUBKEY_TYPE))
                 {
			blocktype = "PUBLIC KEY BLOCK";
                   if (!glb_whole_ring)   /*no titles if extracting whole ring [2.62]*/
                    {
                     kv_title(outFile);     /*[2.62] Title-Line */
                     rewind(inFile);        /*[2.62] Back over CTB*/
#if 0    /* 9/94 [2.62] -kv (not kvv) in -kxa*/
                     setkrent(glb_ringfile);/*[2.62] Setup Hash table for sig lookup*/
                     setkrent(NULL);        /*[2.62] Add default Keyring */
                     init_userhash();
#endif
                     status =
                      kvformat_keypacket(inFile,outFile,TRUE,"",infilename,
                                         FALSE,FALSE,&keycounter);   /* 9/94 [2.62] -kv (not kvv) in -kxa*/
#if 0    /* 9/94 [2.62] -kv (not kvv) in -kxa*/
                     endkrent();
#endif
                    }
                   else
                    fprintf(outFile, PSTR("Entire Key Ring\n"));
                 }  /*[2.62]  Write Label lines*/
	...
	while((bytesRead = fread(buffer,1,LINE_LEN,inFile)) > 0)
		...
		putc('\n',outFile);
         ...
         if (++lines == pem_lines-enc_label_lines && currentSection < noSections)
         {
          lines = 0;
          enc_label_lines = 0;
			outcrc (crc, outFile);
...
char	*litfile = NULL;
	...
	if (strncmp(buf, "-----BEGIN PGP SIGNED MESSAGE-----", 34) == 0) {
		...
		{	fprintf(pgpout,PSTR("\n\007Unable to write ciphertext output file '%s'.\n"), litfile);
			...
			return(-1);
                ...
                /* Skip exactly one blank line [2.62] */
                ++infile_line;
			status = skipline(in);
                if (status)
                 {
                  fprintf(pgpout,PSTR("\n\007Incorrect Format for Clearsigned Message.\n"));
                  exit(8);
                 }
	...
	{	/* Glue the literal file read above to the signature */
		...
		write_ctb_len (out, CTB_LITERAL2_TYPE, fsize(f)+6, FALSE);
                fwritet ( &lit_mode, 1, 1, out );        /*      write lit_mode */
		fputc ('\0', out);			/* No filename */
                fwritet ( &dummystamp, 1, sizeof(dummystamp), out); /* dummy timestamp */
...
armor_file (char *infile, char *outfile, char *filename, char *clearname)
{
	if (verbose)
		fprintf(pgpout,"armor_file: infile = %s, outfile = %s, filename = %s, clearname = %s\n",
                 ...
                 clearname == NULL ? "" : clearname);  /*2.63ui*/
