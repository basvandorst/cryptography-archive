/*	keyadd.c  - Keyring merging routines for PGP.
	...
	and export controls.  
        ...
        12/94 Only ask once for a succession of armored keys in a single
              file, as is presented by key servers with LAST parameter. [2.62]
        07/95 Check for Write errors. [2.63]
        07/95 Check for Write Access for Keyrings [2.63]
...
static boolean never_ask=FALSE;
...
Acknowledge by pressing return: "), keyIDstring(keyID), ringfile);
	...
	*/
	...
	if (newsigs == 0 && newkeys == 0 && newids == 0 && newrvks == 0)
	{
        ...
        if ((status = dokeycheck(NULL, scratchf, CHECK_NEW)) != 0)  /*[2.62] avoid compile warning*/
	{	if (verbose)
...
static int ask_to_sign(byte *keyID, char *ringfile)
{
	...
	if ((trust & KC_LEGIT_MASK) == KC_LEGIT_COMPLETE)
	{
        ...
        if (never_ask)     /*Only ask once for string of armored keys [2.62]*/
                {
                        fclose(f);
                        return 1;
                }
