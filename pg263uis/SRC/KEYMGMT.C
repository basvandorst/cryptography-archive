/*	keymgmt.c  - Key management routines for PGP.
	...
	and export controls.  
        ...
        7/93 Added support for labeling -kxa output  [2.62]

        5/94 Add changes from Howard Rensin & Damien Doligez
             (HRDD) to increase maximum keysize to ~4096

        9/94  Support for pubring/secring config parameters as per MIT PGP. [2.62]
        12/94 Add support for pubring2 config parameter  [2.62]
        12/94 Fix bug in dokeyedit. support pubring/secring in diff dirs. [2.62]

        01/95 No retry of passphrase entry for null passphrase [2.62]
        01/95 Change msg texts per VVB  [2.62]
        03/95 Check for dup output on -KXA  [2.62]
        07/95 Check for write errors. [2.63]
        07/95 Check for Write Access before keygen or remove. [2.63]
        07/95 Insert version_byte into extracted keys. (Support sending
              keys to old versions of PGP). [2.63]
        08/96 Allow Editing UserID's of Other People's Keys [2.63]
*/
...
/*[2.62] add file parameter for -kxa labeling*/
 ...
 */
{
	...
	ctb = CTB_KEYCTRL;
        fwritet(&ctb,1,1,f);             /* write key control header byte */
	keyctrllen = 1;
        fwritet(&keyctrllen,1,1,f);      /* write key control length */
        fwritet(&trustbyte,1,1,f);       /* write key control */
}
...
{	FILE *f;
	...
	{	/* secret key certificate */
        ...
        fwritet(&ctb,1,1,f);             /* write key certificate header byte */
	convert(cert_length);	/* convert to external byteorder */
        fwritet(&cert_length,1,sizeof(cert_length),f);
	version = version_byte;
        fwritet(&version,1,1,f);         /* set version number */
	convert_byteorder(timestamp,4);	/* convert to external form */
        fwritet(timestamp,1,4,f); /* write certificate timestamp */
	convert_byteorder(timestamp,4);	/* convert back to internal form */
	validity = 0;
        fwritet (&validity,1,sizeof(validity),f);        /* validity period */
	...
	if (is_secret_key(ctb))	/* secret key */
	{
		...
		{	for (i=0; i<8; i++)
			...
			if (hidekey)  /* encrypt the IV */
				ideacfb(iv,8);
                        fwritet(iv,1,8,f);       /* write out the IV */
		}
		mpi_checksum = 0;
		...
		convert(mpi_checksum);
                fwritet (&mpi_checksum,1,sizeof(mpi_checksum),f);
	} else {
	...
	/* User ID packet */
	ctb = CTB_USERID;
        fwritet(&ctb,1,1,f);             /* write userid header byte */
        fwritet(userid,1,userid[0]+1,f); /* write user ID */
...
nogood:
	...
	if (secret)
		...
		fprintf(pgpout,PSTR("Enter secret key filename: "));
         ...
         {
          if (!globalPubringName2_tried && globalPubringName2[0]) /*2.63*/
           {
            strcpy(keyfile,globalPubringName2);
            globalPubringName2_tried = TRUE;
            fprintf(pgpout, PSTR("\nContinue Search in file %s (Y/n)? "),
                    keyfile);
            if (getyesno('y')) goto top;
           }
...
*/
{
	...
	{	/* use default pathname */
                strcpy(keyfilename, globalSecringName);  /*[2.62]*/
		keyfile = keyfilename;
	}
	...
	{	hidekey = (GetHashedPassPhrase((char *) ideakey, 1) > 0);
		...
		if (status != -5)
			goto done;
		fprintf(pgpout, PSTR("\n\007Error:  Bad pass phrase.\n"));
                if (hidekey == 0) break;  /*[2.62] no retry if null passphrase*/
	} while (--guesses);
...
{	FILE *f;
	...
	/* Default keyring to check signature ID's */
        strcpy(dfltring, globalPubringName);  /*[2.62]*/
...
*/
	...
	if (!quietmode)
	{
      ...
      /* [2.62]  Change point for Key Block Labeling*/
        fprintf(pgpout,"\n"); /*Leading \n required for -kvv*/
        kv_title(pgpout);  /* [2.62] emit kv-type title line to pgpout*/
	...
	if (show_signatures)
		endkrent();
        fprintf(pgpout,PSTR("%d matching key(s) found.\n"),keycounter); /*[2.62]*/
	close_more();
			...
			{	/* Try checking signature on either this ring or dflt ring */
				...
				if (sigstatus >= 0)
					...
					fprintf(pgpout,"*      ");	/* bad signature */
                                showkeyID(sigkeyID,pgpout);             /*[2.62]*/
				if (sigstatus == -1)
...
Do you want to remove the whole key (y/N)? "));
	...
	if (secring_too)
	{	secring_too = FALSE;
                strcpy(ringfile, globalSecringName); /*[2.62]*/
		strcpy((char *)userid,mcguffin);
...
/*[2.62]  modified for key extract label display*/
...
*/
{
	...
	{	fprintf(pgpout,PSTR("\n\007Can't open key ring file '%s'\n"),ringfile);
        ...
        strcpy(glb_ringfile,ringfile);  /*[2.62]*/
	if (!whole_ring)
	{
		strcpy((char *)userid, mcguffin);
			...
			&& (keyctrl & KC_DISABLED))
		{
			...
			continue;
		}
                {
                 ...
                 int rl;
                 fseek(f,fp,SEEK_SET);
                 rl=fread(kpbuf,1,4,f);
                 if (rl<4)
                 {       status = -2;
                         break;
                 }
                 if (version_byte == 2)
                  kpbuf[3]=version_byte;
                 fwritet(kpbuf,1,4,g);
                 status=copyfilepos(f, g, (long) packetlength-4, fp+4);
                 if (status < 0)     /* Copy key out */
		{	status = -2;
			break;
		}
                }
		...
		{	fp = ftell(f);
			...
			if (status < 0  ||  is_key_ctb(ctb))
				break;
                        if (ctb==CTB_USERID)
                         {
                          status=copyfilepos(f, g, (long) packetlength, fp);
                          if (status < 0)     /* Copy key out */
                          {       status = -2;
                                  break;
                          }
	...
	if (transflag)
        {
         do       /*[2.62] Check for Dup Armor output file*/
          {
           char *t;
           force_extension(transfile, ASC_EXTENSION);
           if (!file_exists(transfile)) break;
           t=ck_dup_output(transfile,TRUE);
...
{	unit n[MAX_UNIT_PRECISION], e[MAX_UNIT_PRECISION],
	...
	force_extension(ringfile,PGP_EXTENSION);
        ckwacc(ringfile); /*Write access required 2.63*/
...
Defaulting to public keyring."));
                strcpy(ringfile, globalPubringName);  /*[2.62]*/
	}
	...
	/* Now read private key, too */
        strcpy(secring, globalSecringName);  /*[2.62]*/
	...
	if (status < 0)		/* key not in secret keyring: edit owner trust */
        {
         ...
         fprintf(pgpout, PSTR("\nNo secret key available.  Want to Edit UserID of Someone Elses Key? "));
         if (getyesno('n'))  /*2.63ui*/
         {
         }
         else
         {
        ...
        else /*key is in secret keyring 2.63ui*/
        {
	...
	CToPascal((char *)userid);
        ...
        fprintf(pgpout, PSTR("\nDo you want to add a user ID (y/N)? ")); /*[2.62]*/
	...
	fclose(f);
        ...
        if (status >= 0)         /* key must be in secret keyring to change passphrase 2.63ui */
        {
	...
	/* First write secret key data to a file */
        if (status >= 0)         /* key must be in secret keyring to update 2.63ui */
        {
...
{	unit n[MAX_UNIT_PRECISION], e[MAX_UNIT_PRECISION], d[MAX_UNIT_PRECISION],
	...
	byte ideakey[16];
        ...
        ckwacc(globalPubringName);  /*2.63*/
        ckwacc(globalSecringName);
...
\nChoose 1, 2, or 3, or enter desired number of bits (384-%d): "),
                MAX_KEYGEN_PRECISION-2*UNITSIZE);    /*HRDD*/
	...
	if (keybits < 384) keybits=384;
        if (keybits > MAX_KEYGEN_PRECISION-2*UNITSIZE)       /* Paranoia */
                keybits = MAX_KEYGEN_PRECISION-2*UNITSIZE;
#else
        if (keybits > MAX_KEYGEN_PRECISION)
                keybits = MAX_KEYGEN_PRECISION;
#endif
...
/* As rsa_keygen does a major accumulation of random bits, if we need
   any others for a seed file, let's get them at the same time. */
	...
	fname = tempfile(TMP_TMPDIR|TMP_WIPE);
	writekeyfile(fname,hidekey,timestamp,userid,n,e,d,p,q,u); 
        if (file_exists(globalSecringName))     /*[2.62]*/
        {
                merge_key_to_ringfile(fname, globalSecringName, 0L, 0, -1L);
		rmtemp(fname);
	}
	else
                savetemp(fname, globalSecringName);  /*[2.62]*/
	...
	writekeyfile(fname,FALSE,timestamp,userid,n,e,NULL,NULL,NULL,NULL);
        ...
        {
                merge_key_to_ringfile(fname, globalPubringName, 0L, 0, -1L);
		rmtemp(fname);
	}
	else
                savetemp(fname, globalPubringName);
...
/* [2.62] Output kv[v]-type key title line to specified open file*/
/* Does double duty for both -kv[v] and -kxa  */
/* NOTE: Removal of leading \n for PSTR!!  */
void kv_title(FILE *fo)
 {
        fprintf(fo,PSTR("Type bits/keyID    Date        User ID\n"));
        return;
 } /* kv_title */

/* [2.62] format key(s) from keyring f to pgpout */
/* Does double duty for both -kv[v] and -kxa  */
                           ...
                           int *keycounter)
  {
    ...
    boolean first_key= FALSE;
        ...
        for ( ; ; )
        {
                ...
                if (ctb == CTB_USERID)
                 {PascalToC((char *)userid);
                  if (!one_key)         /*[2.62] Take 1st key if -kxa*/
                   match = userid_match((char *)userid,mcguffin,n);
                 }
                if (match || one_key)  /*[2.62]*/
                {
                        if (ctb == CTB_USERID)
                        {       if (firstuser)
                                {       (*keycounter)++;
