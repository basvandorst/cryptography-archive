/*	crypto.c  - Cryptographic routines for PGP.
	...
	compatibility with MIT PGP 2.6 release..
        ...
        Modified 9-Oct-94 Jonathan M. Gilligan  [2.62]
        Fixed bug in conversion to canonical form in
        crypto.c, check_signaturefile()

        9/94 Support for pubring/secring/randseed config parameters
             as per MIT PGP. [2.62]

        12/94 Check for dup output for sig files & allow armoring of
              sig files for -b and -ba.   [2.62]
        12/94 Provide random initial vector for conventional encryption [2.62]
        01/95 Change msg text.  [2.62]
        02/95 Provide warning if ENCRYPTTOSELF specified but MYNAME not. [2.62]
        03/95 Correct err in make_signature_certificate per post on a.s.pgp
              on Mar 2 by David A. Barnhart of ViaCrypt. [2.62]
        05/95 Allow option to continue on "Corrupt Plaintext"
        07/95 Check for Write errors.  [2.63]
        07/95 Add PrePend Signature Verification to output text. [2.63]
        10/95 Display more key data on Sig Verification
 ...
 */
{
        ...
        if (file_exists(globalRandseedName))  /*[2.62]*/
		return 1;	/* True */
 ...
 */
{
	...
	int i;
        ...
        f = fopen(globalRandseedName,FOPWBIN);    /* open for writing binary [2.62] */
...
/* This is not part of the string above because it was changed after the 2.2
   release translations were started.  It should be cleaned up eventually. */
	...
	fseek (f, 0, SEEK_SET);
#endif
        fwritet(randbuf,1,RANDSEED_SIZE,f);
	fclose(f);
   ...
   Returns the length of the resultant data, or -1 in case of error. */
{
	...
	static word16 iv0[4] = {0, 0, 0, 0};
        ...
        if (!file_exists(globalRandseedName))     /* No seed file - not available. [2.62]*/
		return(-1);
        f = fopen(globalRandseedName,FOPRBIN);    /* open for reading binary [2.62]*/
...
{	static word16 iv0[4] = {0, 0, 0, 0};
	...
	/* Now at start of file again */
        i = fwritet(buf,RANDSEED_SIZE,1,f);      /* Write it out */
	fclose(f);
...
/*	Make a random IDEA key.  Returns its length (the constant 16).
   ...
   The "skip" parameter says to skip that many bytes at the beginning, [2.62]
...
{
	word32 tstamp;
        byte keysav[IDEAKEYSIZE];  /*[2.62]*/
	int count;

        memcpy(keysav,key,IDEAKEYSIZE); /*[2.62]*/
...
/* If we have no random bytes, the randombyte() part will be useless */
	count = IDEAKEYSIZE+RAND_PREFIX_LENGTH;
        while (count>skip)  /*[2.62]*/
         {
          count--;
		key[count] = idearand() ^ try_randombyte();
         }
        if (skip>0)
         memcpy (key,keysav,skip);  /*[2.62]*/
...
#define RAND_PREFIX_LENGTH 8
	...
	{	/* There is a random prefix followed by 2 key check bytes */
		...
		ideacfb(textbuf,RAND_PREFIX_LENGTH+2);
                fwritet(textbuf,1,RAND_PREFIX_LENGTH+2,g);
	}
...
*/
{
        byte inbuf[MAX_BYTE_PRECISION], outbuf[MAX_BYTE_PRECISION+2]; /*[2.62]*/
 ...
 */
    ...
    fputs("VMS ", g);
    fwritet(&fdl_len, 2, 1, g); /* Byte order *not* important, only VMS reads this!*/
    fwritet(fdl, 1, fdl_len, g);
}
#endif /* VMS */

/*======================================================================*/
...
{	
	...
	{	/* temporary scope for some buffers */
		...
		extract_keyID(keyID, n);
                strcpy(keyfile, globalPubringName); /* use default pathname [2.62]*/
	...
	/* write out certificate record to outfile ... */
        fwritet(certificate,1,certificate_length,g);
...
#endif /* USE_LITERAL2 */
                        fwritet ( &lit_mode, 1, 1, g );  /*      write lit_mode */
			if (lit_mode == MODE_LOCAL) {	
#ifdef VMS
			    ...
			    /* write literalfile name */
                                fwritet (lfile, 1, (unsigned char) lfile[0]+1, g);
			    /* Dummy file creation timestamp */
                            fwritet ( &dummystamp, 1, sizeof(dummystamp), g);
			}
		}
...
{	
	...
	extract_keyID(keyID, n);	/* Remember signer key ID */
        strcpy(pubring, globalPubringName);  /*[2.62]*/
	...
	{	/*	Signature certificate has no plaintext following it.
		...
		{	strcpy(plainfile, outfile);
			...
			{	
				if (batchmode)
					return -1;
                                fprintf(pgpout,PSTR("\nPlease enter filename that signature applies to: ")); /*[2.62]*/
		...
		{	/* Read literal data */
			...
			{	fprintf(pgpout,"\n\007Error: Illegal mode byte %02x in literal packet.\n",
				...
				goto err1;
                        ...
                        if (verbose || PPSigV && !quietmode)  /*[2.63]*/
...
#if defined(MSDOS)  /*[2.62]*/
                        make_canonical(outfile,tempFileName);
                        tempFile = fopen( tempFileName, FOPRBIN);
#else
                        tempFile = fopen( tempFileName, FOPWPBIN );
#endif
                      if( tempFile != NULL )
...
#endif	/* !CANONICAL_TEXT */
         if (PPSigV && !strip_signature && lit_mode == MODE_TEXT) /*PrePend Sig Ver 2.63*/
          g = fopen(outfile, FOPWTXT);
         if (g != NULL)
          {
           fprintf(g,PSTR("[PGP Signature Verification]\n"));
           fprintf(g,PSTR("WARNING: Bad signature, doesn't match file contents!\n"));
           fprintf(g,PSTR("Bad signature from user \"%s\".\n"),
                   LOCAL_CHARSET((char *)userid));
           fprintf(g, PSTR("%d-bit key, key ID %s, created %s\n"),
                   countbits(n), key2IDstring(n), cdate(&dummystamp));
           fprintf(g,PSTR("Signature made %s\n"),ctdate((word32 *)timestamp));
           fprintf(g,"\n");
          }
         fprintf(pgpout,"\007\n");
         fprintf(pgpout,PSTR("WARNING: Bad signature, doesn't match file contents!\n"));
         fprintf(pgpout,"\007");
...
goodsig:
        ...
        if (g != NULL)
         {
          fprintf(g,PSTR("[PGP Signature Verification]\n"));
          fprintf(g,PSTR("Good signature from user \"%s\".\n"),
                  LOCAL_CHARSET((char *)userid));
          fprintf(g, PSTR("%d-bit key, key ID %s, created %s\n"), /*[2.63]*/
                  countbits(n), key2IDstring(n), cdate(&dummystamp));
          fprintf(g,PSTR("Signature made %s\n"),ctdate((word32 *)timestamp));
	...
	signature_checked = TRUE;	/* set flag for batch processing */
        fprintf(pgpout,"\n");
        fprintf(pgpout,PSTR("Good signature from user \"%s\".\n"),
		LOCAL_CHARSET((char *)userid));
        fprintf(pgpout, PSTR("%d-bit key, key ID %s, created %s\n"),  /*[2.63]*/
                countbits(n), key2IDstring(n), cdate(&dummystamp));
	fprintf(pgpout,PSTR("Signature made %s\n"),ctdate((word32 *)timestamp));
...
outsig:
	...
	{	/* produce a plaintext output file from signature file */
		...
		{	/* Copy signature to a .sig file */
                        char *tempf = NULL;       /*[2.62]*/
			strcpy (sigfile, outfile);
			force_extension(sigfile,SIG_EXTENSION);
                 do             /*[2.62] Check for Dup Output of Sig File*/
                  {
                   char *t;
                   if (no_extension(sigfile))
                    force_extension(sigfile,SIG_EXTENSION);
                   if (!file_exists(sigfile)) break;
                   t=ck_dup_output(sigfile,TRUE);
                   if (t==NULL) goto err1;
                   strcpy(sigfile,t);
                  } while (TRUE);
                 if (!emit_radix_64)
			...
			{	fprintf(pgpout,PSTR("\n\007Can't create signature file '%s'\n"),sigfile);
				goto err1;
                 ...
                 else  /*[2.62] Temp for Sig File if to be Armored -ba*/
                  {
			...
			if (!quietmode)
				fprintf(pgpout,PSTR("\nWriting signature certificate to '%s'\n"),sigfile);
                 if (emit_radix_64)   /*[2.62] Cvt Sig File to Armored if -ba*/
                  {
                   process_output(tempf,sigfile,FALSE);
                  }
...
int squish_file(char *infile, char *outfile)
{
	...
	/* We don't put a length field on CTB_COMPRESSED yet */
	ctb = CTB_COMPRESSED;		/* use compression prefix CTB */
        fwritet( &ctb, 1, 1, g );        /* write CTB_COMPRESSED */
	/* No CTB packet length specified means indefinite length. */
	ctb = ZIP2_ALGORITHM_BYTE; 	/* use ZIP compression */
        fwritet( &ctb, 1, 1, g );        /* write ZIP algorithm byte */
	...
	boolean attempt_compression)
{
	...
	{	if (!quietmode)
         ...
         */
        make_random_ideakey(ideakey, 16);  /*[2.62]*/
	...
	boolean attempt_compression)
{
	...
	*/
	...
	{	MD5_CTX MD;
        ...
        ckp_length = make_random_ideakey(ideakey,0);  /*[2.62]*/
...
/* Assume MSB external byte ordering */
	...
	for (; *mcguffins && **mcguffins ; ++mcguffins) {
                ...
                /* use default pathname [2.62]*/

                /*Allow pubring2 search for each UserID*/
                globalPubringName2_tried=FALSE;  /*[2.62]*/
		keys_used = 
	...
	/* encrypt to myself if need be */
		...
		keys_used = 
           ...
           fprintf(pgpout,PSTR("\007\nWARNING: \
ENCRYPTTOSELF ignored because MYNAME not specified.\n"));
         }
	free(keyID_list);
	...
	if (!gotkey || verbose) {
		...
		if (gotkey)	/* verbose flag */
			fprintf(pgpout,"\nRecipients:\n");
                 ...
                 {
                  fprintf(pgpout,"\n");
                  fprintf(pgpout,PSTR("This message can only be read by:\n"));
                 }
...
/* Assume MSB external byte ordering */
	...
	if (status < 0)
        {
         fprintf(pgpout,PSTR("\n\007Error: Decrypted plaintext is corrupted. Continue Anyway? (y/N): "));
         if (getyesno('n'))  /*[2.62]*/
          status=0;
	}
	if (!quietmode)
