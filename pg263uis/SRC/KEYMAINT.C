/*	keymaint.c  - Keyring maintenance pass routines for PGP.
	...
	keymaint.c implemented by Branko Lankester.
        ...
        7/95 Support for Encrypted Armored File Labeling [2.63]
...
struct signature {
	...
	struct signature *sig_nextfrom;	/* list of sigs made by the same key (sig_from) */
	byte sig_trust;
};
...
static int maintenance(char *ringfile);
static int maint_read_data(char *ringfile);
...
static int check_secretkey(FILE *f, long keypos);
...
static void wkeyfmt(FILE *pgpout);
 ...
 */
int
maint_update(char *ringfile)
{
	check_only = mverbose = FALSE;
	return maintenance(ringfile);
}
 ...
 */
int
maint_check(char *ringfile, int options)
{
	int status;
	char *fixfile;
	...
	if (moreflag)
		open_more();
	if (*floppyring != '\0' && (floppy_fp = fopen(floppyring, FOPRBIN)) == NULL)
		fprintf(pgpout,PSTR("\nCan't open backup key ring file '%s'\n"),
				floppyring);
	check_only = TRUE;
	status = maintenance(ringfile);
	if (floppy_fp) {
		fclose(floppy_fp);
		floppy_fp = NULL;
	}
	...
	if (undefined_trust) {
		...
		fprintf(pgpout, PSTR("Continue with '%s' (Y/n)? "),
			ringfile);
		if (!getyesno('y')) {
			close_more();
			return status;
	...
	check_only = mverbose = FALSE;
	if ((status = maintenance(fixfile)) >= 0) {
		maint_list(fixfile);
...
maintenance(char *ringfile)
{
	...
	if (max_cert_depth > MAX_DEPTH)
        ...
        if ((sec_fp = fopen(globalSecringName, FOPRBIN)) == NULL)  /*[2.62]*/
	...
	status = maint_read_data(ringfile);
	if (sec_fp) {
		fclose(sec_fp);
		sec_fp = NULL;
	}
 ...
 */
static int
trace_sig_chain(struct pubkey *pk, int depth)
{
	int d, trust_count = 0;
 ...
 */
static void
compute_legit(struct userid *id)
{
	struct signature *s;
...
static char *owntrust_msg[] = {
	...
	"",
	_PSTR("This user is untrusted to certify other keys.\n"),
	"", /* reserved */
	"", /* reserved */
	_PSTR("This user is generally trusted to certify other keys.\n"),
...
static char *sigtrust_msg[] = {
	...
	"", /* reserved */
	"", /* reserved */
	_PSTR("  Generally trusted certification from:\n  "),
...
static unit n[MAX_UNIT_PRECISION], e[MAX_UNIT_PRECISION];
					...
					n, e, NULL, NULL, NULL, NULL, sigkeyID, &keyctrl)) != -1)
	{
		...
		{	if (userids == 0)
			{	PascalToC(userid);	/* for display */
				...
				if (keystatus == -4)
					fprintf(pgpout,PSTR("Bad key format.\n"));
				else if (keystatus == -6)
					fprintf(pgpout,PSTR("Unrecognized version.\n"));
				else if (what & SHOW_HASH)
					printKeyHash(hash, FALSE);
				if (compromised)
					fprintf(pgpout, PSTR("Key has been revoked.\n"));
				if (disabled)
					fprintf(pgpout, PSTR("Key is disabled.\n"));
				if (print_trust && *owntrust_msg[TRUST_LEV(keyctrl)] != '\0')
					fprintf(pgpout, PSTR (owntrust_msg[TRUST_LEV(keyctrl)]));
			}
			else
			{	PascalToC(userid);
				if (what != 0)
			...
			if (print_trust) {
				...
				fprintf(pgpout, PSTR (keylegit_msg[keyctrl&KC_LEGIT_MASK]));
			}	/* print_trust */
		}
		else if (is_ctb_type(ctb, CTB_SKE_TYPE))
		{
			...
			if (what & SHOW_SIGS) {
				if (print_trust) {
					read_trust(f, &keyctrl);
					fprintf(pgpout, PSTR (sigtrust_msg[TRUST_LEV(keyctrl)]));
				}
				else
 ...
 */
int
setkrent(char *keyring)
{
	int i;

	assert(nkr < MAXKR);
	if (keyring == NULL) {
                keyring = globalPubringName;  /*[2.62]*/
	}
	for (i = 0; i < nkr; ++i)
