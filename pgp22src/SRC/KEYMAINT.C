...
trace_sig_chain(struct pubkey *pk, int depth)
{
	...
	if (pk->pk_signed && TRUST_LEV(pk->pk_owntrust) == KC_OWNERTRUST_UNDEFINED)
		SET_TRUST(&pk->pk_owntrust,
			ask_owntrust(user_from_keyID(pk->pk_keyid), pk->pk_owntrust));
...
setkrent(char *keyring)
{
	...
	assert(nkr < MAXKR);
	if (keyring == NULL) {
		buildfilename(pubring, PUBLIC_KEYRING_FILENAME);
		keyring = pubring;
	}
	for (i = 0; i < nkr; ++i)
