...
static struct mbuf *skip_pullup(struct mbuf *m, int off, int hdrlen, int trllen)
{
  ...
  if (m && ((M_LEADINGSPACE(m) < off) || (m->m_len < hdrlen)))
  {
    /* No, prepare new mbuf */
    MGET(hdr, M_WAIT, MT_HEADER);
    if (hdr)
    {
      u_char *data;
...
static int mbuf2memblk(struct mbuf *m, struct memblk *mb, struct memseg *ms)
{
  ...
  mb->dynamiclen = 0;
  mb->ms = ms;
  mb->offset = M_LEADINGSPACE(m);
  mb->len = m->m_pkthdr.len;
  ...
  if ((m = m->m_next))  /* More than one mbuf? */
  {
    ...
    ms->ptr = mtod(m, u_char *);
    ms->len = m->m_len + M_TRAILINGSPACE(m);
  }
...
static int memblk2mbuf(struct mbuf *m, struct memblk *mb)
{
  ...
  m->m_pkthdr.len = mb->len;
  m->m_len += mtod(m, u_char *) - BLKSTART(mb);  /* Amount added in front */
  m->m_data = BLKSTART(mb);  /* Set new start of first mbuf */
		      ...
		      struct mbuf *m, int dir)
{
  ...
  if (dir == SKIP_OUTPUT)
  {
    /* Prepend interface_output parameters for feedback */
    struct mbuf *hdr;
    ...
    {
      hdr->m_next = m;
      MEMCPY(mtod(hdr, void *), feedback, sizeof(struct interface_outfbparam));
