...
char	prog_name[] = "Privtool V0.81 BETA";
...
m->flags &= ~(MESS_SIGNED|MESS_ENCRYPTED);
	...
	if (m->flags & (MESS_SIGNED|MESS_ENCRYPTED)) {
		if (!(m->flags & MESS_VERIFIED)) {
			...
			i = decrypt_message(m->message,m->decrypted,
		...
		display_message_body(m->message);

		if (buffer_contains_key (m->message))
			show_addkey ();
	}
	...
	while (m) {
		if (!(m->flags & MESS_DELETED) && (m->flags & MESS_SELECTED)) {
			...
			if ((m->flags & MESS_VERIFIED) && !raw) {
				...
				add_to_buffer (out, m->message->message,
					m->message->length);
			}
