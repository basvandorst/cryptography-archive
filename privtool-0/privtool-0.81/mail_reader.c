...
{
	if (!m->message)
		m->message = new_buffer();

	add_to_buffer(m->message,s,strlen(s));
}
...

{
	...
	else if (!strcasecmp(line,"Encrypted: PGP",15)) {
		...
		*d = 0;
		...
		if (lines > 0 || method != LINE_COUNT) {
			...
			if (method == CONTENT_LENGTH) {
				m->message->message = (byte *)malloc(m->size+2);

				fread (m->message->message,
					m->size, 1, fp);
				m->message->length = m->size;
				m->message->size = m->size+2;

				m->message->message[m->size] = 0;
				m->lines = count_lines (m->message->message,
					m->message->length);
			...
			while (!finished) {
				...
				add_body(m,line);
				add_body(m,ret_string);

				if (method == LINE_COUNT) {
					finished = (--lines <= 0);
				}
				else if (method == CONTENT_LENGTH) {
					finished = (m->message->length >= 
						m->size);
				}
			}

			m->size = m->message->length;
		}
		else {
			if (!m->message) {
				m->message = new_buffer();
...

{
	...
	return (s_buf.st_atime < s_buf.st_mtime);
}
...

{
	...
	lines = count_lines(m->message->message,m->message->length);
	...
	if (m->message && m->message->message) 
		fprintf(fp,"\n%s\n",m->message->message);
	else
		fprintf(fp, "\n\n");
}
