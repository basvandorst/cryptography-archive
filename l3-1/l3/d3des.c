 ...
 * MODIFIED FOR L3 - Mark Henderson <markh@wimsey.com>
 *
...
/* just single des, but using Kn4 */
void des4(inblock, outblock)
unsigned char *inblock, *outblock;
{
	unsigned long work[2];
...
void des4key(unsigned char *hexkey, short mode)
{
	unsigned char *first, *third;
	short revmod;
...
void close_des(void)
{
	zap(&KnL[0], sizeof(unsigned long) * 32);
	zap(&KnR[0], sizeof(unsigned long) * 32);
	zap(&Kn3[0], sizeof(unsigned long) * 32);
}
