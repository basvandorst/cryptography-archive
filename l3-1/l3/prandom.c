...
#define RBUFLEN 524288
static void (*oldint) () = (-1);
...
static void getsname()
{
	char *p, *s;
	if ((p = getenv("L3PATH"))) {
		strncpy(sname, p, 1000);
		s = strchr(sname, ':');
		if (!s) {
			sname[999] = '\0';
		}
...
/* DO NOT CHANGE THE FOLLOWING DEFINES - mch */
 ...
 */
void ConvertChunkOfBytes(charBuffer, wordBuffer)
...
word32 input[INPUT_BLOCK_SIZE];
{
	...
	word32 *SBox0;
	word32 *SBox1;
	word32 B00, B01, B02, B03, B04, B05, B06, B07, B08, B09, B10, B11, B12,
	 B13, B14, B15;
 ...
 */
void Increment64BitCounter(counter, increment)
word32 counter[2];
long int increment;
{
...
unsigned long rnd()
{
	register unsigned long v, v1, v2;	/* working value */

	if ((++r1) == 48)
		r1 = 0;	/* advance r1 - for x_{n-48} */
	if ((++r2) == 48)
		r2 = 0;	/* advance r2 - for x_{n-8} */
...
static long initab[800] =
{/* vector of lth 256, with start point
...
void tran(unsigned char *s, int l)
{
	register long i, len;
	...
	unsigned char *buf;
	char tmp;
	len = (l > BLOCKSIZE) ? BLOCKSIZE : l;
	buf = s;
	for (i = 0; i < 256; i++)
		key[i] = 0;	/* init the histogram array */
	for (i = 0; i < len; i++)
		key[buf[i]]++;	/* gather the histogram */
...
{
    int l;
    l = fread(bbuf,1,512000,stdin);
    fwrite (bbuf,1,l,stdout);
}
*/
 ...
 *
 */
void sn512string(in, buflen, out)
unsigned char *in;
int buflen;
unsigned char *out;
{
	...
	word32 bitCount[2] =
	{0, 0};	/* the 64-bit count of the number of
                     * bits in the input */
	...
	do {
		/* Get the next chunk */
		byteCount = (buflen < CHUNK_SIZE * 4) ? buflen : CHUNK_SIZE * 4;
		for (i = OUTPUT_BLOCK_SIZE; i < INPUT_BLOCK_SIZE; i++)
			hash[i] = 0;
#ifdef LITTLE_ENDIAN
		for (i = 0; i < byteCount; i++) {
			hash[OUTPUT_BLOCK_SIZE + i / 4]
				|= (word32) (*(in + i)) << ((3 - (i % 4)) * 8);
		}
#else
		memcpy((unsigned char *) &hash[OUTPUT_BLOCK_SIZE], (unsigned char *) in, byteCount);
#endif
		buflen -= byteCount;
		in += byteCount;
...
void initrandom()
{
	...
	if (!initialised) {
		...
		/* there are some reasonable cases where we might not
           be able to open a seed file -- so just don't */
		if (sfile = fopen(sname, "r")) {
			j = fread(&rbuf[0], 1, 16, sfile);
			fclose(sfile);
		}
		else {
			unlink(sname);
...
unsigned char randombyte()
{
	...
	static int used = 0;
	if (!initialised) {
		fprintf(stderr, "fatal error: randombyte called before initrandom\n");
		exit(1);
	}
	if (firstcall) {	/* we've got stuff to do */
		...
		des2key(b1, 0);
		sn512string(rbuf, j, h);
		memcpy(b2, h, 16);
		memcpy(b5, &h[16], 16);

		zap(h, 32);	/* no reason to leave this lying around */

		/* our state consists of b1 and b2 and b5,
         * b3 will be output */
...
void closerandom()
{
	...
	if (initialised) {
		unlink(sname);
		if (sfile = fopen(sname, "w")) {
			for (i = 0; i < 48; i++)
				(void) randombyte();
			for (i = 0; i < 64; i++)
				sbuf[i] = randombyte();
			MD5Init(&xyzzy);
			MD5Update(&xyzzy, &sbuf[0], 64);
			MD5Final(&xyzzy);
			fwrite(xyzzy.digest, 16, 1, sfile);
			zap(sbuf, 64);
			zap(xyzzy.digest, 16);
			fclose(sfile);
			chmod(sname, S_IRUSR);
