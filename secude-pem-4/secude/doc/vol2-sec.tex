...
\parname  {{\em more}}
\pardescript  {SEC\_MORE: More data will follow for this hash process. \\
                  SEC\_END: Last data of this hash process.} 
...
\nm{3X}{sec\_DH\_*}{Diffie-Hellman Key Agreement}
\label{sec_DH}
\hl{Synopsis}
\#include $<$secure.h$>$ \\ [0.5cm]
...
\parname  {{\em size\_of\_p}}
\pardescript  {Size of the common prime modulus $p$ in octets.} 

\parname  {{\em privat\_value\_length}}
\pardescript  {Size of the privat values $x$ which should be used with this $p$ and $g$ in bits (or 0 for no restrictions).} 

\parname  {{\em Return Value}}
\pardescript  {An algorithm identifier containing the DH Parameter}
...
\parname  {{\em x}}
\pardescript  {Reference to the key to store the privat value $x$.} 

\parname  {{\em with\_pg}}
\pardescript  {Flag which indicates that additionally the DHParameter should be stored in the returned KeyInfo.} 

\parname  {{\em Return Value}}
\pardescript  {The public value $y = g^x\ ({\rm mod\ }p)$ and optionally the DHParameter.}
...
\parname  {{\em x}}
\pardescript  {Reference to the key which holds the privat value $x$.} 

\parname  {{\em peer\_y}}
\pardescript  {The public value $y$ of the peer.} 

\parname  {{\em Return Value}}
\pardescript  {The common secret key.}
...
All three routines expect $p$ and $g$ to be in the parameter component
...
\nm{3X}{sec\_random}{Generate random octetstring, bitstring, characterstring, integer, long, unsigned}
\label{random}
\hl{Synopsis}
\#include $<$secure.h$>$ \\ [0.5cm]
...
OctetString *{\bf d\_INTEGER} {\em (octetstring)} \\
OctetString *{\em octetstring};
...
(See also functions aux\_int2OctetString and aux\_OctetString2int, which transform the 
