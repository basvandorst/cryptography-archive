...
static char (*iperm1)[16][8];	/* Initial and final permutations */
static char (*fperm1)[16][8];

/* 8 6-bit subkeys for each of 16 rounds, initialized by setkey_() */
static unsigned char (*kn)[8];
static unsigned char (*kn1)[8];
...

{
	...
	if(des3) kn1 = (unsigned char (*)[8])malloc(sizeof(char) * 8 * 16);
	if(kn == NULL){
		free((char *)sp);
		return -1;
	}
	...
	if(des3) iperm1 = (char (*)[16][8])malloc(sizeof(char) * 16 * 16 * 8);
	if(iperm == NULL){
		free((char *)sp);
		free((char *)kn);
		if(des3) free((char *)kn1);
		return -1;
	}
	if(des3) if(iperm1 == NULL){
		free((char *)sp);
		free((char *)kn);
		free((char *)iperm);
		if(des3) free((char *)kn1);
	...
	perminit(iperm,ip);
	if(des3) perminit(iperm1,ip);

	fperm = (char (*)[16][8])malloc(sizeof(char) * 16 * 16 * 8);
	if(des3) fperm1 = (char (*)[16][8])malloc(sizeof(char) * 16 * 16 * 8);
	if(fperm == NULL){
		free((char *)sp);
		free((char *)kn);
		free((char *)iperm);
...

{
	...
	if(des3) {
		free((char *)kn1);
		kn1 = NULL;
		if(iperm1 != NULL) free((char *)iperm1);
		if(fperm1 != NULL) free((char *)fperm1);
		iperm1 = NULL;
		fperm1 = NULL;
	}
...
/* Set key (middle key) (initialize key schedule array) */
/***************************************************************
 *
 * Procedure setkey1
 *
...

{
	...
	for (i=0; i<16; i++) {		/* key chunk for each iteration */
		for (j=0; j<56; j++)	/* rotate pc1 the right amount */
			...
			/* rotate left and right halves independently */
		for (j=0; j<48; j++){	/* select bits individually */
			/* check bit that goes to kn1[j] */
			if (pcr[pc2[j]-1]){
				/* mask it in if it's there */
				l= j % 6;
				kn1[i][j/6] |= bytebit[l] >> 2;
			}
		}
	}
...

{
	...
	permute(block,iperm1,(char *)work);	/* Initial Permutation */
#ifdef LITTLE_ENDIAN
	work[0] = byteswap(work[0]);
	work[1] = byteswap(work[1]);
#endif
	...
	work[1] = byteswap(work[1]);
#endif
	permute((char *)work,fperm1,block);	/* Inverse initial permutation */
...

{
	 ...
	 */
	if(num & 1){
		block[1] ^= f(block[0],kn1[num]);
	} else {
		block[0] ^= f(block[1],kn1[num]);
	}
