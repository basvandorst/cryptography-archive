...
extern char sec_verbose, sec_gen_verbose;
...
#else
static char Cycle[5] = "-\\|/";
#define	PrintCycle(i)	{ if(sec_verbose || sec_gen_verbose) {fprintf(stderr,"%c\b",Cycle[i%4]); fflush(stderr);} }
/*
...
#define PrintRabinstest { if(sec_verbose || sec_gen_verbose) {fprintf(stderr,"\b\b\b%d: ",RSAgenCountDown--); fflush(stderr);} }
#define PrintRabinsCount(rt)	PrintCycleCnt(rt)
#define PrintGenRSA(string)   /* NOP */
#define PrintL_NUMBER(x, txt)    /* NOP */
#define PrintNote(string)     { if(sec_verbose || sec_gen_verbose) {fprintf(stderr,string); fflush(stderr);} }
#define PrintSTART(s,c) { if(sec_verbose || sec_gen_verbose) {PrintNote(s); RSAgenCountDown = c;} }
#endif
