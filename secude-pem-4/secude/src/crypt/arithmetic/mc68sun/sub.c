...

{
	...
	if (lngofln(Ap)<lngofln(Bp)) ALU_exception(carry);
	
	cnt = lngofln(Bp); residual = lngofln(Ap) - lngofln(Bp);
	...
	if(Diff != Ap)  for(  ; residual>0 ; residual-- )  *++Diff = *++Ap;

	if (carry) /* OK */;
	else	ALU_exception(carry);
	
	cnt = lngofln(opA);
	Diff = Dp + cnt;
	for (; (Diff>Dp) && !*Diff; Diff--) cnt--;
