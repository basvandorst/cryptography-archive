...
#ifndef	lint
static char *rcsid = "$Header: /home/isode-consortium/isode/src/lib/asn1/pepsy2/RCS/dec.c,v 10.0 1993/02/18 20:15:18 awy Rel $";
#endif

/* 
 * $Header: /home/isode-consortium/isode/src/lib/asn1/pepsy2/RCS/dec.c,v 10.0 1993/02/18 20:15:18 awy Rel $
 *
 *
 * $Log: dec.c,v $
 * Revision 10.0  1993/02/18  20:15:18  awy
 * Release IC-R1
 *
 */
...
char	**parm;
{
...
#ifdef DEBUG
    DLOG (psap_log, LLOG_DEBUG, ("Decoding %s.%s",
				 mod->md_name, pname(p)));
#endif
	...
	*parm = NULLCP;	/* initialise this for the MALLOCs that follow */
	...
	goto bad;
#ifdef DEBUG
    DLOG (psap_log, LLOG_DEBUG, ("Decoding %s.%s suceeded",
				 mod -> md_name, pname(p)));
#endif
    return (OK);
bad:
#ifdef DEBUG
    DLOG (psap_log, LLOG_DEBUG, ("Decoding %s.%s failed",
				 mod->md_name, pname(p)));
#endif
    return (NOTOK);
}
...
modtyp	*mod;			/* Module it is from */
{
...
#ifdef DEBUG
    DLOG (psap_log, LLOG_DEBUG, ("Decode type %s.%s type %s",
				 mod -> md_name, pname(p),
				 pr_petype(p->pe_type)));
#endif
	...
	case ETAG:
		...
		if (pe -> pe_form != PE_FORM_CONS)
		    return pepsylose (mod, p, pe, "ASN.1 Constructor expected");
	...
	case INTEGER:
	    if (pe != NULLPE) {
#ifdef DEBUG
		DLOG (psap_log, LLOG_DEBUG, ("Decode INTEGER %s.%s -> %d",
					     mod -> md_name, pname(p),
					     prim2num(pe)));
#endif
	...
	case REALTYPE:
	    if (pe != NULLPE) {
#ifdef DEBUG
		DLOG (psap_log, LLOG_DEBUG, ("Decode REAL %s.%s -> %g",
					     mod -> md_name, pname(p),
					     prim2real(pe)));
#endif
...
modtyp	*mod;			/* Module it is from */
{
...
#ifdef DEBUG
    DLOG (psap_log, LLOG_DEBUG, ("Decode SEQUENCE %s.%s type %s",
				 mod -> md_name, pname(p),
				 pr_petype(p->pe_type)));
#endif
    p++;
    ...
    while (p->pe_type != PE_END) {
#ifdef DEBUG
	DLOG (psap_log, LLOG_DEBUG, ("Decode SEQUENCE member %s.%s type %s",
				     mod -> md_name, pname(p),
				     pr_petype(p->pe_type)));
#endif
...
modtyp	*mod;			/* Module it is from */
{
...
#ifdef DEBUG
    DLOG (psap_log, LLOG_DEBUG, ("Decode SET %s.%s type %s",
				 mod -> md_name, pname(p),
				 pr_petype(p->pe_type)));
#endif
    if (head == NULLPE)
    ...
    while (p->pe_type != PE_END) {
#ifdef DEBUG
	DLOG (psap_log, LLOG_DEBUG, ("Decode SET member %s.%s type %s",
				     mod -> md_name, pname(p),
				     pr_petype(p->pe_type)));
#endif
	...
	case BOPTIONAL:
	    ...
	    if (pe == NULLPE && head -> pe_errno != PE_ERR_NONE)
...
#ifdef DEBUG
    DLOG (psap_log, LLOG_DEBUG, ("Decode SEQUENCE OF %s.%s type %s",
				 mod -> md_name, pname(p),
				 pr_petype(p->pe_type)));
#endif
    if (head == NULLPE)
...
#ifdef DEBUG
    DLOG (psap_log, LLOG_DEBUG, ("Decode SET OF %s.%s type %s",
				 mod -> md_name, pname(p),
				 pr_petype(p->pe_type)));
#endif
    if (head == NULLPE)
...
#ifdef DEBUG
    DLOG (psap_log, LLOG_DEBUG, ("Decode CHOICE %s.%s type %s",
				 mod -> md_name, pname(p),
				 pr_petype(p->pe_type)));
#endif
    if (head == NULLPE)
...
#ifdef DEBUG
    DLOG (psap_log, LLOG_DEBUG, ("Decode EXPLICIT TAG %s.%s type %s",
				 mod -> md_name, pname(p),
				 pr_petype(p->pe_type)));
#endif
...
modtyp *mod;
{
    ...
    default:
        if (head -> pe_form == PE_FORM_PRIM) {
                (void) ppepsylose (mod, p, head, 
                                   "ASN.1 type is not a constructor");
                head -> pe_errno = PE_ERR_CONS;
                return NULLPE;
        }
