 ...
 */
char *
strcatrealloc(target,source)
char **target;
char *source;
{
	*target = (char *) realloc(*target,strlen(source)+
    strlen(*target)+1);
	if(*target) {
...
char *candidate;
{
	...
	strcpyalloc(&possible,candidate);  /* possible is a copy of candidate */
	 ...
	 */
	do {
		if(match(user,possible)) {
			trying = FALSE;
			matchOK = TRUE;
		} else {
			trying = EmailAddrUpALevel(possible);
		}
	} while(trying); 
 ...
 *
 *  Zero out a buffer.	Generally used to erase sensitive information,
 *  like passwords, so they won't appear in coredumps or whatever.
 */
void
ClearBuffer(buf,nbytes)
void *buf;
int nbytes;
{
	R_memset((unsigned char *)buf,0,nbytes);
}
