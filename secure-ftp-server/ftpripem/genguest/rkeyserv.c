...
#define BUFSIZE 8192
   ...
   while(-1 != (ch = getopt(argc,argv,"D:g:p:fP:Z:t:"))) {
	...
	pid_fd = open(pid_file,O_CREAT | O_RDWR,0700);
	if(pid_fd < 0) {
   ...
   if(err_msg = OpenKeyDatabase(DBName,FALSE,&dbf)) {
	...
	while(running) {
      ...
      selval = select(sel_width,&readfds,NULL,NULL,&timeout);
			   ...
			   exit(1);
				...
				if(Debug) {
				   ...
				   if(!fast) {
					...
					fprintf(DStream,"\nOn %-2.2d/%-2.2d at %-2.2d:%-2.2d:%-2.2d received request from %s\n",
