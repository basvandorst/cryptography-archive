...
//...........................................................................
    ...
    /**
     * Debug diagnostics. Value of property key "Debug.Level.LOKI97_ECB" in
     * the LOKI97.properties file located in the CLASSPATH. <p>
     *
...
System.out.println("Raw="+toString(k));
        ...
        k3 = (k[i++] & 0xFFL) << 56 | (k[i++] & 0xFFL) << 48 |
             ...
             (k[i++] & 0xFFL) << 24 | (k[i++] & 0xFFL) << 16 |
             (k[i++] & 0xFFL) <<  8 | (k[i++] & 0xFFL);
        if (k.length == 16) {   // 128-bit key
            k2 = f(k3, k4);
            k1 = f(k4, k3);
        } else {                // 192 or 256-bit key
            k2 = (k[i++] & 0xFFL) << 56 | (k[i++] & 0xFFL) << 48 |
                 (k[i++] & 0xFFL) << 40 | (k[i++] & 0xFFL) << 32 |
                 (k[i++] & 0xFFL) << 24 | (k[i++] & 0xFFL) << 16 |
                 (k[i++] & 0xFFL) <<  8 | (k[i++] & 0xFFL);
            if (k.length == 24) // 192-bit key
                k1 = f(k4, k3);
            else                // 256-bit key
...
if (DEBUG && debuglevel > 0) debug("makeKey("+ longToString(k4)+","+longToString(k3)+","+longToString(k2)+","+longToString(k1)+")");
        ...
        // iterate over all rounds to generate the subkeys
...
if (DEBUG && debuglevel > 6) System.out.println("PT="+longToString(L)+" "+longToString(R));
        long nR, f_out;
        int k = 0;
        for (int i = 0; i < ROUNDS; i++) { // compute all rounds in 1 block
            nR = R + SK[k++];
            f_out = f(nR, SK[k++]);
            nR += SK[k++];
            R = L ^ f_out;
