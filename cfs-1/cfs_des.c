 ...
 */
block_cipher(short_key, text, decrypting)
char short_key[8];
char text[8];
int decrypting;
{
	...
	if (k<0) {
		...
		key_setup(short_key, expanded_key[k]);
	}
	key = expanded_key[k];
	left = ip_low(text);
	right = ip_high(text);
	...
	for (i = 0; i < 16; i++) {
		...
		left ^= s0p[(temp         & 0x3f) ^ key[0]]
		      ^ s1p[((temp >>  4) & 0x3f) ^ key[1]]
		      ^ s2p[((temp >>  8) & 0x3f) ^ key[2]]
		      ^ s3p[((temp >> 12) & 0x3f) ^ key[3]]
		      ^ s4p[((temp >> 16) & 0x3f) ^ key[4]]
		      ^ s5p[((temp >> 20) & 0x3f) ^ key[5]]
		      ^ s6p[((temp >> 24) & 0x3f) ^ key[6]];
