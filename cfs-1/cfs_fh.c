     ...
     char *vect;  /* perturbation vector */
{
	...
	bcopy(buf+fronterr,blk,userbytes);
	return(userbytes);
}
     ...
     fhdata *h;	/* note recasting here - may give trouble with strict ansi */
{
	...
	for (i=0; i<8; i++)
		if (h->magic[i] != magictest[i]) {
			...
			bcopy(h,&roothandle,sizeof(roothandle));
			return &rootnode;
		}
...
fillinh(h,iid,ino,check)
     ...
     char *check;
{
	bzero(h,sizeof(fhdata));
	bcopy(magictest,h->magic,sizeof(magictest));
	bcopy(check,h->check,8);
	h->htype=H_REG;
	h->instance=iid;
	h->fileid=ino;
}
     ...
     nfs_fh *h;
{
	...
	if (!strcmp(n,".") || !strcmp(n,"..")) {	/* self */
		bcopy(&roothandle,h,sizeof(roothandle));
		return 0;
	}
