 ...
 */
char *
GetPublicKey(user,source,key,found)
char *user;
TypKeySource *source;
	...
	switch(source->origin[sour]) {
		...
		case KEY_FROM_FILE:
		   ...
		   if(!fptr->stream) {
			...
			err_msg = GetUserRecordFromFile(user,fptr,RECSIZE,rec,found);
		...
		case KEY_FROM_SERVER:
			if(server_ok) {
				err_msg = GetUserRecordFromServer(user,source,
				 rec,RECSIZE,&server_ok,found);
				goto decodekey;
			}
				
		/* Get key from finger. */
		case KEY_FROM_FINGER:
			err_msg = GetUserRecordFromFinger(user,rec,RECSIZE,found);
	decodekey:;
			if(Debug) {
				if(err_msg) {
					fprintf(DebugStream,
	...
	if(*found) {
		...
		if(DERToPubKey(bytes,key)) {
		   /* Conversion didn't work. */
			sprintf(ErrMsgTxt,"Error parsing public key for %s.",user);
			err_msg = ErrMsgTxt;
		} else {
			...
			if(CrackKeyField(rec,PUBLIC_KEY_DIGEST_FIELD,
			  records_hex_digest,MAXDIGESTSIZE)) {
				...
				if(strcmp(computed_hex_digest,records_hex_digest)) {
					sprintf(ErrMsgTxt,
					"Public key of '%s' is garbled--digest does not match.",user);
					err_msg = ErrMsgTxt;
				}
			} else {
 ...
 */
char *
GetPublicKeyList(userList,pubKeySource)
TypList *userList;
TypKeySource *pubKeySource;
{
	...
	for(entry_ptr = userList->firstptr; entry_ptr;
		...
		err_msg = GetPublicKey(recip_ptr->emailaddr,pubKeySource,
		 &(recip_ptr->pubkey),&(recip_ptr->gotpubkey));	
 ...
 */
char *
GetUserRecordFromFile(user,fileptr,maxBytes,userRec,found)
char *user;
TypFile *fileptr;
