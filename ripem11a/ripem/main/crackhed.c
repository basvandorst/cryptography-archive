 ...
 */
char *
CrackHeader(stream,prependHeaders,headerList,userList,msgInfo)
FILE *stream;
BOOL prependHeaders;
TypList *userList;
TypList *headerList;
      ...
      switch(state) {
         ...
         case ST_Begin_Field:
		   ...
		   case ST_Cont_Field:
               ...
               line_prev = line;
			      state = ST_Begin_Field;
               if (cptr = DoHeaderLine(ext_line,userList,msgInfo))
					return(cptr);
 ...
 */
char *
DoHeaderLine(ext_line,userList,msgInfo)
char *ext_line;
TypList *userList;
TypMsgInfo *msgInfo;
{
   ...
   switch(tok_field) {
	   case PROC_TYPE_ENUM:
		   ...
		   if(tok_vals[0] != PROC_TYPE_RIPEM_ID_ENUM) {
			   sprintf(ErrMsgTxt,"RIPEM processes only Proc-Type %s.",
