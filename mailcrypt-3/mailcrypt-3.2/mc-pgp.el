...
(defconst mc-pgp-newkey-re "^[ \t]*\\(No\\|[0-9]+\\) +new.*"
  ...
  (let ((process-environment process-environment)
    ...
    (save-excursion
	  ...
	  (progn
	    ...
	    (setq pgp-id (buffer-substring (match-beginning 1) (match-end 1)))
	    (and
	     (y-or-n-p
...
(defun mc-pgp-decrypt-region (start end &optional id)
  ...
  (let ((process-environment process-environment)
    ...
    (if passwd
	(setenv "PGPPASSFD" "0"))
    (setq args '("+verbose=1" "+batchmode" "-f"))
    (if mc-pgp-alternate-keyring
	(setq args (append args (list (format "+pubring=%s"
					      mc-pgp-alternate-keyring)))))
    (message "Decrypting...")
	...
	(if (and
	     ...
	     (setq pgp-id (concat "0x" (buffer-substring (match-beginning 1)
							 (match-end 1))))
	     (y-or-n-p
...
(defun mc-pgp-insert-public-key (&optional id)
  (let ((buffer (get-buffer-create mc-buffer-name))
    ...
    (setq args (list "+verbose=1" "+batchmode" "-kxaf" id))
    (if mc-pgp-comment
	(setq args (cons (format "+comment=%s" mc-pgp-comment) args)))
    (if mc-pgp-alternate-keyring
	(setq args (append args (list (format "+pubring=%s"
...
(defun mc-pgp-snarf-keys (start end)
  ...
  (let ((buffer (get-buffer-create mc-buffer-name)) tmpstr args)
    (setq args '("+verbose=1" "+batchmode" "-kaf"))
    (if mc-pgp-alternate-keyring
	(setq args (append args (list (format "+pubring=%s"
					      mc-pgp-alternate-keyring)))))
    (message "Snarfing...")
...
(defvar mc-pgp-keyserver-address "www-swiss.ai.mit.edu"
...
request for the key."
  ...
  (let ((methods mc-pgp-fetch-methods)
	(process-connection-type nil) key proc buf win args)
    (if (null id)
