...
static unsigned long Kn4[32] = { 0L };
...
void cleanup_des()  /* zap sensitive info */
{
	bzero(&KnR[0],sizeof(unsigned long)*32);
	bzero(&KnL[0],sizeof(unsigned long)*32);
	bzero(&Kn3[0],sizeof(unsigned long)*32);
	bzero(&Kn4[0],sizeof(unsigned long)*32);
}
...
void des4key(hexkey)  /* MODE IS ASSUMED TO BE ENCRYPT -- for use with CFB */
unsigned char *hexkey;			/* unsigned char[32] */
{
	unsigned char *first, *third, *fourth;
	int revmod;
	int mode;
