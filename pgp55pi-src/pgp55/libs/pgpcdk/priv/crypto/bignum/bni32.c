/*
 * bni32.c - Low-level bignum routines, 32-bit version.
 *
 * WARNING: This file was automatically generated from bni16.c -- DO NOT EDIT!
 *
 * Written by Colin Plumb.
 *
 * $Id: bni32.c,v 1.4 1997/05/13 01:18:43 mhw Exp $
 *
 ...
 *
 * If you want to write asm routines, start with bniMulAdd1_32().
 * This is the workhorse of modular exponentiation.  bniMulN1_32() is
 * also used a fair bit, although not as much and it's defined in terms
 * of bniMulAdd1_32 if that has a custom version.  bniMulSub1_32 and
 * bniDiv21_32 are used in the usual division and remainder finding.
 * (Not the Montgomery reduction used in modular exponentiation, though.)
 * Once you have bniMulAdd1_32 defined, writing the other two should
 * be pretty easy.  (Just make sure you get the sign of the subtraction
 * in bniMulSub1_32 right - it's dest = dest - source * k.)
 *
 * The only definitions that absolutely need a double-word (BNWORD64)
 * type are bniMulAdd1_32 and bniMulSub1_32; if those are provided,
 * the rest follows.  bniDiv21_32, however, is a lot slower unless you
 * have them, and bniModQ_32 takes after it.  That one is used quite a
...
volatile const char * volatile const bniCopyright_32 = bnCopyright;
 ...
 */
int
bniExpMod_32(BNWORD32 *result, BNWORD32 const *n, unsigned nlen,
	BNWORD32 const *e, unsigned elen, BNWORD32 *mod, unsigned mlen)
{
	...
	elen = bniNorm_32(e, elen);
	if (!elen) {
		/* x ^ 0 == 1 */
		bniZero_32(result, mlen);
		BIGLITTLE(result[-1],result[0]) = 1;
		return 0;
	}
	ebits = bniBits_32(e, elen);
	if (ebits == 1) {
		/* x ^ 1 == x */
		if (n != result)
			bniCopy_32(result, n, nlen);
		if (mlen > nlen)
			bniZero_32(BIGLITTLE(result-nlen,result+nlen),
			           mlen-nlen);
		return 0;
	}
	...
	BNIALLOC(a, BNWORD32, 2*mlen);
	if (!a)
		return -1;
	BNIALLOC(b, BNWORD32, 2*mlen);
	if (!b) {
		BNIFREE(a, 2*mlen);
		return -1;
	}
	...
	inv = bniMontInv1_32(mod[BIGLITTLE(-1,0)]);	/* LSW of modulus */
	...
	(void)bniDiv_32(t, a, mlen+nlen, mod, mlen);
	/* Copy into first table entry */
	bniCopy_32(table[0], a, mlen);

	/* Square a into b */
	bniMontSquare_32(b, a, mod, mlen, inv);

	/* Use high half of b to initialize the table */
	t = BIGLITTLE(b-mlen, b+mlen);
	for (i = 1; i < tblmask; i++) {
		bniMontMul_32(a, t, table[i-1], mod, mlen, inv);
		bniCopy_32(table[i], BIGLITTLE(a-mlen, a+mlen), mlen);
#if BNYIELD
	...
	for (;;) {
		...
		if (ebits == multpos) {
			...
			if (isone) {
				...
				bniCopy_32(t, mult, mlen);
				isone = 0;
			} else {
				bniMontMul_32(a, t, mult, mod, mlen, inv);
				/* Swap a and b */
				t = a; a = b; b = t;
			}
		}
		...
		if (!isone) {
			t = BIGLITTLE(b-mlen, b+mlen);
			bniMontSquare_32(a, t, mod, mlen, inv);
			/* Swap a and b */
			t = a; a = b; b = t;
		}
#if BNYIELD
	...
	bniCopy_32(b, t, mlen);
	bniZero_32(t, mlen);
	bniMontReduce_32(b, mod, mlen, inv);
	bniCopy_32(result, t, mlen);
	/*
 ...
 */
int
bniDoubleExpMod_32(BNWORD32 *result,
                   ...
                   BNWORD32 *mod, unsigned mlen)
{
	...
	/* First, a couple of trivial cases. */
	e1len = bniNorm_32(e1, e1len);
	e2len = bniNorm_32(e2, e2len);

	/* Ensure that the first exponent is the longer */
	e1bits = bniBits_32(e1, e1len);
	e2bits = bniBits_32(e2, e2len);
	if (e1bits < e2bits) {
	...
	if (!e2len)
		return bniExpMod_32(result, n1, n1len, e1, e1len, mod, mlen);
	pgpAssert(e2bits);
	...
	if (e1bits == 1) {
		...
		BNIALLOC(a, BNWORD32, n1len+n2len);
		if (!a)
			return -1;

		bniMul_32(a, n1, n1len, n2, n2len);
		/* Do a direct modular reduction */
		if (n1len + n2len >= mlen)
			(void)bniDiv_32(a+mlen, a, n1len+n2len, mod, mlen);
		bniCopy_32(result, a, mlen);
	...
	t = BIGLITTLE(a-mlen, a+mlen);
	bniCopy_32(t, n1, n1len);
	bniZero_32(a, mlen);
	/* Do the division - lose the quotient into the high-order words */
	(void)bniDiv_32(t, a, mlen+n1len, mod, mlen);
	/* Copy into first table entry */
	bniCopy_32(table1[0], a, mlen);
	...
	t = BIGLITTLE(b-mlen, b+mlen);
	for (i = 1; i < tblmask; i++) {
		bniMontMul_32(a, t, table1[i-1], mod, mlen, inv);
		bniCopy_32(table1[i], BIGLITTLE(a-mlen, a+mlen), mlen);
#if BNYIELD
		if (bnYield && (y = bnYield()) < 0)
			goto yield;
#endif
	...
	bniCopy_32(t, n2, n2len);
	bniZero_32(a, mlen);
	/* Do the division - lose the quotient into the high-order words */
	(void)bniDiv_32(t, a, mlen+n2len, mod, mlen);
	/* Copy into first table entry */
	bniCopy_32(table2[0], a, mlen);

	/* Square it into a */
	bniMontSquare_32(a, table2[0], mod, mlen, inv);
	/* Copy to b, low half */
	bniCopy_32(b, t, mlen);

	/* Use b to initialize the second table */
	for (i = 1; i < buf2; i++) {
		bniMontMul_32(a, b, table2[i-1], mod, mlen, inv);
		bniCopy_32(table2[i], t, mlen);
#if BNYIELD
		if (bnYield && (y = bnYield()) < 0)
			goto yield;
#endif
	...
	if (buf2 & tblmask) {
		...
		if (mult2pos == e1bits) {
			t = BIGLITTLE(b-mlen, b+mlen);
			if (isone) {
				bniCopy_32(t, b, mlen);	/* Copy low to high */
				isone = 0;
			} else {
				bniMontMul_32(a, t, b, mod, mlen, inv);
				t = a; a = b; b = t;
			}
		}
	}
	...
	for (;;) {
		...
		if (e1bits == mult1pos) {
			...
			if (isone) {
				/* Multiply by 1 is a trivial case */
				bniCopy_32(t, mult1, mlen);
				isone = 0;
			} else {
				bniMontMul_32(a, t, mult1, mod, mlen, inv);
				/* Swap a and b */
				t = a; a = b; b = t;
			}
		}
		...
		if (e1bits == mult2pos) {
			...
			if (isone) {
				/* Multiply by 1 is a trivial case */
				bniCopy_32(t, mult2, mlen);
				isone = 0;
			} else {
				bniMontMul_32(a, t, mult2, mod, mlen, inv);
				/* Swap a and b */
				t = a; a = b; b = t;
			}
		}
 ...
 */
int
bniTwoExpMod_32(BNWORD32 *n, BNWORD32 const *exp, unsigned elen,
	BNWORD32 *mod, unsigned mlen)
{
	...
	for (;;) {
		...
		if (bitpos & bitword) {
			 ...
			 * the modulus.
			 */
			if (bniDouble_32(a1, mlen) ||
			    bniCmp_32(a1, mod, mlen) > 0)
				(void)bniSubN_32(a1, mod, mlen);
		}
		 ...
		 */
		bniMontSquare_32(b, a1, mod, mlen, inv);
	 ...
	 */
	bniCopy_32(a, a1, mlen);
	bniZero_32(a1, mlen);
	bniMontReduce_32(a, mod, mlen, inv);
	bniCopy_32(n, a1, mlen);
 ...
 */
void
bniExtractBigBytes_32(BNWORD32 const *n, unsigned char *buf,
 ...
 */
void
bniInsertBigBytes_32(BNWORD32 *n, unsigned char const *buf,
                  unsigned lsbyte,  unsigned buflen)
{
 ...
 */
void
bniExtractLittleBytes_32(BNWORD32 const *n, unsigned char *buf,
	unsigned lsbyte, unsigned buflen)
{
	BNWORD32 t = 0;	/* Needed to shut up uninitialized var warnings */
 ...
 */
void
bniInsertLittleBytes_32(BNWORD32 *n, unsigned char const *buf,
                  unsigned lsbyte,  unsigned buflen)
{
	BNWORD32 t = 0;	/* Shut up uninitialized varibale warnings */
 ...
 */
unsigned
bniFromBytes_32(BNWORD32 *a, unsigned char const *b, unsigned blen)
{
	BNWORD32 t;
 ...
 */
int
bniGcd_32(BNWORD32 *a, unsigned alen, BNWORD32 *b, unsigned blen,
	unsigned *rlen)
{
#if BNYIELD
	int y;
	...
	while (blen != 0) {
		(void)bniDiv_32(BIGLITTLE(a-blen,a+blen), a, alen, b, blen);
		alen = bniNorm_32(a, blen);
		if (alen == 0) {
			*rlen = blen;
			return 1;
		}
		(void)bniDiv_32(BIGLITTLE(b-alen,b+alen), b, blen, a, alen);
		blen = bniNorm_32(b, alen);
#if BNYIELD
		if (bnYield && (y = bnYield()) < 0)
			return y;
#endif
 ...
 */
int
bniInv_32(BNWORD32 *a, unsigned alen, BNWORD32 const *mod, unsigned mlen)
{
	...
	alen = bniNorm_32(a, alen);
	if (!alen)
		return 1;	/* No inverse */

	mlen = bniNorm_32(mod, mlen);

	pgpAssert (alen <= mlen);

	/* Inverse of 1 is 1 */
	if (alen == 1 && BIGLITTLE(a[-1],a[0]) == 1) {
		bniZero_32(BIGLITTLE(a-alen,a+alen), mlen-alen);
		return 0;
	}

	/* Allocate a pile of space */
	BNIALLOC(b, BNWORD32, mlen+1);
	if (b) {
		/*
		 ...
		 */
		BNIALLOC(p, BNWORD32, mlen+1);
		if (p) {
			BNIALLOC(t0, BNWORD32, mlen);
			if (t0) {
				BNIALLOC(t1, BNWORD32, mlen);
				if (t1)
						goto allocated;
				BNIFREE(t0, mlen);
			}
...
allocated:
	...
	/* b = mod */
	bniCopy_32(b, mod, mlen);
	/* blen = mlen (implicitly) */
	
	/* t1 = b / a; b = b % a */
	cy = bniDiv_32(t1, b, mlen, a, alen);
	*(BIGLITTLE(t1-(mlen-alen)-1,t1+(mlen-alen))) = cy;
	t1len = bniNorm_32(t1, mlen-alen+1);
	blen = bniNorm_32(b, alen);

	/* while (b > 1) */
	while (blen > 1 || BIGLITTLE(b[-1],b[0]) != (BNWORD32)1) {
		/* q = a / b; a = a % b; */
		if (alen < blen || (alen == blen && bniCmp_32(a, a, alen) < 0))
			pgpAssert(0);
		cy = bniDiv_32(BIGLITTLE(a-blen,a+blen), a, alen, b, blen);
		*(BIGLITTLE(a-alen-1,a+alen)) = cy;
		plen = bniNorm_32(BIGLITTLE(a-blen,a+blen), alen-blen+1);
		pgpAssert(plen);
		alen = bniNorm_32(a, blen);
		if (!alen)
		...
		bniMul_32(p, BIGLITTLE(a-blen,a+blen), plen, t1, t1len);
		plen = bniNorm_32(p, plen + t1len);
		pgpAssert(plen <= mlen);
		if (plen > t0len) {
			bniZero_32(BIGLITTLE(t0-t0len,t0+t0len), plen-t0len);
			t0len = plen;
		}
		cy = bniAddN_32(t0, p, plen);
		if (cy) {
			if (t0len > plen) {
				cy = bniAdd1_32(BIGLITTLE(t0-plen,t0+plen),
						t0len-plen, cy);
			}
			if (cy) {
		...
		if (alen <= 1 && BIGLITTLE(a[-1],a[0]) == (BNWORD32)1) {
			...
			bniCopy_32(a, t0, t0len);
			bniZero_32(BIGLITTLE(a-t0len, a+t0len), mlen-t0len);
			goto success;
		}

		/* q = b / a; b = b % a; */
		if (blen < alen || (blen == alen && bniCmp_32(b, a, alen) < 0))
			pgpAssert(0);
		cy = bniDiv_32(BIGLITTLE(b-alen,b+alen), b, blen, a, alen);
		*(BIGLITTLE(b-blen-1,b+blen)) = cy;
		plen = bniNorm_32(BIGLITTLE(b-alen,b+alen), blen-alen+1);
		pgpAssert(plen);
		blen = bniNorm_32(b, alen);
		if (!blen)
			goto failure;	/* GCD not 1 */
		...
		bniMul_32(p, BIGLITTLE(b-alen,b+alen), plen, t0, t0len);
		plen = bniNorm_32(p, plen + t0len);
		pgpAssert(plen <= mlen);
		if (plen > t1len) {
			bniZero_32(BIGLITTLE(t1-t1len,t1+t1len), plen-t1len);
			t1len = plen;
		}
		cy = bniAddN_32(t1, p, plen);
		if (cy) {
			if (t1len > plen) {
				cy = bniAdd1_32(BIGLITTLE(t1-plen,t0+plen),
						t1len-plen, cy);
			}
			if (cy) {
	...
	/* return mod-t1 */
	bniCopy_32(a, mod, mlen);
	pgpAssert(t1len <= mlen);
	cy = bniSubN_32(a, t1, t1len);
	if (cy) {
		pgpAssert(mlen > t1len);
		cy = bniSub1_32(BIGLITTLE(a-t1len, a+t1len), mlen-t1len, cy);
		pgpAssert(!cy);
	}
