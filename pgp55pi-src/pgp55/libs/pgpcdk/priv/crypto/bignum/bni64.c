 ...
 *
 * $Id: bni64.c,v 1.5 1997/10/09 20:56:36 mhw Exp $
 *
	...
	BNWORD64 const *mod, unsigned len, BNWORD64 inv)
{
	...
	while (carry)
		carry -= bniSubN_64(prod, mod, len);
	while (bniCmp_64(prod, mod, len) >= 0)
		(void)bniSubN_64(prod, mod, len);
}
/* Suppress later definition */
#define bniMontMul_64 bniMontMul_64
#endif

#if !defined(bniSquare_64) && defined(BNWORD128) && PRODUCT_SCAN
/*
 * Trial code for product-scanning squaring.  This seems to slow the C
 * code down rather than speed it up.
 */
void
bniSquare_64(BNWORD64 *prod, BNWORD64 const *num, unsigned len)
{
	BNWORD128 x, y, z;
	BNWORD64 const *p1, *p2;
	unsigned carry;
...
/* Suppress later definition */
#define bniSquare_64 bniSquare_64
#endif
...
bniSquare_64(BNWORD64 *prod, BNWORD64 const *num, unsigned len)
{
	...
	while (lenx--) {
		...
		bniMulN1_64(prodx, numx, 1, t);
		BIGLITTLE(--numx,numx++);
		BIGLITTLE(prodx -= 2, prodx += 2);
#endif
	}
	/* Then, shift right 1 bit */
	(void)bniRshift_64(prod, 2*len, 1);
	...
	while (--lenx) {
		...
		t = bniMulAdd1_64(prodx, numx, lenx, t);
		bniAdd1_64(BIGLITTLE(prodx-lenx,prodx+lenx), lenx+1, t);
		BIGLITTLE(--prodx,prodx++);
	}

	/* Shift it back up */
	bniDouble_64(prod, 2*len);
...
/*
 * bniNorm_64 - given a number, return a modified length such that the
 * most significant digit is non-zero.  Zero-length input is okay.
 */
#ifndef bniNorm_64
unsigned
bniNorm_64(BNWORD64 const *num, unsigned len)
{
	BIGLITTLE(num -= len,num += len);
...
/*
 * bniBits_64 - return the number of significant bits in the array.
...
#ifndef bniBits_64
unsigned
bniBits_64(BNWORD64 const *num, unsigned len)
{
	BNWORD64 t;
	unsigned i;

	len = bniNorm_64(num, len);
	if (len) {
...
BNWORD64
bniDiv21_64(BNWORD64 *q, BNWORD64 nh, BNWORD64 nl, BNWORD64 d)
{
...
#ifndef bniDiv1_64
BNWORD64
bniDiv1_64(BNWORD64 *q, BNWORD64 *rem, BNWORD64 const *n, unsigned len,
	BNWORD64 d)
{
	unsigned shift;
	unsigned xlen;
	...
	if (shift) {
		d >>= shift;
		qhigh = (qhigh << shift) | bniLshift_64(q, len-1, shift);
		BIGLITTLE(q[-1],*q) |= r/d;
		r %= d;
	}
	*rem = r;
 ...
 */
#ifndef bniModQ_64
/* If there's a custom bniMod21_64, no normalization needed */
#ifdef bniMod21_64
unsigned
bniModQ_64(BNWORD64 const *n, unsigned len, unsigned d)
{
	unsigned i, shift;
	BNWORD64 r;
	...
	if (r >= d)
		r %= d;
	while (--len)
		r = bniMod21_64(r, BIGLITTLE(*n++,*--n), d);
...
bniModQ_64(BNWORD64 const *n, unsigned len, unsigned d)
{
	...
	while (--len)
		r = bniDiv21_64(&q, r, BIGLITTLE(*n++,*--n), d);
...
#define t2low BIGLITTLE(t2[1],t2[0])
	 ...
	 */
	if (dlen == 1)
		return bniDiv1_64(q, BIGLITTLE(n-1,n), n, nlen,
		                  BIGLITTLE(d[-1],d[0]));
	...
	if (dlen == 2) {
		...
		for (shift = 0; (dh & (BNWORD64)1 << 64-1-shift)) == 0;
								shift++)
			;
		if (shift) {
		}
	...
	nh = 0;
	if (shift) {
		bniLshift_64(d, dlen, shift);
		dh = BIGLITTLE(*(d-dlen),*(d+(dlen-1)));
		nh = bniLshift_64(n, nlen, shift);
	}
 ...
 */
int
bniExpMod_64(BNWORD64 *result, BNWORD64 const *n, unsigned nlen,
	BNWORD64 const *e, unsigned elen, BNWORD64 *mod, unsigned mlen)
{
	...
	elen = bniNorm_64(e, elen);
	if (!elen) {
		/* x ^ 0 == 1 */
		bniZero_64(result, mlen);
		BIGLITTLE(result[-1],result[0]) = 1;
		return 0;
	}
	ebits = bniBits_64(e, elen);
	if (ebits == 1) {
		/* x ^ 1 == x */
		if (n != result)
			bniCopy_64(result, n, nlen);
		if (mlen > nlen)
			bniZero_64(BIGLITTLE(result-nlen,result+nlen),
			           mlen-nlen);
		return 0;
	}
	...
	BNIALLOC(a, BNWORD64, 2*mlen);
	if (!a)
		return -1;
	BNIALLOC(b, BNWORD64, 2*mlen);
	if (!b) {
		BNIFREE(a, 2*mlen);
		return -1;
	}
	...
	while (--i > tblmask)
		BNIFREE(table[i], mlen);

	/* Okay, fill in the table */

	/* Compute the necessary modular inverse */
	inv = bniMontInv1_64(mod[BIGLITTLE(-1,0)]);	/* LSW of modulus */
	...
	(void)bniDiv_64(t, a, mlen+nlen, mod, mlen);
	/* Copy into first table entry */
	bniCopy_64(table[0], a, mlen);

	/* Square a into b */
	bniMontSquare_64(b, a, mod, mlen, inv);

	/* Use high half of b to initialize the table */
	t = BIGLITTLE(b-mlen, b+mlen);
	for (i = 1; i < tblmask; i++) {
		bniMontMul_64(a, t, table[i-1], mod, mlen, inv);
		bniCopy_64(table[i], BIGLITTLE(a-mlen, a+mlen), mlen);
#if BNYIELD
	...
	for (;;) {
		...
		if (ebits == multpos) {
			...
			if (isone) {
				...
				bniCopy_64(t, mult, mlen);
				isone = 0;
			} else {
				bniMontMul_64(a, t, mult, mod, mlen, inv);
				/* Swap a and b */
				t = a; a = b; b = t;
			}
		}
		...
		if (!isone) {
			t = BIGLITTLE(b-mlen, b+mlen);
			bniMontSquare_64(a, t, mod, mlen, inv);
			/* Swap a and b */
			t = a; a = b; b = t;
		}
#if BNYIELD
	...
	bniCopy_64(b, t, mlen);
	bniZero_64(t, mlen);
	bniMontReduce_64(b, mod, mlen, inv);
	bniCopy_64(result, t, mlen);
	/*
 ...
 */
int
bniDoubleExpMod_64(BNWORD64 *result,
                   ...
                   BNWORD64 *mod, unsigned mlen)
{
	...
	pgpAssert(mlen);
	pgpAssert(n1len <= mlen);
	pgpAssert(n2len <= mlen);

	/* First, a couple of trivial cases. */
	e1len = bniNorm_64(e1, e1len);
	e2len = bniNorm_64(e2, e2len);

	/* Ensure that the first exponent is the longer */
	e1bits = bniBits_64(e1, e1len);
	e2bits = bniBits_64(e2, e2len);
	if (e1bits < e2bits) {
	...
	pgpAssert(e1bits >= e2bits);

	/* Handle a trivial case */
	if (!e2len)
		return bniExpMod_64(result, n1, n1len, e1, e1len, mod, mlen);
	pgpAssert(e2bits);
	...
	if (e1bits == 1) {
		...
		BNIALLOC(a, BNWORD64, n1len+n2len);
		if (!a)
			return -1;

		bniMul_64(a, n1, n1len, n2, n2len);
		/* Do a direct modular reduction */
		if (n1len + n2len >= mlen)
			(void)bniDiv_64(a+mlen, a, n1len+n2len, mod, mlen);
		bniCopy_64(result, a, mlen);
		BNIFREE(a, n1len+n2len);
		return 0;
	}
	...
	t = BIGLITTLE(a-mlen, a+mlen);
	bniCopy_64(t, n1, n1len);
	bniZero_64(a, mlen);
	/* Do the division - lose the quotient into the high-order words */
	(void)bniDiv_64(t, a, mlen+n1len, mod, mlen);
	/* Copy into first table entry */
	bniCopy_64(table1[0], a, mlen);
	...
	t = BIGLITTLE(b-mlen, b+mlen);
	for (i = 1; i < tblmask; i++) {
		bniMontMul_64(a, t, table1[i-1], mod, mlen, inv);
		bniCopy_64(table1[i], BIGLITTLE(a-mlen, a+mlen), mlen);
#if BNYIELD
		if (bnYield && (y = bnYield()) < 0)
			goto yield;
#endif
	...
	bniCopy_64(t, n2, n2len);
	bniZero_64(a, mlen);
	/* Do the division - lose the quotient into the high-order words */
	(void)bniDiv_64(t, a, mlen+n2len, mod, mlen);
	/* Copy into first table entry */
	bniCopy_64(table2[0], a, mlen);

	/* Square it into a */
	bniMontSquare_64(a, table2[0], mod, mlen, inv);
	/* Copy to b, low half */
	bniCopy_64(b, t, mlen);

	/* Use b to initialize the second table */
	for (i = 1; i < buf2; i++) {
		bniMontMul_64(a, b, table2[i-1], mod, mlen, inv);
		bniCopy_64(table2[i], t, mlen);
#if BNYIELD
		if (bnYield && (y = bnYield()) < 0)
			goto yield;
#endif
	...
	if (buf2 & tblmask) {
		...
		if (mult2pos == e1bits) {
			t = BIGLITTLE(b-mlen, b+mlen);
			if (isone) {
				bniCopy_64(t, b, mlen);	/* Copy low to high */
				isone = 0;
			} else {
				bniMontMul_64(a, t, b, mod, mlen, inv);
				t = a; a = b; b = t;
			}
		}
	}
	...
	for (;;) {
		...
		if (e1bits == mult1pos) {
			...
			if (isone) {
				/* Multiply by 1 is a trivial case */
				bniCopy_64(t, mult1, mlen);
				isone = 0;
			} else {
				bniMontMul_64(a, t, mult1, mod, mlen, inv);
				/* Swap a and b */
				t = a; a = b; b = t;
			}
		}
		...
		if (e1bits == mult2pos) {
			...
			if (isone) {
				/* Multiply by 1 is a trivial case */
				bniCopy_64(t, mult2, mlen);
				isone = 0;
			} else {
				bniMontMul_64(a, t, mult2, mod, mlen, inv);
				/* Swap a and b */
				t = a; a = b; b = t;
			}
		}
...
yield:
	...
	while (--tblmask) {
		if (tblmask < buf2)
			BNIFREE(table2[tblmask], mlen);
		BNIFREE(table1[tblmask], mlen);
	}
	t = table1[0];
	BNIFREE(t, mlen);
	BNIFREE(b, 2*mlen);
 ...
 */
int
bniTwoExpMod_64(BNWORD64 *n, BNWORD64 const *exp, unsigned elen,
	BNWORD64 *mod, unsigned mlen)
{
	...
	bits = bniBits_64(mod, mlen);
	pgpAssert(bits > 1);	/* a 1-bit modulus is just stupid... */
	...
	pgpAssert(inv & 1);	/* Modulus must be odd */
	inv = bniMontInv1_64(inv);
	...
	for (;;) {
		...
		if (bitpos & bitword) {
			 ...
			 * the modulus.
			 */
			if (bniDouble_64(a1, mlen) ||
			    bniCmp_64(a1, mod, mlen) > 0)
				(void)bniSubN_64(a1, mod, mlen);
		}
		 ...
		 */
		bniMontSquare_64(b, a1, mod, mlen, inv);
	 ...
	 */
	bniCopy_64(a, a1, mlen);
	bniZero_64(a1, mlen);
	bniMontReduce_64(a, mod, mlen, inv);
	bniCopy_64(n, a1, mlen);
 ...
 */
void
bniExtractBigBytes_64(BNWORD64 const *n, unsigned char *buf,
 ...
 */
void
bniInsertBigBytes_64(BNWORD64 *n, unsigned char const *buf,
                  unsigned lsbyte,  unsigned buflen)
{
 ...
 */
void
bniExtractLittleBytes_64(BNWORD64 const *n, unsigned char *buf,
	unsigned lsbyte, unsigned buflen)
{
	BNWORD64 t = 0;	/* Needed to shut up uninitialized var warnings */
 ...
 */
void
bniInsertLittleBytes_64(BNWORD64 *n, unsigned char const *buf,
                  unsigned lsbyte,  unsigned buflen)
{
	BNWORD64 t = 0;	/* Shut up uninitialized varibale warnings */
...
#ifdef DEADCODE	/* This was a precursor to the more flexible bniExtractBytes */
/*
 ...
 */
unsigned
bniFromBytes_64(BNWORD64 *a, unsigned char const *b, unsigned blen)
{
	BNWORD64 t;
 ...
 */
int
bniGcd_64(BNWORD64 *a, unsigned alen, BNWORD64 *b, unsigned blen,
	unsigned *rlen)
{
#if BNYIELD
	int y;
	...
	while (blen != 0) {
		(void)bniDiv_64(BIGLITTLE(a-blen,a+blen), a, alen, b, blen);
		alen = bniNorm_64(a, blen);
		if (alen == 0) {
			*rlen = blen;
			return 1;
		}
		(void)bniDiv_64(BIGLITTLE(b-alen,b+alen), b, blen, a, alen);
		blen = bniNorm_64(b, alen);
#if BNYIELD
		if (bnYield && (y = bnYield()) < 0)
			return y;
#endif
 ...
 */
int
bniInv_64(BNWORD64 *a, unsigned alen, BNWORD64 const *mod, unsigned mlen)
{
	...
	alen = bniNorm_64(a, alen);
	if (!alen)
		return 1;	/* No inverse */

	mlen = bniNorm_64(mod, mlen);

	pgpAssert (alen <= mlen);

	/* Inverse of 1 is 1 */
	if (alen == 1 && BIGLITTLE(a[-1],a[0]) == 1) {
		bniZero_64(BIGLITTLE(a-alen,a+alen), mlen-alen);
		return 0;
	}

	/* Allocate a pile of space */
	BNIALLOC(b, BNWORD64, mlen+1);
	if (b) {
		/*
		 ...
		 */
		BNIALLOC(p, BNWORD64, mlen+1);
		if (p) {
			BNIALLOC(t0, BNWORD64, mlen);
			if (t0) {
				BNIALLOC(t1, BNWORD64, mlen);
				if (t1)
						goto allocated;
				BNIFREE(t0, mlen);
			}
...
allocated:
	...
	/* b = mod */
	bniCopy_64(b, mod, mlen);
	/* blen = mlen (implicitly) */
	
	/* t1 = b / a; b = b % a */
	cy = bniDiv_64(t1, b, mlen, a, alen);
	*(BIGLITTLE(t1-(mlen-alen)-1,t1+(mlen-alen))) = cy;
	t1len = bniNorm_64(t1, mlen-alen+1);
	blen = bniNorm_64(b, alen);

	/* while (b > 1) */
	while (blen > 1 || BIGLITTLE(b[-1],b[0]) != (BNWORD64)1) {
		/* q = a / b; a = a % b; */
		if (alen < blen || (alen == blen && bniCmp_64(a, a, alen) < 0))
			pgpAssert(0);
		cy = bniDiv_64(BIGLITTLE(a-blen,a+blen), a, alen, b, blen);
		*(BIGLITTLE(a-alen-1,a+alen)) = cy;
		plen = bniNorm_64(BIGLITTLE(a-blen,a+blen), alen-blen+1);
		pgpAssert(plen);
		alen = bniNorm_64(a, blen);
		if (!alen)
		...
		/* t0 += q * t1; */
		pgpAssert(plen+t1len <= mlen+1);
		bniMul_64(p, BIGLITTLE(a-blen,a+blen), plen, t1, t1len);
		plen = bniNorm_64(p, plen + t1len);
		pgpAssert(plen <= mlen);
		if (plen > t0len) {
			bniZero_64(BIGLITTLE(t0-t0len,t0+t0len), plen-t0len);
			t0len = plen;
		}
		cy = bniAddN_64(t0, p, plen);
		if (cy) {
			if (t0len > plen) {
				cy = bniAdd1_64(BIGLITTLE(t0-plen,t0+plen),
						t0len-plen, cy);
			}
			if (cy) {
		...
		if (alen <= 1 && BIGLITTLE(a[-1],a[0]) == (BNWORD64)1) {
			if (alen == 0)
				goto failure;	/* FAIL */
			pgpAssert(t0len <= mlen);
			bniCopy_64(a, t0, t0len);
			bniZero_64(BIGLITTLE(a-t0len, a+t0len), mlen-t0len);
			goto success;
		}

		/* q = b / a; b = b % a; */
		if (blen < alen || (blen == alen && bniCmp_64(b, a, alen) < 0))
			pgpAssert(0);
		cy = bniDiv_64(BIGLITTLE(b-alen,b+alen), b, blen, a, alen);
		*(BIGLITTLE(b-blen-1,b+blen)) = cy;
		plen = bniNorm_64(BIGLITTLE(b-alen,b+alen), blen-alen+1);
		pgpAssert(plen);
		blen = bniNorm_64(b, alen);
		if (!blen)
			goto failure;	/* GCD not 1 */
		...
		/* t1 += q * t0; */
		pgpAssert(plen+t0len <= mlen+1);
		bniMul_64(p, BIGLITTLE(b-alen,b+alen), plen, t0, t0len);
		plen = bniNorm_64(p, plen + t0len);
		pgpAssert(plen <= mlen);
		if (plen > t1len) {
			bniZero_64(BIGLITTLE(t1-t1len,t1+t1len), plen-t1len);
			t1len = plen;
		}
		cy = bniAddN_64(t1, p, plen);
		if (cy) {
			if (t1len > plen) {
				cy = bniAdd1_64(BIGLITTLE(t1-plen,t0+plen),
						t1len-plen, cy);
			}
			if (cy) {
	...
	/* return mod-t1 */
	bniCopy_64(a, mod, mlen);
	pgpAssert(t1len <= mlen);
	cy = bniSubN_64(a, t1, t1len);
	if (cy) {
		pgpAssert(mlen > t1len);
		cy = bniSub1_64(BIGLITTLE(a-t1len, a+t1len), mlen-t1len, cy);
		pgpAssert(!cy);
	}
