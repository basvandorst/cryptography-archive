From vince@ghost.dsi.unimi.it Thu Dec 29 22:53:48 1994
Received: from ghost.dsi.unimi.it by idea.sec.dsi.unimi.it with SMTP
	(1.38.193.4/16.2) id AA21604; Thu, 29 Dec 1994 22:53:48 +0100
Return-Path: <vince@ghost.dsi.unimi.it>
Received: by ghost.dsi.unimi.it id AA23698
  (5.65+/IDA-1.3.5 for vince@idea.sec.dsi.unimi.it); Thu, 29 Dec 94 22:53:32 +0100
Date: Thu, 29 Dec 94 22:53:32 +0100
From: David Vincenzetti <vince@ghost.dsi.unimi.it>
Message-Id: <9412292153.AA23698@ghost.dsi.unimi.it>
...
>Path: ghost.dsi.unimi.it!univ-lyon1.fr!jussieu.fr!news.oleane.net!oleane!pipex!uunet!news.inhouse.compuserve.com!news.production.compuserve.com!news
>From: Bob Jenkins <74512.261@CompuServe.COM>
>Newsgroups: alt.sources
>Subject: tool for testing random number generators
>Date: 28 Dec 1994 23:13:13 GMT
>Organization: via CompuServe Information Service
>Lines: 988
>Message-ID: <3dsre9$d1i$1@mhadf.production.compuserve.com>

/*
Followup-To: sci.crypt
Archive-name: rngtest
Submitted-by: 74512.261@CompuServe.COM  
*/
...
#define OMEGA ALPHA       /* omega bits per value in internal state */
#define OMICRON OMEGA            /* number of bits in values tested */
...
#define ind(x) ((x)&MASK)
  ...
  for (i=0; i<SIZE; ++i)
  {
    ...
    rr[i] = b & UMASK;
    ss[i] = x & UMASK;  /* ss is part of the secret internal state */
  }
  rtest->b = b;   rtest->a = a;
    prng(x,rgood);
#endif
...
u4           use;
{
  ...
  for (i=1; i<SIZE; ++i)
  {
    if (1)
    {
      v->rr[v->vc++] = (r->rr[i]+r->ss[i]-i)&UMASK;
      if (v->vc == SIZE) val_all( v, use);
    }
  }
}
...
static val *flow_allo()
{
  ...
  {
    val *vt;
    if ((i != VALNORM) && (i != VALJUMP)) continue;
    vt = val_allo( i);
    for (j=TESTLAST; j--;)
    {
      test *t;
      ...
      t = test_allo( j);
      t->all.next = vt->test;
      if (i == VALJUMP && j == TESTUNI) t->all.display = TDTEST;
      vt->test = t;
    }
    vt->next = v;
    v= vt;
