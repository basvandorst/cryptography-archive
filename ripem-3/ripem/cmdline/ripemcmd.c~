...
typedef struct {
  BOOL addRecip, includeHeaders, prependHeaders, abortIfRecipUnknown,
    myselfAsRecip, useRndMessage, askDistinguishedName;
} RIPEMFlags;
...
static char *DoEncipherDriver
  P ((RIPEMInfo *, RIPEMFlags *, FILE *, FILE *, enum enhance_mode, int, BOOL,
      int, TypList *, FILE *, RIPEMDatabase *));
static char *DoEncipherPKCSDriver
  P ((RIPEMInfo *, FILE *, FILE *, enum enhance_mode, int, RecipientKeyInfo *,
      unsigned int, BOOL, BOOL, TypList *, RIPEMDatabase *));
static char *DoDecipherDriver
  P ((RIPEMInfo *, FILE *, FILE *, int, unsigned int, BOOL, BOOL, FILE *,
      RIPEMDatabase *));
static char *DoChangePWDriver
  P ((RIPEMInfo *, BOOL, char *, unsigned int, FILE *, RIPEMDatabase *));
static char *DoGenerateKeysDriver
  P ((RIPEMInfo *, BOOL, char *, unsigned int, unsigned int, unsigned char *,
    ...
    if(Action == ACT_ENCRYPT) {
      errorMessage = DoEncipherDriver
        (&ripemInfo, &ripemFlags, InStream, OutStream, EnhanceMode,
         MessageFormat, UsePKCSFormat, EncryptionAlgorithm, &RecipientList,
      ...
      errorMessage = DoDecipherDriver
        (&ripemInfo, InStream, OutStream, GotValidityMonths, ValidityMonths,
         ripemFlags.prependHeaders, UsePKCSFormat, CertInfoStream,
         &RipemDatabase);
      if (errorMessage != (char *)NULL &&
        ...
        if (InStream != stdin) {
          ...
          errorMessage = DoDecipherDriver
            (&ripemInfo, InStream, OutStream, GotValidityMonths,ValidityMonths,
             ripemFlags.prependHeaders, TRUE, CertInfoStream, &RipemDatabase);
        }
      }
...
char *argv[];
{
  ...
  for(iarg=0; iarg<2; optind=1,optsp=1,cracking=TRUE,iarg++)  {
    while(cracking &&
          ...
          "3edgGcr:h:b:A:R:p:s:P:S:m:M:u:k:K:i:o:D:F:Z:C:y:Y:T:v:H:")) != -1) {
      switch (ch) {
        case '?':
...
/* Do processing on recipient list and email headers.  Then call DoEncipher.
   ...
   If usePKCSFormat is TRUE, this ignores messageFormat and uses
     PKCS_SIGNED|PKCS_ENVELOPED if enhanceMode is MODE_ENCRYPTED, otherwise
...
static char *DoEncipherDriver
  (ripemInfo, ripemFlags, inStream, outStream, enhanceMode, messageFormat,
   usePKCSFormat, encryptionAlgorithm, recipientList, certInfoStream,
   ripemDatabase)
RIPEMInfo *ripemInfo;
RIPEMFlags *ripemFlags;
FILE *inStream;
  ...
  do {
    ...
    if (usePKCSFormat) {
      ...
      errorMessage = DoEncipherPKCSDriver
        (ripemInfo, inStream, outStream, enhanceMode, encryptionAlgorithm,
         recipientKeys, recipientKeyCount, ripemFlags->useRndMessage,
         useHeaderList, &headerList, ripemDatabase);
      break;
    }
...
/* This is a helper to DoEncipherDriver to process a PKCS message.
   ...
   Change '\n' to <CR><LF> because RIPEMEncipherPKCSUpdate processes the
     input "as is".  This assumes inStream is opened as "r".
...
static char *DoEncipherPKCSDriver
  (ripemInfo, inStream, outStream, enhanceMode, encryptionAlgorithm,
   recipientKeys, recipientKeyCount, useRndMessage, useHeaderList, headerList,
   ripemDatabase)
RIPEMInfo *ripemInfo;
FILE *inStream;
FILE *outStream;
...
UNUSED_ARG (useRndMessage)
  ...
  do {
    ...
    if ((errorMessage = RIPEMEncipherPKCSInit
         (ripemInfo, &partOut, &partOutLen, enhanceMode == MODE_ENCRYPTED ?
          (PKCS_SIGNED|PKCS_ENVELOPED) : PKCS_SIGNED, encryptionAlgorithm,
          recipientKeys, recipientKeyCount, ripemDatabase)) != (char *)NULL)
      break;
    ...
    if (useHeaderList) {
      ...
      for (entry = headerList->firstptr; entry; entry = entry->nextptr) {
        ...
        /* Do a <CR><LF> at the end of the line.
         */
        if ((errorMessage = RIPEMEncipherPKCSUpdate
...
static char *DoDecipherDriver
  (ripemInfo, inStream, outStream, validate, validityMonths, prependHeaders,
   usePKCSFormat, certInfoStream, ripemDatabase)
RIPEMInfo *ripemInfo;
FILE *inStream;
FILE *outStream;
int validate;
...
RIPEMDatabase *ripemDatabase;
{
  ...
  enum enhance_mode enhanceMode;
  int pkcsMode, i;
  char *errorMessage, *lineIn = (char *)NULL, *messageType,
    signingTime[18];
  ...
  unsigned char *partOut, *decodedLine = (unsigned char *)NULL,
    *partOutEnd, *p;
  BOOL foundCR;
  unsigned int partOutLen, decodedLineLen;
  UINT4 endValidity;
  ...
  do {
    ...
    if ((lineIn = (char *)malloc (INPUT_BUFFER_SIZE)) == (char *)NULL) {
      errorMessage = ERROR_MALLOC;
      break;
    }
    ...
    while (fgets (lineIn, INPUT_BUFFER_SIZE, inStream)) {
      if (usePKCSFormat) {
        /* Strip whitespace off the end of the line.
         */
        for (i = strlen (lineIn) - 1; i >= 0; --i) {
          if (IsWhitespace ((int)lineIn[i]))
            lineIn[i] = '\0';
          else
            break;
        }
         ...
         */
        if (R_DecodePEMBlock
            (decodedLine, &decodedLineLen, (unsigned char *)lineIn,
             strlen (lineIn)) != 0) {
    ...
    if (usePKCSFormat) {
      if (pkcsMode == PKCS_CERTS_AND_CRLS_ONLY) {
        ...
        /* Enveloped only, there is not sender, so just print a message and
  ...
  if (lineIn != (char *)NULL) {
    R_memset ((POINTER)lineIn, 0, INPUT_BUFFER_SIZE);
    free (lineIn);
  }
...
static char *DoGenerateKeysDriver
  (ripemInfo, askDistinguishedName, username, bits, validityMonths, password,
   passwordLen, certInfoStream, ripemDatabase)
RIPEMInfo *ripemInfo;
