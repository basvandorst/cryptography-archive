 ...
 * k0,k1..k9
 */
extern void subkey_table_gen (uint8 * key, uint8 ** key_tables)
{
  int i, k ;
  for (k = 0 ; k < 10 ; k++)
    {
      uint8   key_byte = key [k] ;
      uint8 * table = (uint8 *) malloc (0x100) ;
      key_tables [k] = table ;
      for (i = 0 ; i < 0x100 ; i++)
 ...
 */
extern void skipjack_forwards (uint8 * plain, uint8 * cipher, uint8 ** key_tables)
{
  uint8 wh1 = plain[0] ;  uint8 wl1 = plain[1] ;
  uint8 wh2 = plain[2] ;  uint8 wl2 = plain[3] ;
  uint8 wh3 = plain[4] ;  uint8 wl3 = plain[5] ;
  uint8 wh4 = plain[6] ;  uint8 wl4 = plain[7] ;

  uint8 * k0 = key_tables [0] ;
  uint8 * k1 = key_tables [1] ;
  uint8 * k2 = key_tables [2] ;
  uint8 * k3 = key_tables [3] ;
  uint8 * k4 = key_tables [4] ;
  uint8 * k5 = key_tables [5] ;
  uint8 * k6 = key_tables [6] ;
  uint8 * k7 = key_tables [7] ;
  uint8 * k8 = key_tables [8] ;
  uint8 * k9 = key_tables [9] ;
...
extern void skipjack_backwards (uint8 * cipher, uint8 * plain, uint8 ** key_tables)
{
  /* setup 4 16-bit portions */
  uint8 wh1 = cipher[0] ;  uint8 wl1 = cipher[1] ;
  uint8 wh2 = cipher[2] ;  uint8 wl2 = cipher[3] ;
  uint8 wh3 = cipher[4] ;  uint8 wl3 = cipher[5] ;
  uint8 wh4 = cipher[6] ;  uint8 wl4 = cipher[7] ;
