 ...
 */
#ifndef lint
static char RCSid[] = "@(#)$Header: /usr/inet6/src.CVS/test/ttcp/orig/ttcp.c,v 1.1 1995/08/23 19:56:54 danmcd Exp $ (BRL)";
#endif
...
char **argv;
{
...
#ifndef SOLARIS
#ifdef INET6
	if (bind(fd, &sinme6, sizeof(sinme6)) < 0)
		err("bind");
#else
	...
	if (!udp)  {
...
#ifdef INET6
		if(connect(fd, &sinhim6, sizeof(sinhim6) ) < 0)
			err("connect");
#else
		...
		domain = AF_INET6;
		if((fd=accept(fd, &frominet6, &fromlen) ) < 0)
			err("accept");
		mes("accept");
#else
...
Nread( fd, buf, count )
{
	...
	if( udp )  {
#ifdef INET6
		cnt = recvfrom( fd, buf, count, 0, &from6, &len );
#else
...
again:
#ifdef INET6
		cnt = sendto( fd, buf, count, 0, &sinhim6, sizeof(sinhim6) );
#else
