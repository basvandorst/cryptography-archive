 ...
 *******************************************************************************
 * $Log: elm.c,v $
 * Revision 5.17  1994/01/08  00:09:11  Mic
 * Adds extra commands to main menu, 'P' to pipe the message though pgp
 * before printing, and '\' to pipe though PGP before doing normal pipe
	  ...
	  switch (ch) {
	    ...
	    case '\\'    :if (!access_keys())
               ...
               PutLine0(LINES-3, strlen(Prompt),
                 ...
                 "Pipe though PGP filter to a command"));
               fflush(stdout);
			   if (message_count < 1) {
			     error(catgets(elm_msg_cat, ElmSet, ElmNoMailToPipe,
			       "No mail to pipe!"));
			   ...
			   if(current > 0) {
				...
				i = process_showmsg_cmd(i);
			     ...
			     error(catgets(elm_msg_cat, ElmSet, ElmNoMailToRead,
			       "No mail to read!"));
			   break;
		case 'k'	:  if (!access_keys())
						break;
				PutLine0(LINES-3, strlen(Prompt),
					 catgets(elm_msg_cat, ElmSet, ElmMail,
					 "Mail PGP-public-key"));
				redraw += send_msg("","","",FALSE,-1,FALSE);
				break;
	    ...
	    case 'p'    :  PutLine0(LINES-3, strlen(Prompt),
			   ...
			   if (message_count < 1) {
			     error(catgets(elm_msg_cat, ElmSet, ElmNoMailToPrint,
			       "No mail to print!"));
			     fflush(stdin);
			   } else if (print_msg(TRUE,0) != 0)
			     redraw++;
			   break;
	    case 'P'    :if (!access_keys())
                        break;
               PutLine0(LINES-3, strlen(Prompt),
			     catgets(elm_msg_cat, ElmSet, ElmPrintMail,
			     "Print mail"));
               ...
               print_msg(TRUE,1);    /* the screen always needs redrawing */
			   redraw++;
			   break;
		...
		case '.'    : if (!access_keys())
		        break;
		      sprintf(junk,"PGPPASS=%s", ElmDestroyPasswd);
		      putenv(junk);
		      redraw++;
		      break;
