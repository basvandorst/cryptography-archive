 ...
 *******************************************************************************
 * $Log: mailmsg2.c,v $
 * Revision 5.37  1994/01/08  00:19:35  Mic
 * Can now mail a key file, and uses MINE headers in all pgp mailing
 * From egd017@cent1.lancs.ac.uk
 *
 * Revision 5.36  1993/11/27  05.22.25  Mic
...
int  copy_msg, edit_message, form;
{
	...
	**/
	...
	/* remember if form = -1 then it means send our public keyring to them
	 */
	...
	if (! retransmit)
	  if (form == -1) {
	  	sprintf(Cmd,"/bin/sh -c '%s -kxa +verbose=0 %s %s'",PGP_PATH, keyid, filename);
	  	if (system_call(Cmd,SY_ENAB_SIGHUP) != 0) {
			printf("could not get pgp public key of %s\n", keyid);
	  		return(need_redraw);
	  	}
	  	strcat(filename,".asc");
	  	reply = fopen(filename,"a");
	  } else 
...
int force_cmd;		/* command to do, '\0' to prompt user for cmd	*/
{
	...
	/* if they dont have pgp keys then they cant use this option! */
	
		if (!access_keys()) {
		    bad_cmd = TRUE;
		    break;
		}
	/* yes this is messy, but it works */
		if (pgpmenu(filename,form_p) == 0)
			return;
...
# endif
...
#ifndef NO_XHEADER
	if (access_keys())
		fprintf(filedesc, "X-Mailer: PGP-ELM [version %s]\n", version_buff);
	else
	...
	if (form == YES)
	  ...
	  fprintf(filedesc, "%s\n", MIME_HEADER);
	  if (pgp_encoded)
        fprintf(filedesc, "%s %s\n",MIME_CONTENTTYPE, MIME_HEADER_PGP_MESSAGE);
      else if (form == -1)
      	fprintf(filedesc, "%s %s\n",MIME_CONTENTTYPE, MIME_HEADER_PGP_PUBLIC_KEY);
      else if ((form != -1) && (!pgp_encoded))
...
char *form_p;
{
	...
	/* build up prompt and menu strings */
		if (prev_form == *form_p) {
		    ...
		    prompt_mssg = catgets(elm_msg_cat, ElmSet, ElmVfyPromptPleaseChoose,
				"Please choose one of the following options by parenthesized letter: s");
	    	strcpy(prompt_menu, catgets(elm_msg_cat, ElmSet, ElmVfyMenuUser0,
				"p)gp encode message, clear(s)ign message, e(x)it menu"));
		} else {
	    	prompt_mssg = catgets(elm_msg_cat, ElmSet, ElmVfyPromptAndNow,
				"And now: x");
	    	strcpy(prompt_menu, catgets(elm_msg_cat, ElmSet, ElmVfyMenuVfyCpy,
				"p)gp encode, s)ign, b)oth or e(x)it"));
		}
		prev_form = *form_p;
	...
	/* handle command */
		...
		case 'x':
		    if (did_prompt)
			Write_to_screen("Exit", 0);
		    return(1);
		    /*NOTREACHED*/
		case 'p':
			if (did_prompt)
			Write_to_screen("Pgp encrypt message\n\n", 0);
			strip_brackets(to);
			sprintf(Cmd,"/bin/sh -c '%s -weat +verbose=0 %s %s %s'\0",PGP_PATH, filename, to, username);
		...
		case 'b':
			if (did_prompt)
			Write_to_screen("Pgp encrypt and sign message\n\n", 0);
			strip_brackets(to);
			sprintf(Cmd,"/bin/sh -c '%s -sweat +verbose=0 %s %s %s'\0",PGP_PATH, filename, to, username);
			if (system_call(Cmd, SY_ENAB_SIGINT) == 0) {
				strcat(filename,".asc");
				pgp_encoded++;
				return(0);
		...
		case 's': 
	    	if (did_prompt)
			Write_to_screen("Clearsign message\n\n", 0);
			sprintf(Cmd,"%s -staw +verbose=0 +clearsig=on %s", PGP_PATH, filename);
			if (system_call(Cmd, SY_ENAB_SIGINT) == 0) {
				strcat(filename,".asc");
				pgp_encoded++;
				return(0);
