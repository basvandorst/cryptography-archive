 ...
 *******************************************************************************
 * $Log: mailmsg1.c,v $
 * Revision 5.8  1994/01/08  00:16:37  Mic
...
char subject[SLEN], in_reply_to[SLEN], expires[SLEN],
     ...
     cc[VERY_LONG_STRING], expanded_to[VERY_LONG_STRING], keyid[LONG_STRING],
...
char *keyid;
{
	...
	/** get the subject and return non-zero if all okay... **/
	int len = 10, prompt_line;

	prompt_line = mail_only ? 4 : LINES;

	strcpy(keyid,"\0");
	if (user_level == 0) {
	  PutLine0(prompt_line,0, catgets(elm_msg_cat, ElmSet, ElmSubjectOfMessage,
		"enter key name to send: "));
	  len = 24;
	}
	else
	  PutLine0(prompt_line,0, catgets(elm_msg_cat, ElmSet, ElmSubject, "Send Key: "));

	CleartoEOLN();

	if(optionally_enter(keyid, prompt_line, len, TRUE, FALSE)==-1){
	  /** User hit the BREAK key! **/
	  MoveCursor(prompt_line,0); 	
	  CleartoEOLN();
	  error(catgets(elm_msg_cat, ElmSet, ElmMailNotSent, "Mail not sent."));
	...
	if (strlen(keyid) == 0) {	/* no key entered -- send whole keyring */
	  	PutLine0(prompt_line,0,"Sending: Whole KeyRing");
	    CleartoEOLN();
	    strcpy(keyid,"\"\"");
	}
