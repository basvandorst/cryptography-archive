 ...
 *******************************************************************************
 * $Log: showmsg.c,v $
 * Revision 5.17  1994/01/08  00:28:53  Mic
 * Uses MIME headers to check for pgp messages.
 * From: egd017@cent1.lancs.ac.uk
 *
 * Revision 5.16  1993/11/27  05:27:59  Mic
...
int number;
{
	...
	***/
	...
	if ((current_header->status & MIME_MESSAGE) &&
	    ...
	    !getenv("NOMETAMAIL") &&
		!(current_header->status & MIME_PGP_MESSAGE)) {
	...
	if (current_header->status & MIME_PGP_MESSAGE) 
		pgp_message++;


	/* handle pgp keys */
	if (current_header->status & MIME_PGP_KEY) {
		char buffer[SLEN];
		int ret;
		int old_raw;
		...
		sprintf(buffer, "%s -Ih|%s -fka +verbose=0 +interactive=on", readmsg, PGP_PATH);
		ret = system_call(buffer, SY_ENAB_SIGINT|SY_DUMPSTATE);
		SetXYLocation(0,40);
	...
	if (pgp_message) {
		sprintf(fname,"%spgp.%d.%d", temp_dir, getpid(), getuid());
		if ((fpout = fopen(fname, "w")) != NULL) {
			ClearScreen();
			pgp_copy_message("",fpout, ( CM_DECODE || CM_REMOVE_HEADER ));
		    fclose(fpout);
			PutLine0(LINES,0, catgets(elm_msg_cat, ElmSet, ElmPressAnyKeyIndex,
	...
	/* since we cant use the headers to calculate the number of lines
	 *  . . . .  we might as well count them :)
     */

		lines = 0;
		for(;;) {
			if (fgets(junk,VERY_LONG_STRING, fpout) == 0)
				break;
			lines++;
		}
		rewind(fpout);
		clearerr(fpout);
		                  ...
		                  /* there, that wasnt too painfull was it?   */
