 ...
 *******************************************************************************
 * $Log: screen.c,v $
 * Revision 5.5  1994/01/08  00:26:58  Mic
 ...
 *
 * Revision 5.4  1993/11/27  05:25:54  Mic
...
update_title()
{
	...
	if (selected)
	  MCsprintf(buffer, catgets(elm_msg_cat, ElmSet, ElmShownWithSelect,
	      "%s is '%s' with %d shown out of %d [PGPELM %s]"),
	...
	if(access_keys() == 0)
		return(0);
	ClearLine(3);
	Centerline(3, "-=-=- PGP Encryption available -=-=-");
	setpasswd();
}
...
"a)lias, C)opy, c)hange folder, d)elete, e)dit, f)orward, g)roup reply, m)ail,"));
  ...
  "n)ext, o)ptions, p)rint, q)uit, r)eply, s)ave, t)ag, u)ndelete, or e(x)it"));
	...
	if (access_keys())
		Centerline(LINES-4,"'.' =  re-enter PGP password");
}
...
int status;
{
	...
	**/
	...
	else if (status & MIME_PGP_MESSAGE) mybuffer[1] = 'E';
	else if (status & MIME_PGP_KEY) mybuffer[1] = 'K';
#endif /* MIME */
	else 			        mybuffer[1] = ' ';
	mybuffer[2] = '\0';
