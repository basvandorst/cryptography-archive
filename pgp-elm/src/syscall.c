 ...
 *******************************************************************************
 * $Log: syscall.c,v $
 * Revision 5.10 1994/01/08  00:30:19  Mic
 ...
 *
 * Revision 5.9  1993/11/27  05:30:36  Mic
...
do_pgp_pipe()
{
	...
	sprintf(buffer, "%s -Ih|%s -f +verbose=0 | %s", readmsg, PGP_PATH, command);
	ret = system_call(buffer, SY_USER_SHELL|SY_ENAB_SIGINT|SY_DUMPSTATE);
...
int pass_through_pgp;
{
	...
	if (in_string(printout, "%s")) {
	    ...
	    if (pass_through_pgp)
		    sprintf(buffer,"(%s -Ip | %s -f +verbose=0 > %s; %s 2>&1) > /dev/null",
				readmsg, PGP_PATH, filename, printbuffer);
	    else
		    sprintf(buffer,"(%s -Ip > %s; %s 2>&1) > /dev/null",
				readmsg, filename, printbuffer);
	} else {
		if (pass_through_pgp)
		    sprintf(buffer,"%s -Ip | %s -f +verbose=0 | %s", readmsg, PGP_PATH, printout);
		else
