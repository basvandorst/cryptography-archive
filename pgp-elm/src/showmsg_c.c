...
int command;
{
	...
	while (TRUE) {
	  ...
	  switch (command) {
	    ...
	    case '\\' : if (!access_keys())
	                   break;
               put_cmd_name(catgets(elm_msg_cat, ElmSet, ElmPipe, "Pipe though pgp to a command"), TRUE);
		       (void) do_pgp_pipe();     /* do pipe - ignore return val */
		       ClearScreen();
		       build_bottom();
		       break; 
