 ...
 *******************************************************************************
 * $Log: fileio.c,v $
 * Revision 5.16  1994/01/08  00:14:08  Mic
 * Alter check in copy_message to check for MIME headers to determin if
 * the mail is pgp encoded
 * From: egd017@cent1.lancs.ac.uk
 *
 * Revision 5.15  1993/11/27  05.18.33  Mic
...
int cm_options;
{
	...
	**/
	...
	int only_header = cm_options & CM_ONLY_HEADER;
    int	end_header = 0;
    int sender_added = 0;
    int crypted = 0;
    int bytes_seen = 0;
	...
	pgp_message = (current_header->status & MIME_PGP_MESSAGE);
...
/* problem: since this routine is used so much for reading the mail box, we
 * must alter it to enable it to work ok with all the functions, or we could
 * indevidualy hack it for each of them!   :)
 */
...
int cm_options;
{
	...
	char Cmd[VERY_LONG_STRING];
	char alttempfilename[SLEN];
	char *junk;
	
	int i;
/* plan is to copy the message to a temp file, just the message!
 * then if it is a pgp message we strut our stuff, if not, then we
 * simply run copymessage properly.
 * simple? huh?
 */
	if (access_keys() == 0) {
		copy_message(prefix,dest_file,cm_options);
		return(0);
	}	


	sprintf(tempfilename,"%smesg.%d",temp_dir,getpid());
	if ((TempFile=fopen(tempfilename,"w")) == 0) {
		fputs("\nFile error with pgp\n\n",stderr);
		unlink(tempfilename);
		emergency_exit();
	}
	i = copy_message("",TempFile,CM_DECODE);
	fclose(TempFile);
	switch(i){
		case 0:
			copy_message(prefix,dest_file,cm_options);
			break;
		case 1:
		/* file contains a pgp_key.  */
		/* this is to be ignored unless we are reading the file, and that hack
		 * is put in the showmessage code  :)  ie do nothing cenpt normal
		 */
		 	copy_message(prefix,dest_file,cm_options);
		 	break;
		 default:
			strcpy(alttempfilename,tempfilename);
			strcat(alttempfilename,".pgp");
		 	sprintf(Cmd, "%s -f +verbose=0 <%s >%s.pgp",PGP_PATH,tempfilename,tempfilename);
		 	if (system_call(Cmd, SY_ENAB_SIGINT) == 0) {
		 		if (cm_options & CM_REMOVE_HEADER) {
		 			;
		 		} else {
			 		copy_message(prefix,dest_file,CM_ONLY_HEADER);
				}
		 		/* then copy the decoded file to the new file */

				if ((TempFile=fopen(alttempfilename,"r"))== 0) {
				fputs("\nError with pgp files\n",stderr);
				/* try to remove the pgp plain text befor exiting */
				
					unlink(tempfilename);
					unlink(alttempfilename);
					emergency_exit();
				}
				while (fgets(junk,SLEN,TempFile) != 0) {
					fprintf(dest_file,"%s%s",prefix,junk);
				}
				fclose(TempFile);
				
			} else {
	/* err - pgp returned an error code for some reason
	 * lets give the undecoded message then :)
	 */
			 	copy_message(prefix,dest_file,cm_options);
			} 				
			unlink(alttempfilename);    /* now get rid of the temp file */
		 	break;
	}
	unlink(tempfilename);            /* also remove temp file */
}
...
/* this file tries to find the keys.  It looks in the current directory
 * and also in the directory indicated by pgppath
 * it returns 1 if keys are found, otherwise returns 0;
 */
...
access_keys()
{
	...
	/* if pgp is not enabled then no we cant find keys!  */

	if (!pgp_enabled)
		return(0);
		
	if (access("pubring.pgp", 0) == 0)
		return(1);
	
	strcpy(Temp,getenv("PGPPATH"));
	if (strlen(Temp) <1)
		return(0);          /* pgppath not defined */
	
	/*
	 *  for systems that need the slash
	 *  at the end of PGPPATH
	 *  then we had better not add it now
	 *
	 */
	 
	if (strlen(PGP_SLASH) == 0)
		strcat(Temp,"/");
...
/*
 * this code looks to see if the enviroment variable PGPPASS has been defined
 * if so then assumes it is correct, otherwize it prompts you for the password
 * to unlock your key.
 */
 
setpasswd()
{
	char password[VERY_LONG_STRING];
	char *line = "\";
	char junk[VERY_LONG_STRING];
	int i= 0, ch, pass = 0;
	
	if ((getenv("PGPPASS") != NULL) && (strcmp(getenv("PGPPASS"),ElmDestroyPasswd) ))
		return(0);
	do {
	if (pass > 2)
		emergency_exit();
	...
	i = 0;		
	ClearLine(4);
	Centerline(4,"Enter Password :");
	for(;;) {
		ch = getchar();
		if ((i+1 > VERY_LONG_STRING)  && (ch != '\r') && (ch != '\n') && (ch != 127 )) {
			putchar('\007');
			continue;
		}
		if (((ch == '\b') || (ch == 127)) && i > 0) {
    ...
    sprintf(line,"PGPPASS=%s",password);
	putenv(line);
	pass++;
#ifdef DONT_CHECK_PASSWD
#else
	ClearLine(4);
	Centerline(4,"Checking Passwd  - - Please wait\n");
#endif
...
passwd_is_correct()
{
	...
	/*
	 * easiest way to check passwd is to try to use it
	 * get to detatch sign your own mbox file
	 * if there is an error then assume passwd was wrong.
	 */
	...
	return(1);
#else
    sprintf(fileis,"%s.sig.%d", temp_dir, getpid());
    
    sprintf(line,"%s -sbf +batchmode +force +verbose=0 < %s > %s" ,PGP_PATH, cur_folder, fileis);
    
	if (system_call(line, SY_ENAB_SIGINT) != 0) {
		unlink(fileis);
		return(0);
	}
	unlink(fileis);
