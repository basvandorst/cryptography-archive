 ...
 *******************************************************************************
 * $Log: newmbox.c,v $
 * Revision 5.35  1994/01/08  00:21:13  Mic
	...
	**/
	...
	"Error: Couldn't seek temp file %s: (offset %ld) Errno %s (%s)\n",
	  ...
	  if (in_header == TRUE) {
	    ...
	    else if ((header_cmp(buffer, "Content-Type", MIME_HEADER_PGP_MESSAGE)) && (pgp_enabled))
	      current_header->status |= MIME_MESSAGE|MIME_PGP_MESSAGE;
	    else if ((header_cmp(buffer, "Content-Type", MIME_HEADER_PGP_PUBLIC_KEY)) && (pgp_enabled))
	      current_header->status |= MIME_MESSAGE|MIME_PGP_KEY;
	     ...
	     * Sun's Openwindows mailtool */
	      ...
	      if (! (current_header->status & MIME_NOTPLAIN) && notplain(buffer+strlen(MIME_CONTENTTYPE)))
