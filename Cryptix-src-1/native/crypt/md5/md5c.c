...
static void MD5_md_Transform(unsigned long [4], unsigned char [64]);
 ...
 */
void MD5_md_Init(MD5_CTX * context)
{
...
/* MD5 block update operation. Continues an MD5 message-digest
  ...
  context.
 */
void MD5_md_Update (MD5_CTX *context, unsigned char * input, unsigned int inputLen)
{
	...
	if (inputLen >= partLen)
	{
		...
		MD5_md_Transform (context->state, context->buffer);

		for (i = partLen; i + 63 < inputLen; i += 64)
			MD5_md_Transform (context->state, &input[i]);
 ...
 */
void MD5_md_Final (unsigned char digest[16], MD5_CTX *context)
{
	...
	MD5_md_Update (context, PADDING, padLen);

	/* Append length (before padding) */
	MD5_md_Update (context, bits, 8);
 ...
 */
static void MD5_md_Transform (unsigned long state[4], unsigned char block[64])
{
