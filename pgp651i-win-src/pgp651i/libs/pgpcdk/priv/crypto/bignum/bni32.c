 ...
 *
 * $Id: bni32.c,v 1.7 1998/05/14 19:07:21 cbertsch Exp $
 *
...
bniExpMod_32(
	PGPMemoryMgrRef	mgr,
	PGPBoolean		secure,
	BNWORD32 *result, BNWORD32 const *n, unsigned nlen,
	BNWORD32 const *e, unsigned elen, BNWORD32 *mod, unsigned mlen)
{
	...
	BNIALLOC(mgr, secure, a, BNWORD32, 2*mlen);
	if (!a)
		return -1;
	BNIALLOC(mgr, secure, b, BNWORD32, 2*mlen);
	if (!b) {
		BNIFREE(a, 2*mlen);
		return -1;
	}
	 ...
	 */
	for (i = 1; i < tblmask; i++) {
		BNIALLOC(mgr, secure, t, BNWORD32, mlen);
   ...
   BNWORD32 *mod, unsigned mlen)
{
	...
	if (!e2len)
		return bniExpMod_32( mgr, secure, result, n1, n1len, e1, e1len,
							mod, mlen);
	pgpAssert(e2bits);
	...
	if (e1bits == 1) {
		...
		BNIALLOC(mgr, secure, a, BNWORD32, n1len+n2len);
		if (!a)
			return -1;
...
bniTwoExpMod_32(
	...
	BNWORD32 *n, BNWORD32 const *exp, unsigned elen,
	BNWORD32 *mod, unsigned mlen)
{
...
bniInv_32(
	PGPMemoryMgrRef	mgr,
	PGPBoolean secure,
	BNWORD32 *a, unsigned alen, BNWORD32 const *mod, unsigned mlen)
{
	...
	BNIALLOC(mgr, secure, b, BNWORD32, mlen+1);
	if (b) {
		/*
		 ...
		 */
		BNIALLOC(mgr, secure, p, BNWORD32, mlen+1);
		if (p) {
			BNIALLOC(mgr, secure, t0, BNWORD32, mlen);
			if (t0) {
				BNIALLOC(mgr, secure, t1, BNWORD32, mlen);
				if (t1)
						goto allocated;
				BNIFREE(t0, mlen);
			}
