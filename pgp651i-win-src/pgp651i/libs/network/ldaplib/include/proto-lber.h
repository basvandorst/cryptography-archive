...
LDAPFUNCDECL unsigned long ber_get_stringal( BerElement *ber, 
	struct berval **bv );
...
LDAPFUNCDECL unsigned long ber_first_element( BerElement *ber, 
	unsigned long *len, char **last );
LDAPFUNCDECL unsigned long ber_next_element( BerElement *ber, 
	unsigned long *len, char *last );
#if defined(PGPSOCKETSLDAP) || defined(MACOS) || defined(BC31) || defined(_WIN32)
...
LDAPFUNCDECL int ber_put_ostring( BerElement *ber, char *str, unsigned long len,
	unsigned long tag );
LDAPFUNCDECL int ber_put_string( BerElement *ber, char *str, 
	unsigned long tag );
