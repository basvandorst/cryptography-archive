...
printf("\
Secure Drive CryptDisk Version 1.4b\n\
\n\
...
if(tsr_wrong_version)
 {
  printf ("\nERROR: Wrong Version (not 1.4/a/b) of SECTSR is Installed\n");
  exit(1);
 }
...
if(encrypt)
    ...
    if (drvltr == 'C')
     {
      printf("WARNING: Drive C: is usually your boot drive.\n\
If you encrypt your boot drive, you will only be able to boot\n\
from a diskette. Are you SURE you want to proceed? ");
      if (!getyn()) exit(8);
     }
  ...
  for(head=0;head<maxhead;head++)
   {
    ...
    if (c_break_hit)                                 /*1.4*/
     { 
      ...
      gotoxy(1,12);                                     /*1.4b*/
      clreol();
      printf("\nWARNING: If you interrupt encryption or decryption,\n\
data sectors not yet processed may not be recoverable.\n\
If you're not VERY SURE that no useful data (i.e. nothing but\n\
empty sectors) remain, better continue processing.\n\
Continue processing? ");
if (!getyn()) break;
 else
  {
