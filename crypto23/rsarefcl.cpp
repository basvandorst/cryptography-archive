// rsarefcl.cpp - written and placed in the public domain by Wei Dai
...
RSAREFPublicKey::RSAREFPublicKey(const Integer &n, const Integer &e)
...
bool RSAREFPublicKey::Verify(const byte *message, unsigned int messageLen, const byte *signature)
{
...
#ifdef USE_BIGNUM_WITH_RSAREF
RSAREFPrivateKey::RSAREFPrivateKey(const Integer &n, const Integer &e, const Integer &d,
...
RSAREFPrivateKey::RSAREFPrivateKey(RandomNumberGenerator &rng, int keybits, bool F4)
	: keyBlock(1), key(keyBlock)
{
...
#ifdef USE_BIGNUM_WITH_RSAREF
RSAREF_DH::RSAREF_DH(const Integer &p, const Integer &g)
...
void RSAREF_DH::Agree(const byte *in, byte *agreedKey) const
{
	R_ComputeDHAgreedKey(agreedKey, const_cast<byte *>(in), 
						 privateValue.ptr, 
						 privateValue.size, 
						 const_cast<R_DH_PARAMS *>(&params));
}
